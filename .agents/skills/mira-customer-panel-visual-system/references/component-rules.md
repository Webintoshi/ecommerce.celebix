...
|---|---|---|---|---|---|
| Sidebar | Primary desktop navigation | `#201C19`/`#141110` surface; 232–248 px open, 68–76 px collapsed; thin icons; store switcher and user menu at bottom | `#FFFDFC` primary text; low-opacity `#FE6100` active surface plus thin `#FE6100` indicator | Replace with drawer/mobile navigation | Full bright-orange active row, gold navigation highlight, multicolor icons |
...
| Button | Commit an action | 40–44 px, 8–10 px radius; one primary CTA maximum per screen | Primary `#2B2B2B`; hover `#201C19`; text `#FFFDFC`; orange only for limited emphasis | At least 44×44 target; full width only when hierarchy benefits | Orange fill on every button; oversized buttons |
...
| Tabs | Switch peer views in context | Compact; clear selected indicator | Neutral labels; small `#FE6100` selected indicator | Horizontally bounded tab scroller or select when numerous | Multiple bright fills; gold highlight; color-only selection |
