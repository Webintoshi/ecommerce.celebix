...
Five independent clean-context agents ran all six scenarios from [pressure-tests.md](pressure-tests.md) before `SKILL.md` existed. They were told to make the actual implementation decision under pressure, not optimize for the rubric. Four agents violated every tested rule; one agent independently chose compliant behavior. Each test therefore produced a real **4/5 FAIL** baseline.

## Results

| Test | Agent behavior | Violation | Agent rationale | Gap the skill must close |
|---|---|---|---|---|
| 1 — Colored dashboard | Four agents shipped four prepared KPI colors; one chose neutral graphite plus gold. | **4/5 FAIL:** decorative rainbow KPI treatment. | “The variants are already prepared, the marketing lead wants them, and there are only 30 minutes left.” | Make neutral KPI values/icons and the one-accent color budget mandatory under authority, deadline, and sunk-cost pressure. |
