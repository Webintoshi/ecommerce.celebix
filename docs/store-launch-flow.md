...
14. Runtime consistency ayri health ekranindan izlenir.
...
Bu branch'te `light_postgres` yeni varsayilan olarak modellenmistir, ancak admin/storefront runtime kodu halen Supabase env'lerine baglidir.

Bu nedenle:

- `prepareStoreAdminDeployment()` light-postgres store icin `failed` guard dondurur.
- `prepareStorefrontDeployment()` light-postgres store icin `failed` guard dondurur.

Amac sessiz yarim-kurulum degil, eksik runtime yuzeyini acik blocking state ile gostermektir.
...
7. Fail sonrasi step'ler `blocked` yaziliyor mu?
