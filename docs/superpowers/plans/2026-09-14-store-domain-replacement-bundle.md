...
- [x] Create `apps/owner/scripts/sql/saas/202609140128_store_domain_replacement_bundles.up.sql` with `saas.store_domain_replacements`, immutable operation identity/fingerprint, tenant-bound foreign keys, a partial unique index that permits only one status in `preparing|activated|rolled_back` per store, projection/list, prepare, activate, cancel, and rollback functions. Sequence 128 avoids the independently developed Orders archive and safe-delete sequence-127 migrations; this replacement migration was still unapplied when renamed.
