...
Atlas reviewed the required additive contract and SQL migration `202609260157_category_images_and_order`.
...
- Disposable PostgreSQL harness: 14 scenarios pass, including migration apply/assertions/empty rollback, image validation/preservation/clear, asset archive guard, atomic multi-group order, stale membership/version rejection, concurrent update winner and concurrent same-operation replay. No live database was used.
...
1. Apply SQL157 and its assertions with the existing migration workflow before starting the new runtime; it requires the migration at preflight.
