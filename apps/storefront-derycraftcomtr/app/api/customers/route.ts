...
import {
    ...
    isStorefrontPublicOrdersCustomersDisabled,
} from "@/lib/supabase-disconnect-readiness";

function requirePrivateCustomersAccess(request: NextRequest) {
    if (!isStorefrontPublicOrdersCustomersDisabled() || isInternalApiRequest(request)) {
        return null;
    }
    ...
    return NextResponse.json(
        {
            ...
            error: "DeryCraft light_postgres provasinda bu customers endpoint'i yalnizca ic otomasyon icin aciktir.",
        },
        { status: 403 }
    );
}
...
export async function GET(request: NextRequest) {
    const accessResponse = requirePrivateCustomersAccess(request);
    if (accessResponse) {
