...
function normalizeOrderItem(item: Record<string, unknown>): OrderItem {
  ...
  const productRecord = Array.isArray(productValue)
    ? productValue.find((entry): entry is Record<string, unknown> => isRecord(entry)) ?? null
  ...
  return {
    ...
    variant_name: toString(item.variant_name),
    ...
    product: productRecord
      ? {
          images: Array.isArray(productRecord.images)
            ? productRecord.images.filter((entry): entry is string => typeof entry === "string" && entry.length > 0)
            : [],
          category: typeof productRecord.category === "string" ? productRecord.category : "",
        }
      : null,
...
export default async function OrderSuccessPage({
  ...
  try {
    ...
    order = loadedOrder as Order | null;
  } catch (orderError) {
    console.error("Order success page order read failed:", orderError);
  }

  try {
    paymentGateways = (await getStoredPaymentGateways()) as PaymentGateway[];
  } catch (settingsError) {
    console.error("Order success page payment gateway read failed:", settingsError);
  }
  ...
  const paymentBanner = getPaymentBanner(resolvedSearchParams.payment || order.payment_status || undefined);
        ...
        <section className="grid gap-8 xl:grid-cols-[1.25fr_0.85fr]">
          <div className="rounded-[30px] border border-stone-200 bg-white p-6 shadow-[0_14px_42px_rgba(15,23,42,0.05)] sm:p-8">
            ...
            <div className="mt-8 space-y-5">
              {items.map((item) => {
                const imageSource = resolveStorefrontAssetUrl(
