...
const PREVIEW_COPY = [
  ...
  {
    lead: null,
    text: "Seçilen kişiselleştirme alanına baş harf, isim veya özel metin eklenebilir. Metin, tanımlı baskı alanına net ve zarif şekilde uygulanır.",
  },
  {
    lead: null,
    text: "Metniniz, yazdığınız büyük-küçük harf ve boşluk düzeni korunarak hazırlanır. Lütfen dikkatle kontrol edin.",
  },
  {
    lead: "Not:",
    text: "Kişiselleştirilmiş siparişlerde iade ve değişim yapılamaz.",
  },
];
...
export function PersonalizationPreview({
  ...
  const displayText = typedText || previewConfig.defaultText || "Ön İzleme";
  ...
  }, [displayText, selectedFont.faceName, selectedFont.publicSrc]);
  ...
  useLayoutEffect(() => {
    ...
    const baseFontSize = isWatchPreview ? BASE_WATCH_FONT_SIZE : BASE_LEATHER_FONT_SIZE;
    const minimumFontSize = isWatchPreview ? MIN_WATCH_FONT_SIZE : MIN_LEATHER_FONT_SIZE;
    const maximumFontSize = isWatchPreview ? MAX_WATCH_FONT_SIZE : MAX_LEATHER_FONT_SIZE;
    ...
    const measuredHeight = measurementElement.getBoundingClientRect().height || baseFontSize;
    let nextFontSize = (baseFontSize * targetVisualHeightPx) / measuredHeight;
    ...
    <section className="mx-auto w-full max-w-[360px] border-t border-neutral-200 pt-4">
      ...
      <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_170px]">
        ...
        <input
          ...
          value={previewText}
          onChange={(event) =>
            setPreviewText(event.target.value.slice(0, previewConfig.maxCharacters))
          }
