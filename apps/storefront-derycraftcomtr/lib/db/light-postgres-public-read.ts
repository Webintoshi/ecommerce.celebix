...
function normalizeCategoryRow(row: CategoryRow): LightPostgresCategoryRecord {
  return {
    ...
    isActive: row.is_active === undefined ? true : normalizeBoolean(row.is_active),
    seoTitle: row.seo_title,
    seoDescription: row.seo_description,
    seoKeywords: normalizeStringArray(row.seo_keywords),
    createdAt: row.created_at,
...
export const LIGHT_POSTGRES_PUBLIC_READ_SQL = {
  ...
  `,
  listProductVariants: `
    select id, product_id, name, sku, barcode, price, original_price, stock, weight, images, attributes, created_at,
