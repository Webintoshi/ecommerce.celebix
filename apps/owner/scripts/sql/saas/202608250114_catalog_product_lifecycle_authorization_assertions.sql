...
BEGIN
  ...
  FOREACH function_name IN ARRAY ARRAY[
    ...
    'saas.catalog_archive_variant(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,uuid,text,uuid,uuid,bigint)'
  ] LOOP
  ...
  FOREACH function_name IN ARRAY ARRAY[
    'saas.catalog_create_product_implementation_v1(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,uuid,text,uuid,uuid,text,text,text,text,text,text,text,text,bigint,bigint,bigint,boolean,bigint,jsonb)',
    'saas.catalog_update_product_implementation_v1(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,uuid,text,uuid,bigint,text,text,text,text,text)',
    'saas.catalog_archive_product_implementation_v1(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,uuid,text,uuid,bigint)',
    'saas.catalog_create_variant_implementation_v1(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,uuid,text,uuid,uuid,text,text,text,bigint,bigint,bigint,boolean,bigint,jsonb)',
    'saas.catalog_update_variant_implementation_v1(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,uuid,text,uuid,uuid,bigint,text,text,text,bigint,bigint,bigint,boolean,bigint,jsonb)',
    'saas.catalog_archive_variant_implementation_v1(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,uuid,text,uuid,uuid,bigint)'
  ] LOOP
    IF pg_catalog.to_regprocedure(function_name) IS NULL
       OR pg_catalog.has_function_privilege('celebix_saas_app',function_name,'EXECUTE') THEN
      RAISE EXCEPTION 'CATALOG_PRODUCT_LIFECYCLE_ACL_INVALID';
    END IF;
  END LOOP;
  ...
  FOREACH function_name IN ARRAY ARRAY[
    ...
    IF pg_catalog.strpos(pg_catalog.pg_get_functiondef(function_name::pg_catalog.regprocedure),'catalog_admin.manage')=0 THEN
      RAISE EXCEPTION 'CATALOG_PRODUCT_LIFECYCLE_ACL_INVALID';
    END IF;
  END LOOP;
  ...
  FOREACH function_name IN ARRAY ARRAY[
    ...
    IF pg_catalog.strpos(pg_catalog.pg_get_functiondef(function_name::pg_catalog.regprocedure),'catalog_admin.archive')=0 THEN
      RAISE EXCEPTION 'CATALOG_PRODUCT_LIFECYCLE_ACL_INVALID';
    END IF;
  END LOOP;
  ...
  IF EXISTS(
    ...
    JOIN saas.products AS product
      ON product.id=variant.product_id AND product.store_id=variant.store_id
    WHERE product.status='archived' AND variant.status='archived'
      AND product.archived_at IS NOT NULL
      AND variant.archived_at=product.archived_at
      AND variant.archived_by_product=false
  ) OR EXISTS(
    SELECT 1
    FROM saas.product_variants AS variant
    WHERE variant.archived_by_product=true
      AND NOT EXISTS(
        SELECT 1 FROM saas.products AS product
        WHERE product.id=variant.product_id AND product.store_id=variant.store_id
          AND product.status='archived' AND variant.status='archived'
          AND product.archived_at IS NOT NULL
          AND variant.archived_at=product.archived_at
      )
  ) THEN
    RAISE EXCEPTION 'CATALOG_PRODUCT_LEGACY_PROVENANCE_INVALID';
  END IF;
