...
BEGIN
  ...
  IF pg_catalog.to_regprocedure('saas.promotion_evaluator_context_valid(uuid,jsonb)') IS NULL
     ...
     OR pg_catalog.to_regprocedure('saas.promotion_evaluator_empty_result(text,text)') IS NULL THEN RAISE EXCEPTION 'PROMOTIONS_STUDIO_EVALUATOR_HELPERS_INVALID'; END IF;
