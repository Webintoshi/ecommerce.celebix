...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid = 'saas.admin_domains'::regclass
      AND conname = 'admin_domains_management_check'
  ) THEN
    RAISE EXCEPTION 'CELEBIX_NET_ADMIN_DOMAIN_MANAGEMENT_PRECONDITION_FAILED';
  END IF;
END
$precondition$;
...
AS $celebix_net_admin_domain_management$
DECLARE
...
BEGIN
  ...
  IF FOUND THEN
    IF existing.id <> p_domain_id OR existing.store_id <> p_store_id OR existing.hostname <> p_hostname
       OR existing.kind <> 'platform_subdomain' OR existing.management <> 'platform'
       OR existing.status <> 'active' OR NOT existing.canonical OR existing.verified_at IS NULL THEN
  ...
  INSERT INTO saas.admin_domains(
    id, store_id, hostname, kind, management, status, canonical, verified_at, version, created_at, updated_at
  ) VALUES (
    p_domain_id, p_store_id, p_hostname, 'platform_subdomain', 'platform', 'active', true, p_now, 1, p_now, p_now
  );
...
END
$celebix_net_admin_domain_management$;
