...
DO $side_cart_quantity_selector_precondition$
BEGIN
  IF pg_catalog.to_regclass('saas.campaign_starter_publications') IS NULL
     ...
     OR pg_catalog.to_regprocedure('saas.campaign_starter_composition_valid_without_quantity_selector(jsonb)') IS NOT NULL THEN
    RAISE EXCEPTION 'SIDE_CART_QUANTITY_SELECTOR_PRECONDITION_FAILED';
  END IF;
END
$side_cart_quantity_selector_precondition$;
...
ALTER FUNCTION saas.campaign_starter_composition_valid(jsonb)
  RENAME TO campaign_starter_composition_valid_without_quantity_selector;
...
AS $function$
  SELECT CASE p_config->>'schemaVersion'
    WHEN '1' THEN saas.campaign_starter_composition_valid_without_quantity_selector(p_config)
    WHEN '2' THEN
      saas.campaign_starter_exact_keys(
        p_config->'cart',
        ARRAY['showCheckoutReadiness','showShippingProgress','showQuantitySelector'],
        ARRAY['trustMessage']
      )
      AND pg_catalog.jsonb_typeof(p_config->'cart'->'showQuantitySelector')='boolean'
      AND saas.campaign_starter_composition_valid_without_quantity_selector(
        p_config #- ARRAY['cart','showQuantitySelector']
      )
    ELSE false
  END
$function$;
...
SET config=pg_catalog.jsonb_set(config,ARRAY['cart','showQuantitySelector'],'true'::jsonb,true)
WHERE config->>'schemaVersion'='2' AND NOT (config->'cart'?'showQuantitySelector');

UPDATE saas.storefront_designs
SET draft_config=CASE
      WHEN draft_config->'composition'->>'schemaVersion'='2' AND NOT (draft_config->'composition'->'cart'?'showQuantitySelector')
        THEN pg_catalog.jsonb_set(draft_config,ARRAY['composition','cart','showQuantitySelector'],'true'::jsonb,true)
      ELSE draft_config
    END,
    published_config=CASE
      WHEN published_config->'composition'->>'schemaVersion'='2' AND NOT (published_config->'composition'->'cart'?'showQuantitySelector')
        THEN pg_catalog.jsonb_set(published_config,ARRAY['composition','cart','showQuantitySelector'],'true'::jsonb,true)
