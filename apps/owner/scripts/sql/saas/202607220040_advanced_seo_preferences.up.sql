-- Phase 3H advanced SEO preferences authority. No provider credentials or arbitrary HTML are persisted.
BEGIN;
...
CREATE OR REPLACE FUNCTION saas.merchant_admin_setting_text(p_value jsonb,p_min integer,p_max integer)
...
CREATE OR REPLACE FUNCTION saas.merchant_admin_setting_https_media_url(p_value jsonb)
RETURNS boolean LANGUAGE sql IMMUTABLE STRICT SET search_path=pg_catalog,saas AS $f$
...
CREATE OR REPLACE FUNCTION saas.merchant_admin_setting_destination(p_value jsonb)
RETURNS boolean LANGUAGE sql IMMUTABLE STRICT SET search_path=pg_catalog,saas AS $f$
 SELECT saas.merchant_admin_setting_text(p_value,1,2048)
...
CREATE OR REPLACE FUNCTION saas.merchant_admin_setting_email(p_value jsonb)
RETURNS boolean LANGUAGE sql IMMUTABLE STRICT SET search_path=pg_catalog,saas AS $f$
...
CREATE OR REPLACE FUNCTION saas.merchant_admin_setting_timestamp_value(p_value jsonb)
...
CREATE OR REPLACE FUNCTION saas.merchant_admin_setting_timestamp(p_value jsonb)
RETURNS boolean LANGUAGE sql IMMUTABLE STRICT SET search_path=pg_catalog,saas AS $f$
...
RETURNS text LANGUAGE sql IMMUTABLE STRICT SET search_path=pg_catalog,saas AS $f$
 SELECT CASE
  ...
  WHEN p_kind IN('marketplace_connection','accounting_profile','invoice_integration','seo_control','sitemap','social_preview','code_integration','indexing_request','seo_geo_profile','seo_internal_link','seo_content_entry','seo_category_entry','seo_page_entry','seo_product_entry') THEN CASE WHEN p_mutation THEN 'integrations.manage' ELSE 'integrations.read' END
  WHEN p_kind='ai_setting' THEN CASE WHEN p_mutation THEN 'configuration.manage' ELSE 'configuration.read' END
  ELSE NULL END
$f$;
