-- Normalize deterministic manual-order identifiers to the public UUID contract.
BEGIN;
SET LOCAL ROLE celebix_saas_owner;

DO $manual_order_uuid_contract$
DECLARE
...
BEGIN
  IF convert_signature IS NULL OR transition_signature IS NULL
     OR pg_catalog.to_regprocedure('saas.inventory_deterministic_uuid(text,text)') IS NULL THEN
    RAISE EXCEPTION 'MANUAL_ORDER_UUID_CONTRACT_PRECONDITION';
  END IF;
  ...
  IF pg_catalog.strpos(convert_definition,old_order_id)=0
     OR pg_catalog.strpos(convert_definition,old_item_id)=0
     OR pg_catalog.strpos(convert_definition,old_created_event_id)=0
     OR pg_catalog.strpos(transition_definition,old_transition_event_id)=0
     OR pg_catalog.strpos(convert_definition,new_order_id)>0
     OR pg_catalog.strpos(convert_definition,new_item_id)>0
     OR pg_catalog.strpos(convert_definition,new_created_event_id)>0
     OR pg_catalog.strpos(transition_definition,new_transition_event_id)>0 THEN
    RAISE EXCEPTION 'MANUAL_ORDER_UUID_CONTRACT_SOURCE_MISMATCH';
  END IF;

  convert_definition := pg_catalog.replace(convert_definition,old_order_id,new_order_id);
  convert_definition := pg_catalog.replace(convert_definition,old_item_id,new_item_id);
  convert_definition := pg_catalog.replace(convert_definition,old_created_event_id,new_created_event_id);
  transition_definition := pg_catalog.replace(transition_definition,old_transition_event_id,new_transition_event_id);
  EXECUTE convert_definition;
  EXECUTE transition_definition;
END
$manual_order_uuid_contract$;
