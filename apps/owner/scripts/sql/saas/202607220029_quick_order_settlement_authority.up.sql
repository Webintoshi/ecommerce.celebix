-- Phase 3B2 additive quick-order settlement cardinality authority repair.
BEGIN;
...
AS $function$
  SELECT pg_catalog.jsonb_build_object(
    ...
    'itemCount',(SELECT pg_catalog.count(*) FROM saas.quick_order_link_items AS item
      WHERE item.store_id=attempt.store_id AND item.quick_order_link_id=attempt.quick_order_link_id),
...
AS $function$
  SELECT saas.quick_checkout_attempt_authority_projection(p_attempt_id)||pg_catalog.jsonb_build_object(
    'leaseToken',p_lease_token,'attemptNumber',p_attempt_number
  )
$function$;
