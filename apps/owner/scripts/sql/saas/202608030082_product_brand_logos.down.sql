...
DO $product_brand_logos_down_precondition$
BEGIN
  IF pg_catalog.to_regprocedure('saas.public_product_brand_logo(uuid,jsonb)') IS NULL
     OR pg_catalog.to_regprocedure('saas.public_campaign_product_projection(uuid,uuid,timestamp with time zone)') IS NULL THEN
    RAISE EXCEPTION 'PRODUCT_BRAND_LOGOS_DOWN_PRECONDITION_FAILED';
  END IF;
END
$product_brand_logos_down_precondition$;
...
DROP FUNCTION saas.public_product_brand_logo(uuid,jsonb);
