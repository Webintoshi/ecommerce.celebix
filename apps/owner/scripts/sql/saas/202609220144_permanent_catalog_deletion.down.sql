...
BEGIN
  IF EXISTS(
    SELECT 1 FROM saas.record_deletion_operations
    WHERE resource_kind IN ('product','category')
  ) OR EXISTS(SELECT 1 FROM saas.product_deletion_preparations)
  THEN RAISE EXCEPTION 'PERMANENT_CATALOG_DELETION_ROLLBACK_BLOCKED'; END IF;
END
$block$;
DROP FUNCTION saas.delete_category_recover(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamptz,uuid,text);
DROP FUNCTION saas.delete_category(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamptz,uuid,text,uuid,bigint,text);
DROP FUNCTION saas.catalog_category_deletion_impact(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamptz,uuid);
DROP FUNCTION saas.catalog_detach_category_reference(jsonb,text);
DROP FUNCTION saas.delete_product_recover(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamptz,uuid,text);
DROP FUNCTION saas.delete_product(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamptz,uuid,text,uuid,bigint,text);
DROP FUNCTION saas.catalog_product_deletion_impact(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamptz,uuid);
DROP TRIGGER product_deletion_preparations_immutable ON saas.product_deletion_preparations;
DROP TABLE saas.product_deletion_preparations;
DROP FUNCTION saas.guard_product_deletion_preparation();
