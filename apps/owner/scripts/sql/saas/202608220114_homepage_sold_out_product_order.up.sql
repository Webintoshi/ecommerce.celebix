...
BEGIN
 ...
 FOR section IN SELECT value FROM pg_catalog.jsonb_array_elements(presentation->'sections') LOOP
  ...
  SELECT COALESCE(pg_catalog.jsonb_agg(ordered.value ORDER BY COALESCE((ordered.value->>'available')::boolean,false) DESC,ordered.ordinality),'[]'::jsonb) INTO items
  FROM pg_catalog.jsonb_array_elements(items) WITH ORDINALITY AS ordered(value,ordinality);
