...
LOCK TABLE saas.shipping_shipment_action_operations,saas.shipping_shipment_action_jobs,saas.shipping_shipment_labels,saas.shipping_shipment_returns,
  saas.shipping_fulfillment_operations,saas.shipping_shipment_events,
...
DO $function$
BEGIN
  IF EXISTS(SELECT 1 FROM saas.shipping_shipment_action_operations)
    OR EXISTS(SELECT 1 FROM saas.shipping_shipment_action_jobs)
    OR EXISTS(SELECT 1 FROM saas.shipping_shipment_labels)
    OR EXISTS(SELECT 1 FROM saas.shipping_shipment_returns)
    OR EXISTS(SELECT 1 FROM saas.shipping_fulfillment_operations)
...
DROP FUNCTION saas.shipping_shipment_action_mark_unknown(uuid,text,uuid,bigint,timestamptz,uuid,text);
DROP FUNCTION saas.shipping_shipment_action_fail(uuid,text,uuid,bigint,timestamptz,text,text);
DROP FUNCTION saas.shipping_shipment_action_complete(uuid,text,uuid,bigint,timestamptz,uuid,text,text,text,text,text,bigint,text,text);
DROP FUNCTION saas.shipping_shipment_action_open(uuid,text,uuid,bigint,timestamptz);
DROP FUNCTION saas.shipping_shipment_action_claim(uuid,text,timestamptz,integer,uuid);
DROP FUNCTION saas.shipping_shipment_label_current(uuid,uuid,uuid,uuid,text,bigint,timestamptz,uuid,uuid);
DROP FUNCTION saas.shipping_shipment_action_recover(uuid,uuid,uuid,uuid,text,bigint,timestamptz,uuid,text);
DROP FUNCTION saas.shipping_shipment_action_begin(uuid,uuid,uuid,uuid,text,bigint,timestamptz,uuid,uuid,bigint,text,uuid,text,uuid);
...
DROP TRIGGER shipping_shipment_action_operations_immutable ON saas.shipping_shipment_action_operations;
...
DROP TABLE saas.shipping_shipment_action_operations;
DROP TABLE saas.shipping_shipment_action_jobs;
DROP TABLE saas.shipping_shipment_labels;
DROP TABLE saas.shipping_shipment_returns;
