...
DO $storefront_hero_slider_down_guard$
BEGIN
  IF pg_catalog.current_setting('celebix.allow_storefront_hero_slider_down',true) IS DISTINCT FROM 'on' THEN
  ...
  IF EXISTS(
    ...
    WHERE design.schema_version<>2
       ...
       OR pg_catalog.jsonb_array_length(design.draft_config->'hero'->'slides')<>1
       OR pg_catalog.jsonb_array_length(design.published_config->'hero'->'slides')<>1
       OR design.draft_config->'hero'->'slides'->0->'mobileImage'<>'null'::jsonb
       OR design.published_config->'hero'->'slides'->0->'mobileImage'<>'null'::jsonb
       OR (design.draft_config->'hero'->'slides'->0->>'enabled')::boolean IS NOT TRUE
       OR (design.published_config->'hero'->'slides'->0->>'enabled')::boolean IS NOT TRUE
  ) THEN
    RAISE EXCEPTION 'STOREFRONT_HERO_SLIDER_DOWN_DATA_LOSS';
  END IF;
END
$storefront_hero_slider_down_guard$;

CREATE FUNCTION saas.storefront_design_downgrade_v1(p_config jsonb)
RETURNS jsonb
LANGUAGE sql
STABLE
STRICT
...
AS $function$
  SELECT pg_catalog.jsonb_build_object(
    ...
    'hero',pg_catalog.jsonb_build_object(
      'headline',p_config->'hero'->'slides'->0->>'headline',
      'body',p_config->'hero'->'slides'->0->>'body',
      'image',p_config->'hero'->'slides'->0->'desktopImage',
      'destination',p_config->'hero'->'slides'->0->'destination',
...
ALTER TABLE saas.storefront_designs ALTER COLUMN schema_version SET DEFAULT 1;
UPDATE saas.storefront_designs
SET schema_version=1,
    draft_config=saas.storefront_design_downgrade_v1(draft_config),
    published_config=saas.storefront_design_downgrade_v1(published_config);
ALTER TABLE saas.storefront_designs ADD CONSTRAINT storefront_designs_schema_version_check CHECK(schema_version=1);
...
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,saas AS $function$
  SELECT pg_catalog.jsonb_build_object(
    'schemaVersion',1,'publicationVersion',p_version,'publishedAt',saas.storefront_design_timestamp(p_published_at),
    'brand',pg_catalog.jsonb_build_object(
      'logo',saas.storefront_design_public_media(p_store_id,p_config->'brand'->'logo'),'favicon',saas.storefront_design_public_media(p_store_id,p_config->'brand'->'favicon'),
      'primaryColor',p_config->'brand'->>'primaryColor','accentColor',p_config->'brand'->>'accentColor','backgroundColor',p_config->'brand'->>'backgroundColor','textColor',p_config->'brand'->>'textColor','fontFamily',p_config->'brand'->>'fontFamily'
    ),
    'hero',pg_catalog.jsonb_build_object(
      'headline',p_config->'hero'->>'headline','body',p_config->'hero'->>'body','image',saas.storefront_design_public_media(p_store_id,p_config->'hero'->'image'),
      'destination',saas.storefront_design_public_destination(p_store_id,p_config->'hero'->'destination'),'enabled',p_config->'hero'->'enabled'
    ),
    'promotion',pg_catalog.jsonb_build_object(
      'headline',p_config->'promotion'->>'headline','body',p_config->'promotion'->>'body','destination',saas.storefront_design_public_destination(p_store_id,p_config->'promotion'->'destination'),
...
BEGIN
  ...
  UPDATE saas.storefront_designs SET published_config=draft_config,published_version=published_version+1,published_at=p_now,published_by=p_principal_id WHERE store_id=p_store_id RETURNING * INTO current_design;
...
DROP FUNCTION saas.storefront_design_publishable(uuid,jsonb);
DROP FUNCTION saas.storefront_design_upgrade_v2(jsonb,boolean);
DROP FUNCTION saas.storefront_design_downgrade_v1(jsonb);
