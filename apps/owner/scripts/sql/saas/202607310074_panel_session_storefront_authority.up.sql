...
DO $panel_session_storefront_precondition$
BEGIN
  IF pg_catalog.to_regclass('saas.panel_sessions') IS NULL
     OR pg_catalog.to_regclass('saas.store_domains') IS NULL
     OR pg_catalog.to_regprocedure('saas.resolve_panel_session(text,text,timestamptz)') IS NULL
     OR NOT EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = 'celebix_saas_identity') THEN
    RAISE EXCEPTION 'PANEL_SESSION_STOREFRONT_AUTHORITY_PRECONDITION_FAILED';
  END IF;
END
$panel_session_storefront_precondition$;
...
AS $panel_session_storefront_resolve$
DECLARE
...
BEGIN
  ...
  SELECT pg_catalog.jsonb_strip_nulls(pg_catalog.jsonb_build_object(
    ...
    'resolvedHost', CASE WHEN primary_domain.id IS NULL THEN NULL ELSE pg_catalog.jsonb_build_object(
      'schemaVersion', 1,
      'hostname', primary_domain.hostname,
      'domainId', primary_domain.id,
      'domainType', CASE primary_domain.hostname_type WHEN 'custom_domain' THEN 'custom' ELSE primary_domain.hostname_type END,
      'storeId', store.id,
      'storeSlug', store.slug,
      'canonicalHostname', primary_domain.hostname,
      'status', 'active',
      'cacheVersion', primary_domain.version
    ) END,
  ...
  LEFT JOIN saas.store_domains AS primary_domain
...
END
$panel_session_storefront_resolve$;
