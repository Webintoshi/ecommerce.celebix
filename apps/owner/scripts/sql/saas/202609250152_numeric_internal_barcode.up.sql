...
-- These are closed-system Code 128 identifiers, not GS1 GTINs. Existing CXI
-- values and the v1 functions stay intact for rolling-release compatibility.
DO $capacity$
BEGIN
  IF EXISTS (SELECT 1 FROM saas.barcode_label_sequences WHERE last_value >= 9999999)
  THEN RAISE EXCEPTION 'numeric_internal_barcode_sequence_capacity_unavailable'; END IF;
END
$capacity$;
CREATE UNIQUE INDEX product_variants_store_numeric_internal_barcode_key
  ON saas.product_variants(store_id,barcode)
  WHERE barcode ~ '^97[0-9]{7}$';
