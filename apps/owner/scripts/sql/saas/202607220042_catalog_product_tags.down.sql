...
BEGIN
  IF EXISTS(
    ...
    WHERE resource_kind='tag'
  ) THEN
    RAISE EXCEPTION 'CATALOG_PRODUCT_TAGS_ROLLBACK_BLOCKED';
  END IF;
END
$f$;
...
ALTER TABLE saas.catalog_admin_resources
  ...
  ADD CONSTRAINT catalog_admin_resources_resource_kind_check
    CHECK (
      resource_kind = ANY (
        ARRAY['collection','brand','attribute','extra','definition']::text[]
      )
    );
...
BEGIN
  ...
  IF e IS NOT NULL THEN
    ...
    RETURN;
  END IF;
  IF p_kind NOT IN('collection','brand','attribute','extra','definition') THEN
...
BEGIN
  ...
  IF
    ...
    OR p_kind NOT IN('collection','brand','attribute','extra','definition')
    OR p_name IS NULL
