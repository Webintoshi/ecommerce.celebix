-- Allow a merchant to deliberately deactivate a reference. Dependent new sales
-- become unavailable; fixed-price products and historical snapshots remain intact.
BEGIN;
...
BEGIN
  ...
  END IF;
  -- The preview may contain unavailable dynamic variants. Publishing this set
  -- intentionally blocks their new sales; no cached price is substituted.
