...
RETURNS boolean LANGUAGE sql IMMUTABLE STRICT SET search_path=pg_catalog,saas AS $f$
 SELECT pg_catalog.jsonb_typeof(p_value)='string'
   AND pg_catalog.char_length(p_value#>>'{}') BETWEEN p_min AND p_max
...
RETURNS boolean LANGUAGE sql IMMUTABLE STRICT SET search_path=pg_catalog,saas AS $f$
 SELECT saas.merchant_admin_setting_text(p_value,1,2048)
   AND p_value#>>'{}' ~ '^https://[a-z0-9]([a-z0-9.-]*[a-z0-9])?(/[A-Za-z0-9._~!$&''()*+,;=:@%/-]*)?$'
$f$;
...
RETURNS boolean LANGUAGE sql IMMUTABLE STRICT SET search_path=pg_catalog,saas AS $f$
 SELECT pg_catalog.jsonb_typeof(p_config)='object' AND pg_catalog.pg_column_size(p_config)<=16384
