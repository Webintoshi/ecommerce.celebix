...
RETURNS jsonb LANGUAGE sql STABLE SET search_path=pg_catalog,saas AS $f$
 ...
 highlights AS (SELECT COALESCE(pg_catalog.jsonb_agg(value ORDER BY name,id),'[]'::jsonb) value FROM (SELECT value,name,id FROM (SELECT config->>'text' value,name,id,pg_catalog.row_number() OVER(PARTITION BY config->>'text' ORDER BY name,id) ordinal FROM linked WHERE resource_kind='definition' AND saas.campaign_starter_exact_keys(config,ARRAY['role','text']) AND config->>'role'='highlight' AND saas.campaign_starter_text_valid(config->'text',1,300)) candidates WHERE ordinal=1 ORDER BY name,id LIMIT 12) selected),
 ...
 certifications AS (SELECT COALESCE(pg_catalog.jsonb_agg(value ORDER BY name,id),'[]'::jsonb) value FROM (SELECT value,name,id FROM (SELECT config->>'label' value,name,id,pg_catalog.row_number() OVER(PARTITION BY config->>'label' ORDER BY name,id) ordinal FROM linked WHERE resource_kind='extra' AND saas.campaign_starter_exact_keys(config,ARRAY['role','label']) AND config->>'role'='certification' AND saas.campaign_starter_text_valid(config->'label',1,160)) candidates WHERE ordinal=1 ORDER BY name,id LIMIT 12) selected),
