...
DO $f$
DECLARE routine_oid oid:=pg_catalog.to_regprocedure('saas.storefront_hosted_checkout_terminal_transition()');
BEGIN
  IF pg_catalog.current_setting(
    'celebix.allow_storefront_hosted_checkout_cart_drift_settlement_down',true
  )<>'on' THEN
    RAISE EXCEPTION 'STOREFRONT_HOSTED_CHECKOUT_CART_DRIFT_SETTLEMENT_DOWN_GUARD_REQUIRED';
  END IF;
  IF routine_oid IS NULL THEN
    RAISE EXCEPTION 'STOREFRONT_HOSTED_CHECKOUT_CART_DRIFT_SETTLEMENT_DOWN_FUNCTION_MISSING';
  END IF;
  IF NOT EXISTS(
    ...
    RAISE EXCEPTION 'STOREFRONT_HOSTED_CHECKOUT_CART_DRIFT_SETTLEMENT_DOWN_FUNCTION_CHANGED';
  END IF;
END
$f$;

CREATE OR REPLACE FUNCTION saas.storefront_hosted_checkout_terminal_transition()
RETURNS trigger
