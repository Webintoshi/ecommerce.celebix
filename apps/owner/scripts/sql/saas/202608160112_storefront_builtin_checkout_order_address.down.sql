...
BEGIN
  IF pg_catalog.current_setting('celebix.allow_storefront_builtin_checkout_order_address_down',true)<>'on' THEN
    RAISE EXCEPTION 'STOREFRONT_BUILTIN_CHECKOUT_ORDER_ADDRESS_DOWN_GUARD_REQUIRED';
  END IF;
  IF completion_oid IS NULL THEN
    RAISE EXCEPTION 'STOREFRONT_BUILTIN_CHECKOUT_ORDER_ADDRESS_DOWN_FUNCTION_MISSING';
  END IF;
  ...
  IF pg_catalog.strpos(routine_definition,normalized_order_address)>0 THEN
    ...
    IF legacy_definition=routine_definition THEN
      RAISE EXCEPTION 'STOREFRONT_BUILTIN_CHECKOUT_ORDER_ADDRESS_DOWN_SOURCE_CHANGED';
    END IF;
