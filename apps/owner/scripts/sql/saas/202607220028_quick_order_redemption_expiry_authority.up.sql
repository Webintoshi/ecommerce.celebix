-- Phase 3B2 additive server-owned redemption-expiry authority repair.
BEGIN;
...
AS $function$
DECLARE current_link saas.quick_order_links%ROWTYPE; canonical_hostname text; resolved_store_id uuid;
  effective_expires_at timestamptz; projection jsonb;
BEGIN
  ...
  IF current_link.currency<>'TRY' OR current_link.status NOT IN ('active','opened')
     OR current_link.expires_at<=p_now THEN
    RETURN QUERY SELECT 'unavailable'::text,NULL::jsonb; RETURN;
  END IF;
  effective_expires_at:=LEAST(p_expires_at,current_link.expires_at);
  IF effective_expires_at<=p_now THEN RETURN QUERY SELECT 'unavailable'::text,NULL::jsonb; RETURN; END IF;
  BEGIN
    ...
    ) VALUES(p_redemption_id,current_link.store_id,current_link.id,p_redemption_digest,effective_expires_at,1,p_now,p_now);
  ...
  projection:=pg_catalog.jsonb_build_object(
    'canonicalHostname',canonical_hostname,'redemptionExpiresAt',saas.quick_links_json_timestamp(effective_expires_at),
