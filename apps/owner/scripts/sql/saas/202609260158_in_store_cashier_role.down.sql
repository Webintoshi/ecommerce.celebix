-- Rollback must not silently replace a cashier with a broader role.
BEGIN;
SET LOCAL ROLE celebix_saas_owner;
SET LOCAL lock_timeout='5s';

DO $cashier_rollback_preflight$
BEGIN
  IF EXISTS(SELECT 1 FROM saas.memberships WHERE role='cashier') THEN
    RAISE EXCEPTION 'in_store_cashier_memberships_exist';
  END IF;
END
$cashier_rollback_preflight$;
...
ALTER TABLE saas.memberships ADD CONSTRAINT memberships_role_check
  CHECK (role IN ('store_owner','admin','editor','analyst'));
...
DO $returning_staff_roles_rollback$
...
BEGIN
  FOREACH signature IN ARRAY ARRAY[
    ...
    IF definition IS NULL OR pg_catalog.strpos(definition,
      'membership.role IN (''store_owner'',''admin'',''editor'',''analyst'',''cashier'')')=0
    THEN RAISE EXCEPTION 'in_store_cashier_login_rollback_precondition_failed: %',signature; END IF;
  ...
  END LOOP;
END
$returning_staff_roles_rollback$;
COMMIT;
