-- V3 is the dynamic-price route. V2 remains executable by the old Storefront
-- only while dynamic activation is closed; private trace is never app-readable.
DO $assertions$
...
BEGIN
  ...
  FOREACH function_name IN ARRAY ARRAY[
    ...
    IF selected IS NULL OR NOT pg_catalog.has_function_privilege('celebix_saas_host_resolver',selected,'EXECUTE')
      OR pg_catalog.has_function_privilege('celebix_saas_app',selected,'EXECUTE')
      OR pg_catalog.has_function_privilege('public',selected,'EXECUTE')
    THEN RAISE EXCEPTION 'REFERENCE_CHECKOUT_V2_OVERLAP_ASSERTION_FAILED: %',function_name; END IF;
  END LOOP;
  IF EXISTS (SELECT 1 FROM saas.pricing_dynamic_activation WHERE enabled) THEN
    RAISE EXCEPTION 'REFERENCE_CHECKOUT_V2_GATE_ASSERTION_FAILED';
  END IF;
END $assertions$;
