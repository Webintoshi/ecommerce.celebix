...
function parseArgs(argv) {
  const parsed = {
    sourceProjectRef: process.env.OWNER_SUPABASE_PROJECT_REF || "",
  ...
  for (let index = 2; index < argv.length; index += 1) {
    ...
    if (arg === "--source-project-ref" && next) {
      parsed.sourceProjectRef = next;
      index += 1;
      continue;
    }
  ...
  if (!parsed.sourceProjectRef) {
    throw new Error("OWNER_SUPABASE_PROJECT_REF veya --source-project-ref gerekli.");
  }
...
function buildOwnerBootstrapQueries() {
  return [
    {
      name: "owner_schema.sql",
      sql: fs.readFileSync(path.join(repoRoot, "apps", "owner", "supabase", "schema.sql"), "utf8"),
    },
  ];
}

async function ensureTargetSchema(args) {
  const ownerProfilesExists = await targetPgMetaQuery(
    args.targetBaseUrl,
    args.targetRef,
    args.targetBasicUser,
    args.targetBasicPassword,
    "select to_regclass('public.owner_profiles') is not null as exists;",
  );

  if (ownerProfilesExists?.[0]?.exists) {
    return { applied: false };
  }

  const files = buildOwnerBootstrapQueries();
...
async function listSourcePublicTables(args) {
  const rows = await managedQuery(
    args.sourceAccessToken,
    args.sourceProjectRef,
    `
...
async function getSourceRowCount(args, schema, table) {
...
async function fetchSourceBatch(args, schema, table, orderByClause, limit, offset) {
  const query = `select * from ${quoteQualified(schema, table)}${orderByClause} limit ${limit} offset ${offset};`;
  return managedQuery(args.sourceAccessToken, args.sourceProjectRef, query, `dump_${schema}_${table}_${offset}`);
}
...
async function copyTable(args, targetTable) {
  const schema = targetTable.schema;
  const table = targetTable.name;
  const orderByClause = buildOrderBy(targetTable);
  const rowCount = await getSourceRowCount(args, schema, table);

  if (rowCount === 0) {
    return { schema, table, rowCount: 0, inserted: 0, batches: 0, skipped: false };
  }
  ...
  if (firstBatch.length > 0) {
    await targetPgMetaQuery(
      ...
      args.targetBasicUser,
      args.targetBasicPassword,
      buildInsertQuery(schema, table, insertableColumns, firstBatch),
    );
  ...
  for (let offset = firstBatch.length; offset < rowCount; offset += args.batchSize) {
    const batch = await fetchSourceBatch(args, schema, table, orderByClause, args.batchSize, offset);
    if (batch.length === 0) {
      continue;
    }
    ...
    await targetPgMetaQuery(
      ...
      args.targetBasicUser,
      args.targetBasicPassword,
      buildInsertQuery(schema, table, insertableColumns, batch),
    );
  ...
  return { schema, table, rowCount, inserted, batches, skipped: false };
}
...
function resolveReportPath(args) {
  ...
  return path.join(repoRoot, ".tmp", "selfhosted-migrations", `owner-${args.targetRef}-${timestamp}.json`);
}

async function main() {
  const args = parseArgs(process.argv);

  console.log(`[1/6] Source owner project: ${args.sourceProjectRef}`);
  ...
  if (!args.skipSchema) {
    console.log("[3/6] Applying owner schema bundle to target if needed...");
  ...
  const sourcePublicTables = await listSourcePublicTables(args);
  ...
  for (const table of uniqueTables) {
    const result = await copyTable(args, table);
    if (result.skipped) {
  ...
  if (failures.length > 0) {
    console.error("Owner migration completed with verification mismatches.");
