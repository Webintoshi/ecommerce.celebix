...
export function selfServeRegistrationJson(
...
export function selfServeRegistrationMessage(code: string): string {
...
function gateResponse(decision: Exclude<SelfServeRequestGateDecision, "allowed">): Response {
  ...
  return selfServeRegistrationJson({
    ...
    message: selfServeRegistrationMessage(mapped.code),
  }, mapped.status);
}
...
function contentType(request: Request): "application/json" | "application/x-www-form-urlencoded" {
  ...
  if (!mediaType || !CONTENT_TYPES.has(mediaType) || parts.length > 2 ||
      (parts[1] && parts[1] !== "charset=utf-8")) {
...
function parseStrictJsonObject(value: string): Record<string, string | boolean> {
  ...
  if (!closed || skipWhitespace(value, position) !== value.length) {
    throw new RegistrationRequestError("self_serve_request_malformed", 400);
  }
  return parsed;
}
...
export type SelfServeRegistrationRequestResult =
  | Readonly<{
      ok: true;
      runtime: PersistentSelfServeRuntime;
...
export async function processSelfServeRegistrationRequest(
  runtime: SelfServeRuntime,
  request: Request,
): Promise<SelfServeRegistrationRequestResult> {
  if (request.method !== "POST") {
    return Object.freeze({
      ok: false,
      response: selfServeRegistrationJson({
        code: "self_serve_register_read_disabled",
        message: selfServeRegistrationMessage("self_serve_register_read_disabled"),
      }, 405),
    });
  }
  if (!exactSameOrigin(request, runtime.registrationOrigin)) {
    return Object.freeze({
      ...
      response: selfServeRegistrationJson({
        ...
        message: selfServeRegistrationMessage("self_serve_origin_required"),
      }, 403),
    });
  }
  ...
  if (decision !== "allowed") {
    audit(runtime, {
      stage: "request_gate",
      outcome: "rejected",
      retryable: decision === "rate_limited" || decision === "unavailable",
      statusCategory: decision === "unavailable" ? "5xx" : "4xx",
    });
    return Object.freeze({ ok: false, response: gateResponse(decision) });
  }
  ...
  try {
    ...
    const parsed = registration(
      mediaType === "application/json" ? parseStrictJsonObject(raw) : parseStrictForm(raw),
    );
    return Object.freeze({ ok: true, runtime, registration: Object.freeze({ ...parsed }) });
  } catch (error) {
    ...
    return Object.freeze({
      ...
      response: selfServeRegistrationJson({
        ...
        state: "rejected",
        retryable: false,
        message: selfServeRegistrationMessage(controlled.code),
      }, controlled.status),
    });
  }
}
