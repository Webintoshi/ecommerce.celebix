...
import { projectOwnerInternalCallbackResponse } from "./internal-callback-response.ts";
...
async function rejected(body: Record<string, unknown>, status: number): Promise<void> {
  await assert.rejects(
    projectOwnerInternalCallbackResponse(upstream(body, status), 4_096),
    /internal_callback_response_invalid/,
  );
}

test("Owner projector accepts every reachable exact B1B2A response and re-emits canonical keys", async () => {
  ...
  for (const fixture of errorFixtures) {
    for (const status of fixture.statuses) {
      ...
      const response = await projectOwnerInternalCallbackResponse(upstream(expected, status), 4_096);
...
test("Owner projector binds every error to exact status and retryable semantics", async () => {
  for (const fixture of errorFixtures) {
...
test("Owner projector binds restartRegistration presence and value exactly", async () => {
...
test("Owner projector rejects wrong state/code pairs, unreachable replay output, unknown fields, and non-200 success", async () => {
  const pending = errorFixtures.find((entry) => entry.state === "in_progress")!;
...
test("Owner projector rejects the reconciliation status/retry regression", async () => {
  await rejected({
...
test("Owner projector rejects malformed JSON, malformed UTF-8, and oversized bodies while retaining bounded valid output", async () => {
  await assert.rejects(projectOwnerInternalCallbackResponse(new Response("{"), 4_096), /internal_callback_response_invalid/);
  await assert.rejects(
    projectOwnerInternalCallbackResponse(new Response(new Uint8Array([0xc3, 0x28])), 4_096),
    /internal_callback_response_invalid/,
  );
  await assert.rejects(
    projectOwnerInternalCallbackResponse(upstream(errorBody(errorFixtures[0]), 405), 8),
    /internal_callback_response_invalid/,
  );
  const response = await projectOwnerInternalCallbackResponse(upstream(successBody(), 200), 4_096);
  assert.deepEqual(await response.json(), successBody());
});

test("Owner projector strips Set-Cookie and Location and keeps audit-independent cache control", async () => {
  const response = await projectOwnerInternalCallbackResponse(
