...
export function TestimonialsSection({
  ...
  countLabel = "Yeni mağaza için placeholder değerlendirmeler",
}: {
  heading?: string;
  countLabel?: string;
}) {
          ...
          <div className="overflow-hidden">
            ...
            >
              {Array.from({ length: totalSlides }).map((_, slideIndex) => (
                <div key={slideIndex} className="grid w-full flex-shrink-0 grid-cols-1 gap-6 lg:grid-cols-2">
                    ...
                    <div key={review.id} className="flex overflow-hidden bg-white shadow-sm">
                      <div className="flex w-32 flex-shrink-0 items-center justify-center bg-neutral-100 sm:w-40 lg:w-48">
                        {review.image ? (
                          <div className="relative h-20 w-20 overflow-hidden rounded-full">
                            <Image src={review.image} alt={review.name} fill className="object-cover" sizes="80px" />
                          </div>
                        ) : (
                      ...
                      <div className="flex flex-1 flex-col justify-center p-4 sm:p-5">
                        ...
                        <div className="mb-3 flex items-center gap-2">
                          <span className="text-sm font-semibold uppercase text-neutral-900">{review.name}</span>
