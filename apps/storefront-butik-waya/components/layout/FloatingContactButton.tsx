    ...
    () =>
      settings.channels
        .map((channel) => ({
          ...
          displayLabel: getChannelLabel(channel.type, channel.label),
        }))
        .filter(
          (channel) =>
      ...
      >
        ...
        >
          ...
          <button
            ...
            className={`relative inline-flex ${BUTTON_HEIGHT_CLASS} ${BUTTON_WIDTH_CLASS} items-center rounded-full border border-[rgba(255,255,255,0.16)] bg-[linear-gradient(180deg,rgba(29,22,22,0.98),rgba(20,15,15,0.96))] px-2.5 text-[14px] font-medium text-white shadow-[0_20px_40px_-24px_rgba(63,30,50,0.48)] backdrop-blur-xl transition-all duration-300 hover:border-white/24 hover:bg-[linear-gradient(180deg,rgba(34,26,26,0.98),rgba(22,16,16,0.96))]`}
          ...
          >
            <span
              className={`absolute left-2 inline-flex h-8 w-8 items-center justify-center rounded-full border border-[rgba(122,92,103,0.10)] bg-[linear-gradient(180deg,#fffaf8,#f2e0db)] text-[#6c5550] shadow-[0_10px_24px_-14px_rgba(88,48,70,0.32)] transition-transform duration-300 ${
                isOpen ? "translate-x-[78px] sm:translate-x-[100px]" : "translate-x-0"
              }`}
            >
              {isOpen ? (
            ...
            <span
              className={`absolute text-[14px] font-medium tracking-[0.01em] transition-all duration-300 ${
                isOpen ? "left-4 text-white/72" : "left-[48px] sm:left-[52px] text-white/94"
              }`}
            >
              {TOGGLE_LABEL}
            </span>
...
function InstagramBrandIcon({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className={className}>
      <defs>
        <linearGradient
          id="butik-waya-instagram-gradient"
          x1="0%"
          y1="100%"
          x2="100%"
          y2="0%"
      ...
      <path
        fill="url(#butik-waya-instagram-gradient)"
