...
const STORAGE_KEY = `${STORE_RUNTIME.slug}_seo_rules`;
...
export function generateMetaTags(product: Product): { title: string; description: string } {
    // Generate Title
    // Pattern: {Name} - {Category/Tags} | Brand
    // e.g., "Sekersiz Fistik Ezmesi - Dogal & Vegan | Celebix E-ticaret"
    ...
    title += ` | ${STORE_RUNTIME.defaultProductBrand}`;
    ...
    description += ` ${STORE_RUNTIME.defaultProductBrand} ile hemen siparis verin.`;
