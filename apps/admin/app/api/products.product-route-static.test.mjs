...
test("product create retries can strip storefront badge columns for older light postgres stores", () => {
  ...
  assert.match(
    routeSource,
    /stripUnsupportedTableColumn\([\s\S]+productInsertPayload[\s\S]+OPTIONAL_PRODUCT_COLUMNS/,
    "insert retry older staging DB columns olmadan tekrar denenmeli",
  );
  assert.match(
    routeSource,
    /stripUnsupportedTableColumn\([\s\S]+productUpdatePayload[\s\S]+OPTIONAL_PRODUCT_COLUMNS/,
    "update retry older staging DB columns olmadan tekrar denenmeli",
  );
});
