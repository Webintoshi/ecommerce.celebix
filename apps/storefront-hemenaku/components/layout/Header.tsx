  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "border-b border-[#D7DEE8] bg-white/95 shadow-sm backdrop-blur-sm"
    ...
    >
        ...
        <div className="flex h-16 items-center justify-between lg:h-20">
          ...
          <Link href={buildPath(ROUTES.home)} className="flex-shrink-0" aria-label={logoAlt}>
            {logoSrc ? (
              ...
              <span className="text-lg font-semibold tracking-tight text-[#0B1220] lg:text-xl">
                {logoAlt}
              </span>
            )}
          </Link>
            ...
            {hasHeaderCategories ? headerCategories.map((category) => {
              ...
              if (category.children.length === 0) {
                return (
                  <Link
                    ...
                    className="store-nav-text group relative text-[0.92rem] text-[#334155] transition-all duration-300 hover:text-[#0B1220] after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-[#22C55E] after:transition-all after:duration-300 after:content-[''] group-hover:after:w-full"
                  >
              ...
              return (
                <div key={category.id} className="group relative">
                  <Link
                    href={buildPath(ROUTES.category(category.slug))}
                    className="store-nav-text relative inline-flex items-center gap-1 text-[0.92rem] text-[#334155] transition-all duration-300 hover:text-[#0B1220] after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-[#22C55E] after:transition-all after:duration-300 after:content-[''] group-hover:after:w-full"
                  >
                    {localizedCategoryName}
                  ...
                  <div className="pointer-events-none absolute left-1/2 top-full z-30 w-72 -translate-x-1/2 pt-4 opacity-0 transition-all duration-200 group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100">
                    <div className="rounded-lg border border-[#D7DEE8] bg-white/95 p-4 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <ShoppingBag className="h-5 w-5 text-[#334155]" />
              {cartItemCount > 0 ? (
                <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-[#FACC15] text-[10px] font-semibold text-[#0B1220]">
                  {cartItemCount}
                </span>
              ) : null}
            </button>
      ...
      {isMenuOpen ? (
        <div className="border-t border-[#D7DEE8] bg-white lg:hidden">
          <nav className="container-premium space-y-4 py-4">
            {hasHeaderCategories ? headerCategories.map((category) => (
              <div key={category.id} className="space-y-2">
                <Link
                  href={buildPath(ROUTES.category(category.slug))}
                  className="store-nav-text block text-[#334155] transition-all duration-300 hover:pl-2 hover:text-[#0B1220]"
                  ...
                  <div className="space-y-2 border-l border-neutral-200 pl-4">
                    {category.children.map((subcategory) => (
                      <Link
                        key={subcategory.id}
                        href={buildPath(ROUTES.category(subcategory.slug))}
                        className="store-nav-text block text-sm text-[#526176] transition-all duration-300 hover:pl-2 hover:text-[#0B1220]"
                        onClick={() => setIsMenuOpen(false)}
                      >
                        {subcategory.name}
                      </Link>
            ...
            <div className="grid grid-cols-2 gap-3 border-t border-[#D7DEE8] pt-4">
              <Link
                href={buildPath(user ? "/hesap" : ROUTES.login)}
                className="inline-flex items-center justify-center gap-2 rounded-lg border border-[#D7DEE8] px-4 py-3 text-sm font-semibold text-[#0B1220]"
                onClick={() => setIsMenuOpen(false)}
              >
              ...
              <button
                type="button"
                className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#0F172A] px-4 py-3 text-sm font-semibold text-white"
                onClick={() => {
                  setIsMenuOpen(false);
                  setIsCartOpen(true);
                }}
