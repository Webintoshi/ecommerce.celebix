...
function BlogHeroImage({ post }: { post: BlogPost }) {
  ...
  return (
    <div className="flex aspect-[1.7/1] items-center justify-center rounded-[30px] border border-black/5 bg-[radial-gradient(circle_at_top,_rgba(138,104,71,0.24),_transparent_48%),linear-gradient(135deg,#111827_0%,#164E49_40%,#0F766E_100%)] text-[5rem] text-white">
    ...
    <div className="min-h-screen bg-[#F6F1EB] text-[#1A120D]">
      <div className="border-b border-black/5 bg-white/70 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-4 lg:px-8">
          <Link href="/blog" className="inline-flex items-center gap-2 text-sm font-medium text-[#526B66] transition-colors hover:text-[#111827]">
            <ArrowLeft className="h-4 w-4" />
            Blog'a don
          </Link>
          <Link href="/urunler" className="text-sm font-medium text-[#0F766E] transition-colors hover:text-[#111827]">
            Urunleri kesfet
          </Link>
        </div>
      </div>
      ...
      <article className="mx-auto max-w-6xl px-6 py-12 lg:px-8 lg:py-16">
        <div className="mx-auto max-w-4xl">
          <div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.32em] text-[#0F766E]">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white px-4 py-2 text-[11px] font-semibold transition-colors hover:border-[#111827] hover:text-[#111827]"
            >
          ...
          <h1 className="mt-6 font-serif text-5xl leading-[0.94] text-[#111827] sm:text-6xl">
            {post.title}
          </h1>
        ...
        <div className="mx-auto mt-12 grid max-w-6xl gap-12 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div className="min-w-0">
            <div
              className="prose prose-lg max-w-none prose-headings:font-serif  prose-headings:text-[#111827] prose-p:text-[#526B66] prose-p:leading-8 prose-a:text-[#0F766E] prose-a:no-underline hover:prose-a:text-[#111827] prose-strong:text-[#111827] prose-li:text-[#526B66] prose-blockquote:border-l-[#0F766E] prose-blockquote:text-[#526B66]"
          ...
          <aside className="space-y-5">
            <div className="rounded-[28px] border border-black/5 bg-white p-6 shadow-[0_24px_70px_-58px_rgba(24,17,11,0.35)]">
              <p className="text-xs font-semibold uppercase tracking-[0.32em] text-[#0F766E]">Editor Notu</p>
              <p className="mt-4 font-serif text-3xl leading-tight text-[#111827]">{profile.name}</p>
            ...
            <div className="rounded-[28px] border border-black/5 bg-white p-6 shadow-[0_24px_70px_-58px_rgba(24,17,11,0.35)]">
              <p className="text-xs font-semibold uppercase tracking-[0.32em] text-[#0F766E]">Sonraki adim</p>
              ...
              <div className="mt-6 flex flex-col gap-3">
                <Link href="/urunler" className="rounded-full bg-[#111827] px-4 py-3 text-center text-sm font-semibold text-white transition-colors hover:bg-[#2A1B13]">
                  Urunleri incele
                </Link>
                <Link href="/blog" className="rounded-full border border-black/10 bg-white px-4 py-3 text-center text-sm font-semibold text-[#111827] transition-colors hover:border-[#111827]">
                  Diger yazilari gor
                </Link>
              </div>
            </div>
        ...
        {relatedPosts.length > 0 ? (
          <section className="mx-auto mt-20 max-w-6xl">
            <div className="mb-8">
              <p className="text-xs font-semibold uppercase tracking-[0.32em] text-[#0F766E]">Ilgili yazilar</p>
              <h2 className="mt-3 font-serif text-4xl text-[#111827]">Okumaya devam et</h2>
            </div>
              ...
              {relatedPosts.map((relatedPost) => (
                <article key={relatedPost.id} className="group overflow-hidden rounded-[28px] border border-black/5 bg-white shadow-[0_24px_70px_-56px_rgba(24,17,11,0.28)]">
                  <Link href={`/blog/${relatedPost.slug}`}>
                    <div className="relative aspect-[1.18/0.82] overflow-hidden">
                      {resolveStorefrontAssetUrl(relatedPost.coverImage) ? (
                        ...
                        <div className="flex h-full items-center justify-center bg-[linear-gradient(135deg,#111827_0%,#164E49_45%,#0F766E_100%)] text-4xl text-white">
                  ...
                  <div className="p-6">
                    ...
                    <Link href={`/blog/${relatedPost.slug}`}>
                      <h3 className="mt-3 font-serif text-2xl leading-tight text-[#111827]">{relatedPost.title}</h3>
                    </Link>
