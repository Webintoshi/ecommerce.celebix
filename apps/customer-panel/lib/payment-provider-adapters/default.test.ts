...
test("panel activation mode promotes only the exact PayTR test source authority", () => {
  ...
  assert.ok(PAYTR_APPROVED_EXECUTION_AUTHORITIES.live, "PayTR live execution authority must be generated for storefront use");
  ...
  assert.notEqual(PAYTR_APPROVED_EXECUTION_AUTHORITIES.test.evidenceDigest, PAYTR_APPROVED_EXECUTION_AUTHORITIES.live.evidenceDigest);
  const registry = createDefaultCustomerPanelPaymentProviderRegistry(
    hosted,
    undefined,
    "approved_test_sandbox",
