...
test("abandoned-cart preflight requires only session and abandoned-cart authority", async () => {
  ...
  try {
    const { initializeApprovedStagingServerAbandonedCartRuntime } = await import(`./postgres-runtime.ts?preflight=${Date.now()}`);
    await assert.rejects(
      initializeApprovedStagingServerAbandonedCartRuntime({ database: { name: "celebix_saas_staging_auth" } } as CustomerPanelStagingAuthConfig),
