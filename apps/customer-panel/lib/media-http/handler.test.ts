...
function fixture(options: Readonly<{ failUnpublish?: boolean; archiveError?: Error; recoveryStatus?: "active" | "archived" }> = {}) {
  const calls = { put: 0, attach: 0, list: 0, archive: 0, publish: 0, unpublish: 0, delete: 0, archiveProof: 0 };
...
test("archive succeeds only after the exact R2 object is deleted and durable deletion proof is recorded", async () => {
  ...
  assert.deepEqual(selected.calls, { put: 0, attach: 0, list: 1, archive: 1, publish: 0, unpublish: 1, delete: 1, archiveProof: 1 });
});

test("archive fails closed before PostgreSQL mutation when R2 cannot be unpublished", async () => {
  const selected = fixture({ failUnpublish: true });
  const body = JSON.stringify({ expectedVersion: 1 });
  const request = new Request(`http://customer-panel:3400/api/catalog/products/${PRODUCT}/media/${MEDIA}/archive`, { method: "POST", headers: { origin: "https://panel.saas-staging.celebix.site", cookie: `__Host-celebix_panel=${CREDENTIAL}`, "idempotency-key": OPERATION, "content-type": "application/json", "content-length": String(Buffer.byteLength(body)) }, body });
  const response = await selected.handlers.archive(request, PRODUCT, MEDIA);
  assert.equal(response.status, 503);
  assert.deepEqual(selected.calls, { put: 0, attach: 0, list: 1, archive: 0, publish: 0, unpublish: 1, delete: 0, archiveProof: 0 });
});

test("stale archive version republishes only after a read-only active-row proof", async () => {
  const selected = fixture({ archiveError: new ProductMediaRepositoryError("version_conflict"), recoveryStatus: "active" });
  const body = JSON.stringify({ expectedVersion: 1 });
  const request = new Request(`http://customer-panel:3400/api/catalog/products/${PRODUCT}/media/${MEDIA}/archive`, { method: "POST", headers: { origin: "https://panel.saas-staging.celebix.site", cookie: `__Host-celebix_panel=${CREDENTIAL}`, "idempotency-key": OPERATION, "content-type": "application/json", "content-length": String(Buffer.byteLength(body)) }, body });
  const response = await selected.handlers.archive(request, PRODUCT, MEDIA);
  assert.equal(response.status, 409);
  assert.deepEqual(selected.calls, { put: 0, attach: 0, list: 2, archive: 1, publish: 1, unpublish: 1, delete: 0, archiveProof: 0 });
});

test("unknown archive commit uses one read-only archived-row recovery and continues deletion", async () => {
  const selected = fixture({ archiveError: new ProductMediaRepositoryError("unavailable"), recoveryStatus: "archived" });
  const body = JSON.stringify({ expectedVersion: 1 });
  const request = new Request(`http://customer-panel:3400/api/catalog/products/${PRODUCT}/media/${MEDIA}/archive`, { method: "POST", headers: { origin: "https://panel.saas-staging.celebix.site", cookie: `__Host-celebix_panel=${CREDENTIAL}`, "idempotency-key": OPERATION, "content-type": "application/json", "content-length": String(Buffer.byteLength(body)) }, body });
  const response = await selected.handlers.archive(request, PRODUCT, MEDIA);
  assert.equal(response.status, 200);
  assert.deepEqual(selected.calls, { put: 0, attach: 0, list: 2, archive: 1, publish: 0, unpublish: 1, delete: 1, archiveProof: 1 });
});
