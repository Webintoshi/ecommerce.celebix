...
function fixture(options: Readonly<{ failUnpublish?: boolean; reserveError?: Error; finalizeError?: Error; recoveryStatus?: "pending" | "archived"; role?: "store_owner" | "admin" | "editor" | "analyst" }> = {}) {
...
test("analyst can read product media but cannot mutate it before repository or storage access", async () => {
  const selected = fixture({ role: "analyst" });
  const list = await selected.handlers.list(new Request(`http://customer-panel:3400/api/catalog/products/${PRODUCT}/media`, { headers: { cookie: `__Host-celebix_panel=${CREDENTIAL}` } }), PRODUCT);
  assert.equal(list.status, 200);
  const upload = await selected.handlers.upload(uploadRequest(), PRODUCT);
  assert.equal(upload.status, 403);
  assert.equal(selected.calls.list, 1);
  assert.equal(selected.calls.attach, 0);
  assert.equal(selected.calls.put, 0);
});
test("media upload accepts tenant admin Origin through the internal reverse-proxy Host", async () => {
  const selected = fixture();
  const response = await selected.handlers.upload(uploadRequest(TENANT_ADMIN_ORIGIN, { host: "customer-panel:3400", forwarded: "host=attacker.example;proto=https", "x-forwarded-host": "attacker.example", "x-forwarded-proto": "https" }), PRODUCT);
  assert.equal(response.status, 201);
  assert.deepEqual(selected.calls, { put: 0, attach: 1, list: 1, reserveArchive: 0, finalizeArchive: 0, recoverArchive: 0, publish: 1, unpublish: 0, delete: 0, archiveProof: 0 });
});
...
test("archive unpublishes the exact R2 object but retains it for restore", async () => {
  const selected = fixture();
  ...
  assert.deepEqual(selected.calls, { put: 0, attach: 0, list: 1, reserveArchive: 1, finalizeArchive: 1, recoverArchive: 0, publish: 0, unpublish: 1, delete: 0, archiveProof: 0 });
});
...
test("unknown archive finalization uses one read-only durable recovery and retains the object", async () => {
  const selected = fixture({ finalizeError: new ProductMediaRepositoryError("unavailable"), recoveryStatus: "archived" });
  const body = JSON.stringify({ expectedVersion: 1 });
  const request = new Request(`http://customer-panel:3400/api/catalog/products/${PRODUCT}/media/${MEDIA}/archive`, { method: "POST", headers: { origin: "https://panel.saas-staging.celebix.site", cookie: `__Host-celebix_panel=${CREDENTIAL}`, "idempotency-key": OPERATION, "content-type": "application/json", "content-length": String(Buffer.byteLength(body)) }, body });
  const response = await selected.handlers.archive(request, PRODUCT, MEDIA);
  assert.equal(response.status, 200);
  assert.deepEqual(selected.calls, { put: 0, attach: 0, list: 1, reserveArchive: 1, finalizeArchive: 1, recoverArchive: 1, publish: 0, unpublish: 1, delete: 0, archiveProof: 0 });
});
