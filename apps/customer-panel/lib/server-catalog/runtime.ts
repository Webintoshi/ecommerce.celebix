...
function facade(repository: CatalogRepository): CatalogRepository {
  ...
  const projected: CatalogRepository = {
    ...
    getProductDeletionImpact: (input) => repository.getProductDeletionImpact(input),
    deleteProduct: (input) => repository.deleteProduct(input),
    ...
    createVariantBatch: (input) => repository.createVariantBatch(input),
