...
const PATH = new RegExp(`^(?:/api/catalog/onboarding/(?:options|products)|/api/catalog/products/${UUID}/(?:merchandising|publish-after-media))$`);
...
export function createCatalogOnboardingRequestAuthorityValidator(options: Readonly<{ panelOrigin: string }>) {
  ...
  return Object.freeze({
    validate(request: unknown, expected: CatalogOnboardingRequestExpectation): CatalogOnboardingRequestDecision {
      ...
      if (exact.method !== "GET" && request.headers.get("origin") !== origin) return "origin_denied";
      let url: URL;
