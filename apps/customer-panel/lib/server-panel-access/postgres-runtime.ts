  ...
  try {
    const result = await client.query(`SELECT
      ...
      to_regclass('saas.record_deletion_operations') IS NOT NULL
        AND to_regprocedure('saas.order_deletion_impact(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid)') IS NOT NULL
        AND has_function_privilege('celebix_saas_app','saas.order_deletion_impact(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid)','EXECUTE')
        AND to_regprocedure('saas.delete_order(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,text,uuid,bigint,text)') IS NOT NULL
        AND has_function_privilege('celebix_saas_app','saas.delete_order(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,text,uuid,bigint,text)','EXECUTE')
        AND to_regprocedure('saas.delete_order_recover(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,text)') IS NOT NULL
        AND has_function_privilege('celebix_saas_app','saas.delete_order_recover(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,text)','EXECUTE') AS order_deletion_repository,
      ...
      to_regprocedure('saas.catalog_list_products_v2(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,text,integer,timestamp with time zone,uuid)') IS NOT NULL
        ...
        AND to_regprocedure('saas.catalog_list_products_v5(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,text,text,text,uuid,uuid,uuid,text,integer,timestamp with time zone,text,uuid)') IS NOT NULL
        AND has_function_privilege('celebix_saas_app','saas.catalog_list_products_v5(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,text,text,text,uuid,uuid,uuid,text,integer,timestamp with time zone,text,uuid)','EXECUTE')
        AND to_regprocedure('saas.catalog_get_dashboard_summary_v2(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone)') IS NOT NULL
        AND has_function_privilege('celebix_saas_app','saas.catalog_get_dashboard_summary_v2(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone)','EXECUTE') AS catalog_list_projection,
      ...
      to_regprocedure('saas.abandoned_carts_summary(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone)') IS NOT NULL
        ...
        AND has_function_privilege('celebix_saas_app','saas.commerce_cart_recovery_attempt_record(uuid,uuid,uuid,uuid,text,bigint,timestamptz,uuid,uuid,text,text)','EXECUTE')
      ...
      ,to_regclass('saas.barcode_label_templates') IS NOT NULL
        ...
        AND to_regprocedure('saas.barcode_label_reserve_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid)') IS NOT NULL
        AND to_regprocedure('saas.barcode_label_generate_ean13_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,jsonb)') IS NOT NULL
        AND to_regprocedure('saas.barcode_label_reserve_ean13_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid)') IS NOT NULL
        ...
        AND has_function_privilege('celebix_saas_app','saas.barcode_label_reserve_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid)','EXECUTE')
        AND has_function_privilege('celebix_saas_app','saas.barcode_label_generate_ean13_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,jsonb)','EXECUTE')
        AND has_function_privilege('celebix_saas_app','saas.barcode_label_reserve_ean13_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid)','EXECUTE')
    ...
    if (
      ...
      row.order_deletion_repository !== true ||
