  ...
  try {
    const result = await client.query(`SELECT
      ...
      ,to_regclass('saas.barcode_label_templates') IS NOT NULL
        ...
        AND to_regprocedure('saas.barcode_label_generate_numeric_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,jsonb)') IS NOT NULL
        AND to_regprocedure('saas.barcode_label_reserve_numeric_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid)') IS NOT NULL
        ...
        AND has_function_privilege('celebix_saas_app','saas.barcode_label_generate_numeric_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,jsonb)','EXECUTE')
        AND has_function_privilege('celebix_saas_app','saas.barcode_label_reserve_numeric_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid)','EXECUTE')
