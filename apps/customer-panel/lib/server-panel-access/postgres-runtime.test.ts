    ...
    async connect() {
      ...
      return {
        async query(sql: string) {
          ...
          const productListV5 = "saas.catalog_list_products_v5(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,text,text,text,uuid,uuid,uuid,text,integer,timestamp with time zone,text,uuid)";
          assert.match(sql, /to_regprocedure\('saas[.]catalog_list_products_v5\(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,text,text,text,uuid,uuid,uuid,text,integer,timestamp with time zone,text,uuid\)'\) IS NOT NULL/);
          assertAppRoleFunctionPrivilege(sql, productListV5);
          ...
          for (const signature of [
            ...
            "saas.barcode_label_generate_ean13_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,jsonb)",
            "saas.barcode_label_reserve_ean13_internal(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid)",
