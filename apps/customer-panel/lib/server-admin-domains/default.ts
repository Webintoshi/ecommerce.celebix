...
import { createCloudflareCustomHostnameProvider, createAdminDomainService } from "@celebix/saas-domain-core";
import { PostgresAdminDomainLifecycleRepository } from "@celebix/saas-data";
import pg from "pg";
...
import {
  registerServerAdminDomainService,
  resolveServerAdminDomainRuntime,
  type ServerAdminDomainRuntime,
} from "./runtime.ts";
...
let initialization: Promise<ServerAdminDomainRuntime | null> | undefined;
...
function required(name: string, maximum: number): string {
  ...
  if (typeof value !== "string" || value.length < 1 || value.length > maximum || value !== value.trim() || /[\u0000-\u001f\u007f]/u.test(value)) {
    throw new Error("server_admin_domain_config_invalid");
  }
  return value;
}

function exactHostname(value: string): string {
  if (value !== value.toLowerCase() || value.length > 253 || !HOSTNAME.test(value)) throw new Error("server_admin_domain_config_invalid");
  return value;
}

async function initialize(): Promise<ServerAdminDomainRuntime | null> {
  ...
  const pool = new Pool({
    ...
    application_name: `celebix-panel-admin-domains-${auth.activationId}`,
  });
  ...
  try {
    const result = await pool.query(`SELECT
      ...
      to_regprocedure('saas.merchant_admin_domain_list(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone)') IS NOT NULL
        AND to_regprocedure('saas.merchant_admin_domain_prepare_create(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,text,uuid,text,text,text)') IS NOT NULL
        ...
        AND to_regprocedure('saas.merchant_admin_domain_request_recheck(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,bigint)') IS NOT NULL
        AND to_regprocedure('saas.merchant_admin_domain_make_primary(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,bigint)') IS NOT NULL
        AND to_regprocedure('saas.merchant_admin_domain_disable(uuid,uuid,uuid,uuid,text,bigint,timestamp with time zone,uuid,bigint)') IS NOT NULL AS domain_lifecycle
    ...
    if (result.rowCount !== 1 || !row || Math.floor(Number(row.version_num) / 10_000) !== 16
      ...
      throw new Error("server_admin_domain_database_preflight_failed");
    }
    const repository = new PostgresAdminDomainLifecycleRepository({ pool, role: "celebix_saas_app", timeouts: TIMEOUTS });
    const provider = createCloudflareCustomHostnameProvider({ zoneId, apiToken, apiBaseUrl, minimumTlsVersion: "1.2", timeoutMs: 5_000 });
    registerServerAdminDomainService(access, createAdminDomainService({
      repository,
      provider,
      hostnamePolicy: Object.freeze({ cnameTarget, reservedSuffixes }),
      generateId: randomUUID,
    }));
    return resolveServerAdminDomainRuntime(access);
  } catch (error) {
...
export async function resolveDefaultServerAdminDomainRuntime(): Promise<ServerAdminDomainRuntime | null> {
