...
test("resource writes preserve imported brands with more than one hundred linked products", async () => {
  const calls: unknown[] = [];
  const productIds = Array.from({ length: 101 }, (_, index) => `72000000-0000-4000-8000-${String(index + 1).padStart(12, "0")}`);
  const h = handlers(repository({ async saveResource(input) { calls.push(input); return { id: RESOURCE, version: 2, status: "active", updatedAt: NOW.toISOString(), replayed: false }; } }));
  const response = await h.saveResource(request("/api/catalog/admin/resources/brand", "POST", { resourceId: RESOURCE, expectedVersion: 1, name: "Güzide Kuyumcu", slug: "guzide-kuyumcu", config: {}, productIds }), "brand");
  assert.equal(response.status, 200);
  assert.equal((calls[0] as { productIds: readonly string[] }).productIds.length, 101);
});
...
test("tenant admin catalog mutations survive internal reverse-proxy Host and stay bound to the authenticated store", async () => {
  const calls: unknown[] = [];
  const h = handlers(repository({ async saveResource(input) { calls.push(input); return { id: RESOURCE, version: 1, status: "active", updatedAt: NOW.toISOString(), replayed: false }; } }));
  const value = { name: "Bileklikler", slug: "bileklikler", config: {}, productIds: [PRODUCT] };
  ...
  assert.equal((await h.saveResource(request("/api/catalog/admin/resources/collection", "POST", value, TENANT_ADMIN_ORIGIN, forwarded), "collection")).status, 200);
  assert.equal((await h.saveResource(request("/api/catalog/admin/resources/collection", "POST", value, OTHER_TENANT_ADMIN_ORIGIN, { ...forwarded, "x-forwarded-host": "store.admin.saas-staging.celebix.site" }), "collection")).status, 403);
