...
test("abandoned-cart list and detail pages stay behind durable server access", async () => {
  for (const path of [
    "../app/orders/abandoned-carts/page.tsx",
    "../app/orders/abandoned-carts/[cartId]/page.tsx",
  ]) {
...
test("console exposes truthful loaded empty error detail and durable mutations", async () => {
  const source = await readFile(
    new URL("../components/orders/AbandonedCartConsole.tsx", import.meta.url),
    "utf8",
  );
  for (const evidence of [
    "Terk Edilen Sepetler",
    "Henüz terk edilmiş sepet yok",
    "Sepetler yüklenemedi",
    "Sepet ayrıntısı",
    "Arşivle",
    "abandonedCartApi.archive",
    "firstProductName",
    "customerId",
    "/customers/",
  ])
  ...
  assert.doesNotMatch(
    source,
    /Kurtarıldı olarak işaretle|abandonedCartApi[.]markRecovered/,
  );
  assert.doesNotMatch(
    source,
    /fake|mock|Supabase|\/api\/admin|storeId|tenantId/i,
  );
});
...
test("responsive console keeps Hemenaku geometry, 48px controls, and reduced motion", async () => {
  const css = await readFile(
    new URL(
      "../components/orders/abandoned-cart-console.module.css",
      import.meta.url,
    ),
    "utf8",
  );
