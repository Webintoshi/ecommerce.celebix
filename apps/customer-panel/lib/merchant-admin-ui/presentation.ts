...
export interface MerchantModuleDefinition {
  readonly cardinality: "collection" | "singleton";
...
function definition(
  value: Omit<MerchantModuleDefinition, "cardinality" | "execution" | "fields"> &
    Readonly<{
      cardinality?: MerchantModuleDefinition["cardinality"];
  ...
  return Object.freeze({
    ...value,
    cardinality: value.cardinality ?? "collection",
...
export const MERCHANT_MODULE_DEFINITIONS = Object.freeze([
  ...
  definition({ kind: "general_setting", cardinality: "singleton", family: "settings", route: "/settings/general", title: "Genel Ayarlar", singular: "genel ayar profili", description: "Mağaza adı, destek adresi, saat dilimi ve isteğe bağlı SKU başlangıcını yönetin.", fields: [field("storeDisplayName", "Mağaza adı", "text", undefined, undefined, undefined, undefined, true), field("supportEmail", "Destek e-postası", "email", undefined, undefined, undefined, undefined, true), field("timezone", "Saat dilimi", "text", "Europe/Istanbul", undefined, undefined, undefined, true), field("skuPrefix", "SKU başlangıcı", "text", "Örn. RSA")] }),
  ...
  definition({ kind: "administrator_invite", family: "settings", route: "/settings/administrators", title: "Yöneticiler", singular: "yönetici daveti", description: "Mağaza ekip davetlerini rol bazında yönetin.", notice: "Bu ekran parola, tek kullanımlık kod veya gizli anahtar saklamaz; yalnız süreli yönetici daveti ve güvenli rol seçimi kaydedilir.", fields: [field("email", "Yönetici e-postası", "email", undefined, undefined, undefined, undefined, true), field("role", "Yetki rolü", "enum", undefined, ["admin", "editor", "analyst"], { admin: "Yönetici", editor: "Editör", analyst: "Analist" }, undefined, true), field("expiresAt", "Davet bitiş zamanı", "datetime", undefined, undefined, undefined, undefined, true)] }),
  definition({ kind: "notification_setting", cardinality: "singleton", family: "settings", route: "/settings/notifications", title: "Bildirimler", singular: "bildirim ayarı", description: "Sipariş bildirimlerini yönetin.", fields: [field("orderNotificationsEnabled", "Yeni sipariş e-postası", "boolean"), field("notificationEmail", "Bildirim adresi", "email"), field("senderLabel", "Gönderici adı"), field("replyToEmail", "Yanıt adresi", "email")] }),
  definition({ kind: "theme_setting", family: "settings", route: "/settings/theme", title: "Tema", singular: "tema profili", description: "Starter vitrinin renk, başlık ve ürün yerleşimini sınırlı seçeneklerle yönetin.", fields: [field("colorScheme", "Renk düzeni", "enum", undefined, ["neutral", "warm", "dark", "ocean"], { neutral: "Nötr", warm: "Sıcak", dark: "Koyu", ocean: "Okyanus" }), field("headingStyle", "Başlık stili", "enum", undefined, ["serif", "sans"], { serif: "Serif", sans: "Sans serif" }), field("productCardStyle", "Ürün kartı", "enum", undefined, ["editorial", "compact"], { editorial: "Editoryal", compact: "Kompakt" }), field("productImageRatio", "Görsel oranı", "enum", undefined, ["portrait", "square"], { portrait: "Dikey", square: "Kare" }), field("homeProductLimit", "Ana sayfa ürün sayısı", "number", undefined, ["4", "8", "12"]), field("showBrandStory", "Marka hikâyesi", "boolean")] }),
  definition({ kind: "hero_banner", family: "settings", route: "/settings/hero-banner", title: "Hero Banner", singular: "hero banner", description: "Vitrin kahraman alanını kalıcı olarak yönetin.", fields: [field("headline", "Başlık"), field("body", "Açıklama", "textarea"), field("assetId", "Mağaza görseli kimliği"), field("destination", "Hedef"), field("enabled", "Etkin", "boolean")] }),
  ...
  definition({ kind: "social_preview", family: "seo", route: "/seo/social-preview", title: "Sosyal Önizleme", singular: "sosyal önizleme", description: "Paylaşım başlığı, açıklaması ve mağaza görselini yönetin.", fields: [field("title", "Başlık"), field("description", "Açıklama", "textarea"), field("assetId", "Mağaza görseli kimliği")] }),
  ...
  definition({ kind: "ai_setting", cardinality: "singleton", family: "settings", route: "/settings/artificial-intelligence", title: "Yapay Zeka", singular: "yapay zeka tercihi", description: "Yapay zeka önerileri için güvenli dil, ton ve özellik tercihlerini yönetin.", notice: "Harici sağlayıcı etkinleştirilmeden öneri üretimi kullanılamaz.", fields: [field("tone", "Ton"), field("locale", "Dil", "text", "tr-TR"), field("enabledFeatures", "Etkin öneriler", "enum-list", undefined, ["description_suggestions", "seo_suggestions", "campaign_drafts"], undefined, 3)] }),
