...
export function createMerchantAdminApi(fetcher: Fetch = fetch, uuid: () => string = crypto.randomUUID.bind(crypto)) {
  ...
  function post(path: string, value: unknown, operationId?: string) { return request(path, { method: "POST", headers: { "content-type": "application/json", "idempotency-key": operationId ? opaqueId(operationId) : uuid() }, body: JSON.stringify(value) }); }
