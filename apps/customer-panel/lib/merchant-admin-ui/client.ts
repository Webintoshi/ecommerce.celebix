...
const CODES = ["invalid_input", "unauthenticated", "membership_denied", "store_inactive", "feature_not_enabled", "record_not_found", "profile_not_found", "provider_capability_mismatch", "provider_disabled", "invalid_transition", "version_conflict", "operation_mismatch", "operation_not_found", "durable_authority_invalid", "unavailable"] as const;
type Code = (typeof CODES)[number];
const MESSAGES: Record<Code, string> = { invalid_input: "Gönderilen bilgiler geçersiz.", unauthenticated: "Oturumunuz sona erdi.", membership_denied: "Bu işlem için yetkiniz yok.", store_inactive: "Mağaza işlemlere kapalı.", feature_not_enabled: "Bu özellik planınızda etkin değil.", record_not_found: "Kayıt bulunamadı.", profile_not_found:"Sağlayıcı bağlantısı bulunamadı.",provider_capability_mismatch:"Sağlayıcı bu işlem türünü desteklemiyor.",provider_disabled:"Sağlayıcı bağlantısı etkin değil.", invalid_transition: "Bu işlem artık uygulanamaz.", version_conflict: "Kayıt sizden önce güncellendi.", operation_mismatch: "İşlem güvenle tekrar edilemedi.", operation_not_found: "İşlem kanıtı bulunamadı.", durable_authority_invalid: "Yetki yeniden doğrulanamadı.", unavailable: "Bu bölüm şu anda kullanılamıyor." };
...
export function createMerchantAdminApi(fetcher: Fetch = fetch, uuid: () => string = crypto.randomUUID.bind(crypto)) {
  ...
  function post(path: string, value: unknown, operationId?: string) { return request(path, { method: "POST", headers: { "content-type": "application/json", "idempotency-key": operationId === undefined ? uuid() : opaqueId(operationId) }, body: JSON.stringify(value) }); }
  ...
  return Object.freeze({
    ...
    async save(recordKind: MerchantAdminRecordKind, value: Readonly<{ recordId?: string; expectedVersion?: number; name: string; config: Readonly<Record<string, MerchantAdminJson>>; status: "draft" | "active" }>, operationId?: string) { return parseMerchantAdminMutationResult(await post(`/api/merchant-admin/records/${kind(recordKind)}`, value, operationId)); },
    ...
    async queueProviderJob(recordKind: MerchantAdminProviderRecordKind, jobId: string, expectedJobVersion: number, profileId: string, expectedProfileVersion: number) { return parseMerchantAdminProviderJobMutationResult(await post(`/api/merchant-admin/provider-jobs/${providerKind(recordKind)}/${opaqueId(jobId)}/queue`, { expectedJobVersion, profileId: opaqueId(profileId), expectedProfileVersion })); },
