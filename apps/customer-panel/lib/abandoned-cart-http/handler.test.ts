...
const OTHER_TENANT_ADMIN_ORIGIN =
  "https://other-merchant.admin.saas-staging.celebix.site";
...
function cart(status: "abandoned" | "recovered" = "abandoned") {
  return {
    ...
    totalCents: 10000,
    itemCount: 1,
    firstProductName: "Ürün",
    checkoutStartedAt: NOW.toISOString(),
    lastActivityAt: NOW.toISOString(),
    ...(status === "abandoned"
      ? { abandonedAt: NOW.toISOString() }
      : { abandonedAt: NOW.toISOString(), recoveredAt: NOW.toISOString() }),
    version: 3,
...
function repository(
  overrides: Partial<AbandonedCartRepository> = {},
  ...
  return {
    ...
    list: reject,
    get: reject,
    issueRecoveryLink: reject,
    recordRecoveryAttempt: reject,
    markRecovered: reject,
    archive: reject,
    ...overrides,
  } as AbandonedCartRepository;
}

function runtime(
  carts: AbandonedCartRepository,
  accessKind: "authenticated" | "unauthenticated" = "authenticated",
  role: "store_owner" | "analyst" = "store_owner",
): ServerAbandonedCartRuntime {
  return {
...
function request(
  ...
  if (options.cookie !== null)
    headers.set(
      "cookie",
      options.cookie ?? `__Host-celebix_panel=${CREDENTIAL}`,
    );
  if (method !== "GET") {
  ...
  return new Request(`http://internal:3400${path}`, {
    ...
    body:
      method === "GET"
        ? undefined
        : JSON.stringify(options.body ?? { expectedVersion: 3 }),
  });
}
...
function handlers(
  ...
  return createAbandonedCartHttpHandlers({
    async resolveRuntime() {
      return runtime(carts, accessKind, role);
    },
    now: () => new Date(NOW),
    requestId: () => REQUEST_ID,
  });
...
test("authenticated summary, list and detail receive only server TenantContext", async () => {
  ...
  assert.equal(
    (await api.list(request(`${BASE}?status=abandoned&sort=newest`))).status,
    200,
  );
...
test("list always masks customer identity while analyst detail also masks contact PII", async () => {
  ...
  const ownerListPayload = (await ownerList.json()) as {
    items: Record<string, unknown>[];
  };
  assert.deepEqual(ownerListPayload.items[0]?.customerName, "Maskeli müşteri");
  assert.equal(
    JSON.stringify(ownerListPayload).includes("ada@example.test"),
    false,
  );
  assert.equal(
    JSON.stringify(ownerListPayload).includes("+905551112233"),
    false,
  );
  assert.equal(
    JSON.stringify(ownerListPayload).includes("eeeeeeee-eeee"),
    false,
  );
  ...
  const analystDetail = await handlers(carts, "authenticated", "analyst").get(
    request(`${BASE}/${ID}`),
    ID,
  );
  const analystPayload = (await analystDetail.json()) as Record<
    string,
    unknown
  >;
  assert.equal(analystPayload.customerName, "Maskeli müşteri");
  assert.equal(
    JSON.stringify(analystPayload).includes("ada@example.test"),
    false,
  );
...
test("recovered and archive mutations require exact Origin and idempotency authority", async () => {
  ...
  assert.equal(
    (
      await api.markRecovered(
        request(`${BASE}/${ID}/recovered`, { method: "POST" }),
        ID,
      )
    ).status,
    200,
  ...
  assert.equal(
    (
      await api.archive(
        request(`${BASE}/${ID}/archive`, { method: "POST", origin: null }),
        ID,
      )
    ).status,
    403,
...
test("store owner can issue an opaque reopenable recovery URL while analyst and body-bearing requests fail closed", async () => {
  ...
  const linkRequest = new Request(
    `http://internal:3400${BASE}/${ID}/recovery-link`,
    {
      method: "POST",
      headers: { origin: ORIGIN, cookie: `__Host-celebix_panel=${CREDENTIAL}` },
    },
  );
  ...
  assert.match(
    payload.url,
    /^https:\/\/shop[.]example[.]test\/cart\/recover#token=[A-Za-z0-9_-]{43}$/u,
  );
  const recovery = new URL(payload.url);
  assert.equal(recovery.search, "");
  assert.equal(JSON.stringify(calls).includes(recovery.hash.slice(7)), false);
  const analyst = handlers(
    repository({
  ...
  assert.equal(
    (
      await api.issueRecoveryLink(
        request(`${BASE}/${ID}/recovery-link`, { method: "POST" }),
        ID,
      )
    ).status,
    403,
...
test("manual contacted and note records are idempotent, bounded, and owner-only", async () => {
  ...
  assert.equal(
    (
      await api.recordRecoveryAttempt(
        request(`${BASE}/${ID}/mark-contacted`, { method: "POST", body: {} }),
        ID,
        "contacted",
      )
    ).status,
  ...
  assert.equal(
    (
      await api.recordRecoveryAttempt(
        request(`${BASE}/${ID}/note`, {
          method: "POST",
          body: { note: "Müşteri mağazayı aradı." },
        }),
        ID,
        "note",
      )
  ...
  assert.equal(
    (calls[1] as Record<string, unknown>).note,
    "Müşteri mağazayı aradı.",
  );
  const analyst = handlers(
    repository({
  ...
  assert.equal(
    (
      await analyst.recordRecoveryAttempt(
        request(`${BASE}/${ID}/mark-contacted`, { method: "POST", body: {} }),
        ID,
        "contacted",
      )
  ...
  assert.equal(
    (
      await api.recordRecoveryAttempt(
        request(`${BASE}/${ID}/note`, { method: "POST", body: { note: " " } }),
        ID,
        "note",
      )
    ).status,
    ...
    (
      await api.archive(
        request(`${BASE}/${ID}/archive`, {
          ...
          headers: {
            ...forwarded,
            "x-forwarded-host": "merchant.admin.saas-staging.celebix.site",
          },
        }),
        ID,
      )
...
test("session, path, private authority, repository errors and disabled runtime fail closed", async () => {
  assert.equal(
    (
      await handlers(repository(), "unauthenticated").getSummary(
  ...
  assert.equal(
    (await handlers(repository()).get(request(`${BASE}/bad`), "bad")).status,
    400,
  );
  assert.equal(
    (
      await handlers(repository()).getSummary(
        request(`${BASE}/summary`, { headers: { "x-store-id": ID } }),
      )
    ).status,
    400,
  );
  assert.equal(
    (
      await handlers(
        repository({
          async getSummary() {
            throw new AbandonedCartRepositoryError("cart_not_found");
          },
        }),
      ).getSummary(request(`${BASE}/summary`))
    ).status,
    404,
  );
  const disabled = createAbandonedCartHttpHandlers({
    async resolveRuntime() {
      return null;
    },
    now: () => NOW,
  ...
  assert.equal(
    (await disabled.getSummary(request(`${BASE}/summary`))).status,
    503,
  );
});
