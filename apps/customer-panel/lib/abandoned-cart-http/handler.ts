...
import { isMerchantActionAllowed, parseAbandonedCartDetail, parseAbandonedCartListItem, parseAbandonedCartMutationResult, parseAbandonedCartSummary, type AbandonedCartDetail, type AbandonedCartListItem, type TenantContext } from "@celebix/saas-contracts";
...
async function readRecoveryAttemptInput(request: Request, kind: "contacted" | "note"): Promise<Readonly<{ operationId: string; note?: string }> | null> {
  try {
    const operationId = request.headers.get("idempotency-key");
    if (!operationId || !REQUEST_ID.test(operationId) || request.headers.get("content-type")?.split(";", 1)[0]?.trim().toLowerCase() !== "application/json") return null;
    const declared = request.headers.get("content-length"); if (declared !== null && (!/^\d{1,4}$/.test(declared) || Number(declared) > 4096)) return null;
    const raw = await request.text(); if (Buffer.byteLength(raw, "utf8") > 4096) return null;
    const value: unknown = JSON.parse(raw); if (typeof value !== "object" || value === null || Array.isArray(value)) return null;
    const record = value as Record<string, unknown>;
    if (kind === "contacted") return Object.keys(record).length === 0 ? Object.freeze({ operationId }) : null;
    if (Object.keys(record).join(",") !== "note" || typeof record.note !== "string" || record.note !== record.note.trim()
      || record.note.length < 1 || record.note.length > 1000 || /[\u0000-\u001f\u007f]/.test(record.note)) return null;
    return Object.freeze({ operationId, note: record.note });
  } catch { return null; }
}
...
function maskCartIdentity<T extends AbandonedCartListItem | AbandonedCartDetail>(cart: T): T {
  const { customerId: _customerId, customerName, customerEmail, customerPhone, ...safe } = cart;
  return Object.freeze({
    ...safe,
    ...(customerName !== undefined || customerEmail !== undefined || customerPhone !== undefined ? { customerName: "Maskeli müşteri" } : {}),
  }) as T;
}
...
export function createAbandonedCartHttpHandlers(dependencies: Dependencies) {
  ...
  return Object.freeze({
    ...
    async list(request: Request) { const authorized = await authorize(dependencies, request, { method: "GET", pathname: BASE, query: "allowed" }); if (isResponse(authorized)) return authorized; const input = readAbandonedCartListInput(request); if (input.kind !== "valid") return error("invalid_input", 400); return execute(() => authorized.runtime.abandonedCarts.list({ tenantContext: authorized.tenantContext, now: authorized.now, ...input.value }), (value) => { if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError(); const record = value as Record<string, unknown>; if (!Array.isArray(record.items) || (Object.keys(record).sort().join(",") !== "items" && Object.keys(record).sort().join(",") !== "items,nextCursor") || (record.nextCursor !== undefined && (typeof record.nextCursor !== "string" || !/^[A-Za-z0-9_-]{1,1024}$/.test(record.nextCursor)))) throw new TypeError(); return Object.freeze({ items: Object.freeze(record.items.map(parseAbandonedCartListItem).map(maskCartIdentity)), ...(record.nextCursor === undefined ? {} : { nextCursor: record.nextCursor }) }); }); },
    async get(request: Request, rawId: unknown) { const cartId = readAbandonedCartPathId(rawId); if (cartId === null) return error("invalid_input", 400); const authorized = await authorize(dependencies, request, { method: "GET", pathname: `${BASE}/${cartId}`, query: "forbidden" }); if (isResponse(authorized)) return authorized; const mayManage = isMerchantActionAllowed(authorized.tenantContext.membership.role, "carts.manage"); return execute(() => authorized.runtime.abandonedCarts.get({ tenantContext: authorized.tenantContext, now: authorized.now, cartId }), (value) => { const parsed = parseAbandonedCartDetail(value); return mayManage ? parsed : maskCartIdentity(parsed); }); },
    async issueRecoveryLink(request: Request, rawId: unknown) {
      const cartId = readAbandonedCartPathId(rawId); if (cartId === null) return error("invalid_input", 400);
      const authorized = await authorize(dependencies, request, { method: "POST", pathname: `${BASE}/${cartId}/recovery-link`, query: "forbidden" }); if (isResponse(authorized)) return authorized;
      if (!isMerchantActionAllowed(authorized.tenantContext.membership.role, "carts.manage") || request.body !== null || request.headers.has("content-type")) return error("membership_denied", 403);
      const rawToken = randomBytes(32).toString("base64url");
      const tokenDigest = createHash("sha256").update(rawToken, "utf8").digest("hex");
      return execute(() => authorized.runtime.abandonedCarts.issueRecoveryLink({ tenantContext: authorized.tenantContext, now: authorized.now, cartId, tokenId: randomUUID(), tokenDigest, keyVersion: 1 }), (value) => {
        if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError();
        const row = value as Record<string, unknown>;
        if (Object.keys(row).sort().join(",") !== "cartId,expiresAt,hostname" || row.cartId !== cartId || typeof row.hostname !== "string" || typeof row.expiresAt !== "string") throw new TypeError();
        return Object.freeze({ url: `https://${row.hostname}/api/cart/recover?token=${rawToken}`, expiresAt: row.expiresAt });
      });
    },
    async recordRecoveryAttempt(request: Request, rawId: unknown, kind: "contacted" | "note") {
      const cartId = readAbandonedCartPathId(rawId); if (cartId === null) return error("invalid_input", 400);
      const leaf = kind === "contacted" ? "mark-contacted" : "note";
      const authorized = await authorize(dependencies, request, { method: "POST", pathname: `${BASE}/${cartId}/${leaf}`, query: "forbidden" }); if (isResponse(authorized)) return authorized;
      if (!isMerchantActionAllowed(authorized.tenantContext.membership.role, "carts.manage")) return error("membership_denied", 403);
      const input = await readRecoveryAttemptInput(request, kind); if (input === null) return error("invalid_input", 400);
      return execute(() => authorized.runtime.abandonedCarts.recordRecoveryAttempt({ tenantContext: authorized.tenantContext, now: authorized.now,
        cartId, operationId: input.operationId, kind, ...(input.note === undefined ? {} : { note: input.note }) }), (value) => {
        if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError();
        const result = value as Record<string, unknown>;
        if (Object.keys(result).sort().join(",") !== "cartId,kind,recordedAt,replayed" || result.cartId !== cartId || result.kind !== kind
          || typeof result.recordedAt !== "string" || typeof result.replayed !== "boolean") throw new TypeError();
        return Object.freeze({ cartId, kind, recordedAt: result.recordedAt, replayed: result.replayed });
      });
    },
