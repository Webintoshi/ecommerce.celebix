...
function validateSafeBody(value: unknown, status: number): Record<string, unknown> {
...
export async function projectSafeCallbackResponse(response: Response, maximumBytes: number): Promise<Response> {
  ...
  const body = validateSafeBody(parsed, response.status);
