...
function handlers(merchantAdmin:MerchantAdminRepository){return createMerchantAdminHttpHandlers({async resolveRuntime(){return{merchantAdmin,access:{readiness:{mode:"approved_staging"},panelOrigin:ORIGIN,async resolveCredential(){return{kind:"authenticated",session:{},tenantContext:tenant()}},async rotateCredential(){return{kind:"unavailable"}},async revokeCredential(){return{kind:"unavailable"}}}}as never},now:()=>new Date(NOW),requestId:()=>REQ})}
...
test("lists prepares and cancels provider work as preparation only",async()=>{
 ...
 const h=handlers(repo({
  async listProviderJobs(input){calls.push(input);return[{id:JOB,recordId:RECORD,recordKind:"marketplace_connection",action:"synchronization",status:"awaiting_provider_activation",version:1,requestedAt:NOW.toISOString(),updatedAt:NOW.toISOString()}]},
  async prepareProviderJob(input){calls.push(input);return{id:JOB,recordId:RECORD,recordKind:"marketplace_connection",action:"synchronization",status:"awaiting_provider_activation",version:1,updatedAt:NOW.toISOString(),replayed:false}},
  async cancelProviderJob(input){calls.push(input);return{id:JOB,recordId:RECORD,recordKind:"marketplace_connection",action:"synchronization",status:"cancelled",version:2,updatedAt:NOW.toISOString(),replayed:false}},
 }));
