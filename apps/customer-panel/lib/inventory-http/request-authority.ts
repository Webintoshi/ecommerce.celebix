...
export type InventoryRouteDecision =
  ...
  | Readonly<{ kind: "method_not_allowed"; allow: "GET" | "POST" }>;
...
function route(pathname: string, method: string): InventoryRouteDecision {
  ...
  if (pathname === "/api/inventory/locations") {
    allowed = "GET"; selected = { kind: "locations", method: allowed, pathname };
  } else if (pathname === "/api/inventory/balances") {
    allowed = "GET"; selected = { kind: "balances", method: allowed, pathname };
  } else if (pathname === "/api/inventory/purchase-orders") {
    allowed = method === "POST" ? "POST" : "GET";
    selected = allowed === "POST"
      ? { kind: "purchase_save", method: allowed, pathname }
      : { kind: "purchase_list", method: allowed, pathname };
  } else if (pathname === "/api/inventory/counts") {
    allowed = method === "POST" ? "POST" : "GET";
    selected = allowed === "POST"
      ? { kind: "count_save", method: allowed, pathname }
      : { kind: "count_list", method: allowed, pathname };
  } else if (pathname === "/api/inventory/transfers") {
    allowed = method === "POST" ? "POST" : "GET";
    selected = allowed === "POST"
      ? { kind: "transfer_save", method: allowed, pathname }
      : { kind: "transfer_list", method: allowed, pathname };
  } else {
  ...
  return method === allowed
    ...
    : Object.freeze({ kind: "method_not_allowed", allow: allowed });
}
