...
async function initialize(): Promise<ServerStorefrontDesignRuntime | null> {
  ...
  try {
    ...
    return createServerStorefrontDesignRuntime({ access, repository: new PostgresStorefrontDesignRepository({ pool, role: "celebix_saas_app", timeouts: TIMEOUTS, audit: () => undefined }), storage: createR2ProductMediaStorage(media) });
