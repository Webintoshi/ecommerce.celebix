...
async function initialize(): Promise<ServerStorefrontDesignRuntime | null> {
  ...
  try {
    ...
    try {
      const previewResult = await pool.query(`SELECT pg_has_role(current_user,'celebix_saas_host_resolver','MEMBER') AS host_resolver_member,to_regprocedure('saas.resolve_public_storefront(text,timestamptz)') IS NOT NULL AND to_regprocedure('saas.public_list_products(uuid,text,timestamptz,integer)') IS NOT NULL AND to_regprocedure('saas.public_list_products_by_category(uuid,text,timestamptz,text,integer)') IS NOT NULL AND to_regprocedure('saas.public_starter_product_detail(uuid,text,timestamptz,text)') IS NOT NULL AND to_regprocedure('saas.storefront_asset_list(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamptz,text,boolean)') IS NOT NULL AS preview_ready`);
      ...
      if (previewResult.rowCount === 1 && previewRow?.host_resolver_member === true && previewRow.preview_ready === true) {
        ...
        registerDefaultServerStorefrontDesignPreviewRuntime(createServerStorefrontDesignPreviewRuntime({ access, design: repository, loader: createServerStorefrontDesignPreviewLoader({ publicStorefront, assets }) }));
      }
