...
const CATALOG_PATH = new RegExp(
  `^(?:/api/catalog/(?:summary|variant-choices)|/api/catalog/products(?:/v2(?:/${UUID})?|/bulk|/${UUID}(?:/(?:archive|restore|remove|removal-eligibility|deletion-impact|delete)|/variants(?:/batch|/${UUID}(?:/archive)?)?)?)?)$`,
);
