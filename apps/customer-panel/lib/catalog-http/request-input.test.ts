...
test("batch variant input accepts bounded distinct combinations and rejects duplicates", async () => {
  const black = { ...CREATE.initialVariant, sku: "BLACK-M", attributes: { renk: "Siyah", beden: "M" } };
...
test("list query accepts bounded canonical options and rejects duplicate unknown or malformed values", () => {
  ...
  for (const [query, expected] of [
    ...
    [
      `?limit=50&q=${encodeURIComponent("  Son SKU  ")}&status=active&stock=in-stock&category=${PRODUCT_ID}&brand=${OPERATION_ID}&collection=${PRODUCT_ID}&sort=title-asc`,
      { kind: "valid", value: { pageSize: 50, search: "Son SKU", status: "active", stock: "in-stock", categoryId: PRODUCT_ID, brandId: OPERATION_ID, collectionId: PRODUCT_ID, sort: "title-asc" } },
    ],
  ...
  for (const query of [
    "?limit=0", "?limit=10", "?limit=40", "?limit=101", "?limit=01", "?limit=1&limit=2", "?status=deleted",
