...
async function initialize(): Promise<ServerMediaRuntime | null> {
  ...
  try {
    const result = await pool.query(`SELECT current_setting('server_version_num')::integer AS version_num,current_database() AS database_name,role.rolsuper AS is_superuser,pg_has_role(current_user,'celebix_saas_app','MEMBER') AS app_member,to_regclass('saas.product_media') IS NOT NULL AND to_regclass('saas.store_domains') IS NOT NULL AND to_regprocedure('saas.media_attach_product(uuid,uuid,uuid,uuid,text,bigint,bigint,timestamp with time zone,uuid,text,uuid,uuid,uuid,text,text,text,text,integer,integer,bigint)') IS NOT NULL AS migration_020 FROM pg_roles AS role WHERE role.rolname=current_user`);
    const row = result.rows[0]; if (result.rowCount !== 1 || !row || Math.floor(Number(row.version_num) / 10_000) !== 16 || row.database_name !== auth.database.name || row.is_superuser !== false || row.app_member !== true || row.migration_020 !== true) throw new Error("server_media_database_preflight_failed");
    return createServerMediaRuntime({ access, media: new PostgresProductMediaRepository({ pool, role: "celebix_saas_app", timeouts: TIMEOUTS, audit: () => undefined }), storage: createR2ProductMediaStorage(mediaConfig) });
