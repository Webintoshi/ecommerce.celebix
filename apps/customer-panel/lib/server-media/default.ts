...
async function initialize(): Promise<ServerMediaRuntime | null> {
  ...
  try {
    ...
    return createServerMediaRuntime({ access, media: new PostgresProductMediaRepository({ pool, role: "celebix_saas_app", mediaOrigin: mediaConfig.publicOrigin, timeouts: TIMEOUTS, audit: () => undefined }), storage: createR2ProductMediaStorage(mediaConfig) });
