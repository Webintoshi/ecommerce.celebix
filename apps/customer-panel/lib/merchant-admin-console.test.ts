...
const routes=Object.freeze([
 ["app/discounts/lucky-wheel/page.tsx","lucky_wheel"],
 ...
 ["app/settings/general/page.tsx","general_setting"],["app/settings/language/page.tsx","language_setting"],["app/settings/administrators/page.tsx","administrator_invite"],
...
test("dedicated promotions pages use one server-owned context and never mount the generic discount console",async()=>{for(const path of["app/discounts/page.tsx","app/discounts/new/page.tsx","app/discounts/[promotionId]/page.tsx","app/discounts/[promotionId]/edit/page.tsx"]){const value=await source(path);assert.match(value,/requirePromotionPageContext/);assert.match(value,/PromotionStudio/);assert.doesNotMatch(value,/MerchantModuleConsole|MerchantRecordEditor|x-store-id|x-tenant-id|localStorage|sessionStorage/)}});
test("shipping settings uses its dedicated server-authorized connection workspace",async()=>{const value=await source("app/settings/shipping/page.tsx");assert.match(value,/requireServerPanelAccess/);assert.match(value,/ShippingSettingsConsole/);assert.match(value,/shipping[.]manage/);assert.doesNotMatch(value,/MerchantModuleConsole|shipping_setting/)});
test("legacy theme route is server-authorized and redirects to the unified design workspace",async()=>{const value=await source("app/settings/theme/page.tsx");assert.match(value,/requireServerPanelAccess/);assert.match(value,/redirect\("\/settings\/design[?]section=theme"\)/);assert.doesNotMatch(value,/StarterThemeComposer|MerchantModuleConsole|isMerchantActionAllowed/)});
...
test("administrator module renders the real role guide instead of hiding permission boundaries",async()=>{const value=await source("components/merchant-admin/MerchantModuleConsole.tsx");assert.match(value,/getAdministratorRoleDefinitions/);assert.match(value,/Yönetici rolleri ve görev sınırları/);assert.match(value,/role\.highlights/);assert.match(value,/role\.limits/);assert.match(value,/data-administrator-role-guide/);assert.match(value,/Mağaza sahibi/);assert.doesNotMatch(value,/store_owner.*allowedValues|allowedValues.*store_owner/)});
...
test("datetime-local, bounded list, and finite enum-list parsing reject unknown or duplicate values",async()=>{
  ...
  assert.equal(JSON.parse(payload!).parsed.email,"manager@example.test");
  ...
  assert.equal(rejection,"invalid_required_field");
  assert.equal(JSON.stringify(result.stdout.trim().split("\n").slice(2)),JSON.stringify(["invalid_string_list_12","invalid_string_list_24","invalid_enum_list","invalid_enum_list","invalid_list_definition","invalid_list_definition","invalid_enum_list"]));
});
