...
export function createInStoreSalesUiClient(options:Readonly<{fetch?:Fetch;randomUUID?:()=>string}>={}) {
  ...
  return Object.freeze({
    ...
    async searchProducts(input:InStoreSearchInput,signal?:AbortSignal){
      ...
      if(selected.barcode!==undefined)query.set("barcode",plainText(selected.barcode,200));else query.set("query",plainText(selected.query,200));
    ...
    listStaff(){return read("/staff",value=>{const list=object(value,["staff"]).staff;if(!Array.isArray(list)||list.length>500)invalid();return Object.freeze(list.map(parseInStoreStaffGrant));});},
