...
function InventoryCountDetail(props: Readonly<{ initial?: InventoryCount; resourceId?: string; create?: boolean; canRead: boolean; canManage: boolean }>) {
  ...
  return <>{!props.create ? <InventoryCountPresentation state={state} canManage={props.canManage} onStart={() => { void lifecycle.current?.getCurrent()?.start(); }} onCommit={() => { void lifecycle.current?.getCurrent()?.commit(); }} onCancel={() => { void lifecycle.current?.getCurrent()?.cancel(); }} /> : null}
    {(props.create || item?.status === "draft" || item?.status === "counting") ? <InventoryOperationForm mode="count" record={item} canManage={props.canManage} phase={state.phase} pending={state.pending} locked={state.locked} message={state.message} onSave={(value) => { void lifecycle.current?.getCurrent()?.save(value as Parameters<ReturnType<typeof createInventoryCountConsoleController>["save"]>[0]); }} /> : null}
  </>;
}
...
export function InventoryCountConsole(props: Readonly<{ mode?: "list" | "new" | "detail"; initial?: InventoryCount; initialItems?: readonly InventoryCount[]; resourceId?: string; canRead?: boolean; canManage: boolean }>) {
  ...
  return <PanelPageShell><PanelPageHeader title={title} description="Sayım farklarını kalıcı konum kayıtları üzerinden izleyin." actions={mode === "list" && props.canManage ? <span className={styles.pageAction}><PanelActionButton primary href="/products/inventory-counts/new">Yeni stok sayımı</PanelActionButton></span> : undefined} /><h1 className={styles.srOnly}>{title}</h1>{detail ? <InventoryCountDetail initial={props.initial} resourceId={props.resourceId} create={mode === "new"} canRead={canRead} canManage={props.canManage} /> : <InventoryCountListPresentation state={list.phase} items={list.items} error={list.error} canManage={props.canManage} onRetry={list.retry} />}</PanelPageShell>;
}
