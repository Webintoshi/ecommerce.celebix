...
function InventoryCountDetail(props: Readonly<{ initial?: InventoryCount; resourceId?: string; canRead: boolean; canManage: boolean }>) {
  ...
  return <InventoryCountPresentation state={state} canManage={props.canManage} onStart={() => { void lifecycle.current?.getCurrent()?.start(); }} onCommit={() => { void lifecycle.current?.getCurrent()?.commit(); }} onCancel={() => { void lifecycle.current?.getCurrent()?.cancel(); }} />;
}
...
export function InventoryCountConsole(props: Readonly<{ initial?: InventoryCount; initialItems?: readonly InventoryCount[]; resourceId?: string; canRead?: boolean; canManage: boolean }>) {
  const canRead = props.canRead ?? true, load = useCallback((signal?: AbortSignal) => inventoryApi.listCounts(signal), []), detail = Boolean(props.initial || props.resourceId), list = useInventoryCollection({ enabled: !detail, canRead, initial: props.initialItems, load });
