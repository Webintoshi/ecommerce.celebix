...
function InventoryTransferDetail(props: Readonly<{ initial?: InventoryTransfer; resourceId?: string; canRead: boolean; canManage: boolean }>) {
  ...
  return <InventoryTransferPresentation state={state} canManage={props.canManage} onDispatch={() => { void lifecycle.current?.getCurrent()?.dispatch(); }} onReceive={() => { void lifecycle.current?.getCurrent()?.receive(); }} onCancel={() => { void lifecycle.current?.getCurrent()?.cancel(); }} />;
}
