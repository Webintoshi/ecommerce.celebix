...
import { createInventoryTransferConsoleController, type InventoryConsoleSnapshot } from "@/lib/inventory-ui/console-controller";
...
export function InventoryTransferPresentation(props: Readonly<{ state: InventoryConsoleSnapshot<InventoryTransfer>; canManage: boolean; onDispatch: () => void; onReceive: () => void; onCancel: () => void }>) {
  ...
  return <><div className={styles.detailSummary}><div><span>Transfer</span><strong>{number(item.id)}</strong></div><div><span>Kaynak</span><code>{item.sourceLocationId}</code></div><div><span>Hedef</span><code>{item.destinationLocationId}</code></div><div><span>Sürüm</span><strong>Sürüm {item.version}</strong></div><div><span>Durum</span><PanelStatusBadge tone={tone(item.status)}>{LABELS[item.status]}</PanelStatusBadge></div></div>{props.state.message ? <p className={props.state.phase === "conflict" ? styles.conflict : props.state.phase === "error" ? styles.errorNotice : styles.notice} role={props.state.phase === "conflict" || props.state.phase === "error" ? "alert" : "status"}>{props.state.message}</p> : null}<div className={styles.lineTable}><table aria-label="Transfer kalemleri"><thead><tr><th>Kalem</th><th>Varyant</th><th>Miktar</th></tr></thead><tbody>{item.lines.map((line) => <tr key={line.id}><td><code>{line.id}</code></td><td><code>{line.variantId}</code></td><td>{line.quantity}</td></tr>)}</tbody></table></div>{props.canManage ? <div className={styles.actions}>{item.status === "draft" ? <button className={styles.primary} type="button" disabled={props.state.pending} onClick={props.onDispatch}>Sevk et</button> : null}{item.status === "in_transit" ? <button className={styles.primary} type="button" disabled={props.state.pending} onClick={props.onReceive}>Teslim al</button> : null}{["draft", "in_transit"].includes(item.status) ? <button type="button" disabled={props.state.pending} onClick={props.onCancel}>İptal et</button> : null}</div> : null}</>;
}

function InventoryTransferDetail(props: Readonly<{ initial?: InventoryTransfer; resourceId?: string; canRead: boolean; canManage: boolean }>) {
  const controller = useRef<ReturnType<typeof createInventoryTransferConsoleController> | null>(null);
  const [state, setState] = useState<InventoryConsoleSnapshot<InventoryTransfer>>({ phase: props.canRead ? (props.initial ? "loaded" : "loading") : "denied", ...(props.initial ? { record: props.initial } : {}), pending: false, message: "" });
  if (!controller.current) controller.current = createInventoryTransferConsoleController({ initial: props.initial, resourceId: props.resourceId, canRead: props.canRead, canManage: props.canManage, api: inventoryApi, onChange: setState });
  useEffect(() => { const selected = controller.current!; void selected.load(); return () => selected.dispose(); }, []);
  return <InventoryTransferPresentation state={state} canManage={props.canManage} onDispatch={() => { void controller.current?.dispatch(); }} onReceive={() => { void controller.current?.receive(); }} onCancel={() => { void controller.current?.cancel(); }} />;
}
...
export function InventoryTransferConsole(props: Readonly<{ initial?: InventoryTransfer; initialItems?: readonly InventoryTransfer[]; resourceId?: string; canRead?: boolean; canManage: boolean }>) {
  const canRead = props.canRead ?? true, load = useCallback((signal?: AbortSignal) => inventoryApi.listTransfers(signal), []), list = useInventoryCollection({ canRead, initial: props.initialItems, load }), detail = Boolean(props.initial || props.resourceId);
