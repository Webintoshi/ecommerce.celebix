...
export function PurchasingConsole(props: Readonly<{ mode?: "list" | "new" | "detail"; initial?: PurchaseOrder; initialItems?: readonly PurchaseOrder[]; resourceId?: string; canRead?: boolean; canManage: boolean }>) {
  ...
  return <PanelPageShell><PanelPageHeader title={title} description="Sipariş ve teslim hareketlerini kalıcı envanter kayıtlarıyla yönetin." actions={mode === "list" && props.canManage ? <span className={styles.pageAction}><PanelActionButton primary href="/products/purchasing/new">Yeni satın alma siparişi</PanelActionButton></span> : undefined} /><h1 className={styles.srOnly}>{title}</h1>{detail ? <PurchasingDetail initial={props.initial} resourceId={props.resourceId} create={mode === "new"} canRead={canRead} canManage={props.canManage} /> : <PurchasingListPresentation state={list.phase} items={list.items} error={list.error} canManage={props.canManage} onRetry={list.retry} />}</PanelPageShell>;
}
