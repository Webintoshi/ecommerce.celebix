...
function initialLines(mode: Mode, record?: RecordValue): readonly DraftLine[] {
  ...
  if (isCount(record)) return Object.freeze(record.lines.map((line) => Object.freeze({
    lineId: line.id, variantId: line.variantId, quantity: String(line.countedQuantity ?? ""), unitCostCents: "0",
  })));
    ...
    <form onSubmit={submit} noValidate>
      <fieldset disabled={disabled || empty}>
        ...
        <div className={styles.operationFields}>
          ...
          {props.mode !== "transfer" ? <label><span>Etkin konum</span><select value={locationId} required disabled={fixedCountAuthority} onChange={(event) => setLocationId(event.target.value)}><option value="">Konum seçin</option>{choices.choices.locations.map((choice) => <option key={choice.locationId} value={choice.locationId}>{choice.name}{choice.isDefault ? " — Varsayılan" : ""} — {choice.locationId}</option>)}</select></label> : <>
      ...
      <fieldset disabled={disabled || empty}>
        ...
        <div className={styles.desktopFormTable}><table><thead><tr><th>Varyant</th><th>{props.mode === "purchase" ? "Sipariş miktarı" : props.mode === "count" ? "Sayılan miktar" : "Transfer miktarı"}</th>{props.mode === "purchase" ? <th>Birim maliyet (kuruş)</th> : null}<th>Kalem kimliği</th><th /></tr></thead><tbody>{lines.map((line, index) => <tr key={`${line.lineId || "new"}-${index}`}><td><select aria-label={`${index + 1}. kalem varyantı`} value={line.variantId} disabled={fixedCountAuthority} onChange={(event) => updateLine(index, { variantId: event.target.value })}><option value="">Varyant seçin</option>{choices.choices.variants.map((choice) => <option key={choice.variantId} value={choice.variantId}>{variantLabel(choice)}</option>)}</select></td><td><input aria-label={`${index + 1}. kalem miktarı`} inputMode="numeric" value={line.quantity} onChange={(event) => updateLine(index, { quantity: event.target.value })} /></td>{props.mode === "purchase" ? <td><input aria-label={`${index + 1}. kalem birim maliyeti`} inputMode="numeric" value={line.unitCostCents} onChange={(event) => updateLine(index, { unitCostCents: event.target.value })} /></td> : null}<td><code>{line.lineId || "Gönderimde atanacak"}</code></td><td><button type="button" disabled={fixedCountAuthority || lines.length === 1} onClick={() => removeLine(index)}>Kaldır</button></td></tr>)}</tbody></table></div>
        <div className={styles.mobileFormCards}>{lines.map((line, index) => <article key={`${line.lineId || "new"}-${index}`}><label><span>Varyant</span><select value={line.variantId} disabled={fixedCountAuthority} onChange={(event) => updateLine(index, { variantId: event.target.value })}><option value="">Varyant seçin</option>{choices.choices.variants.map((choice) => <option key={choice.variantId} value={choice.variantId}>{variantLabel(choice)}</option>)}</select></label><label><span>{props.mode === "count" ? "Sayılan miktar" : "Miktar"}</span><input inputMode="numeric" value={line.quantity} onChange={(event) => updateLine(index, { quantity: event.target.value })} /></label>{props.mode === "purchase" ? <label><span>Birim maliyet (kuruş)</span><input inputMode="numeric" value={line.unitCostCents} onChange={(event) => updateLine(index, { unitCostCents: event.target.value })} /></label> : null}<code>{line.lineId || "Gönderimde atanacak"}</code><button type="button" disabled={fixedCountAuthority || lines.length === 1} onClick={() => removeLine(index)}>Kalemi kaldır</button></article>)}</div>
        <button className={styles.secondaryAction} type="button" disabled={fixedCountAuthority || lines.length >= 500} onClick={addLine}>Kalem ekle</button>
      </fieldset>
