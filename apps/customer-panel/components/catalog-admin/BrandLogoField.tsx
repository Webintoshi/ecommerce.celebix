...
export function BrandLogoField({ assets, selectedId, disabled, brandName, onChange, onUpload }: Readonly<{
  ...
  const retainedWithoutAsset = selectedId !== undefined && selected === undefined;
  ...
  return <fieldset className={`${styles.wide} ${styles.brandLogoField}`}>
    ...
    <div className={styles.brandLogoLayout}>
      <div className={styles.brandLogoPreview}>
        {selected ? <>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={selected.publicUrl} alt={brandName.trim() || selected.altText} width={selected.width} height={selected.height} /></> : retainedWithoutAsset ? <span>Mevcut logo korunuyor</span> : <span>Henüz logo seçilmedi</span>}
      </div>
      <div className={styles.brandLogoControls}>
        <label>Kayıtlı logo<select value={selectedId ?? ""} disabled={disabled || uploading} onChange={(event) => onChange(event.currentTarget.value || undefined)}><option value="">Logo seçilmedi</option>{retainedWithoutAsset ? <option value={selectedId}>Mevcut logo</option> : null}{assets.map((asset) => <option value={asset.id} key={asset.id}>{asset.altText || `Logo ${asset.id.slice(0, 8)}`}</option>)}</select></label>
        ...
        <div className={styles.actions}>
          ...
          {selectedId ? <button className={styles.button} type="button" disabled={disabled || uploading} onClick={() => onChange(undefined)}>Logoyu kaldır</button> : null}
        </div>
