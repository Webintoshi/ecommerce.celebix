...
test("icon fills an empty barcode field but does not submit the product", async () => {
  let requested = 0;
  let submitted = 0;
  await withBarcodeInput({ name: "barcode", reserve: async () => { requested++; return "CXI-000000000123"; } }, async (container, browser) => {
    ...
    assert.equal((container.querySelector('input[name="barcode"]') as HTMLInputElement).value, "CXI-000000000123");
    assert.equal(new browser.FormData(browser.document.querySelector("form")!).get("barcode"), "CXI-000000000123");
