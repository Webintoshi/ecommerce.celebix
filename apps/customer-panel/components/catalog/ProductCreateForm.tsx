...
export function ProductCreateForm({ initialMode = "quick" }: Readonly<{ initialMode?: "quick" | "advanced" }>) {
  ...
  return (
    <section data-presentation="hemenaku-product-create" className={`catalog-page ${styles.createPage}`} aria-labelledby="create-title">
      ...
      {mode === "advanced" && options ? <ProductAdvancedEditor options={options} onCancel={() => location.assign("/products")} onCreated={(result) => location.assign(`/products/${result.product.id}`)} /> : <ProductQuickCreateDialog
          open
          mode="page"
          options={options}
