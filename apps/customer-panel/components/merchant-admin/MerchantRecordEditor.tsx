...
export function MerchantRecordEditor({
  ...
  recoverInvitationSource?: (recordId: string) => Promise<MerchantAdminRecord | "readonly" | "unknown">;
}) {
  ...
  const [invitationRecovery, setInvitationRecovery] = useState<"required" | "readonly" | null>(null);
  ...
  async function recoverSource(sequence: number) {
    if (kind !== "administrator_invite" || !record || !recoverInvitationSource) return;
    setInvitationRecovery("required");
    try {
      const current = await recoverInvitationSource(record.id);
      if (requestSequence.current !== sequence) return;
      if (current === "readonly") {
        setInvitationRecovery("readonly");
        setError("Kayıt artık gönderilmiş bir davete bağlı. Düzenleme kapalı.");
      } else if (current === "unknown") {
        setError("Güncel kayıt ve davet durumu doğrulanamadı. Bilgiler korunuyor; kaydetmeden önce durumu tekrar kontrol edin.");
      } else {
        // Rebase only concurrency metadata. The mounted uncontrolled fields and
        // original datetime snapshot retain the user's unsaved values.
        setRecord({ ...record, version: current.version, updatedAt: current.updatedAt });
        setInvitationRecovery(null);
        setError("Kayıt değişti. Güncel sürüm alındı; korunan bilgilerinizi kontrol edip yeniden Kaydet seçin.");
      }
    } catch {
      if (requestSequence.current === sequence) setError("Güncel kayıt ve davet durumu doğrulanamadı. Bilgiler korunuyor; kaydetmeden önce durumu tekrar kontrol edin.");
    }
  }

  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!canManage || busy || activeSubmission.current !== undefined || invitationRecovery !== null) return;
    ...
    try {
      ...
      if (requestSequence.current === sequence && caught instanceof MerchantAdminApiError && caught.code === "version_conflict" && kind === "administrator_invite" && record && recoverInvitationSource) await recoverSource(sequence);
    } finally {
  ...
  return <PanelPageShell><PanelPageHeader title={title} description={definition.description} /><section className={styles.surface}>
    ...
    {!loading && (!error || preserveOnError && (recordId === undefined || record !== undefined)) ? <form className={styles.form} onSubmit={submit}>
      <fieldset style={{ display: "contents" }} disabled={busy || pendingSave.current !== undefined || invitationRecovery !== null}>
      ...
      <div className={`${styles.wide} ${styles.actions}`}><button type="submit" className={styles.primary} disabled={busy || invitationRecovery !== null}>{busy ? "Kaydediliyor…" : pendingSave.current ? "Aynı kaydı tekrar kontrol et" : saveLabel}</button>{invitationRecovery === "required" ? <button type="button" disabled={busy} onClick={async () => { setBusy(true); try { await recoverSource(requestSequence.current); } finally { setBusy(false); } }}>Durumu tekrar kontrol et</button> : null}</div>
    </form> : null}
