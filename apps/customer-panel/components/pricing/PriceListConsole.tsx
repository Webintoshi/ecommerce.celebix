...
import type { CustomerTag, PriceChannel, PriceList, PriceListItem, PriceListRule } from "@celebix/saas-contracts";
...
import { loadCatalogVariantChoices, type CatalogProductChoice, type CatalogVariantChoice } from "@/lib/catalog-ui/variant-choices";
import { customerApi } from "@/lib/customer-ui/client";
import { buildPriceListIntent, canAddPricingRule, createPricingMutationController, createPricingPreviewController, createPricingRequestLifecycle, pricingApi, pricingErrorState, pricingRuleDraft, type PricingErrorState, type PricingPreviewSnapshot, type PricingRuleDraft } from "@/lib/pricing-ui/client";
...
const CHANNEL = Object.freeze({ storefront: "Mağaza", quick_order: "Hızlı sipariş", in_store: "Mağaza satışı" } as const);
...
function useChoices(enabled: boolean) {
  const [products, setProducts] = useState<readonly CatalogProductChoice[]>([]), [variants, setVariants] = useState<readonly CatalogVariantChoice[]>([]), [tags, setTags] = useState<readonly CustomerTag[]>([]), [failed, setFailed] = useState(false);
  useEffect(() => { if (!enabled) return; const controller = new AbortController(); setFailed(false); void (async () => { try { const [catalogChoices, safeTags] = await Promise.all([loadCatalogVariantChoices(undefined, controller.signal), customerApi.tags()]); if (!controller.signal.aborted) { setProducts(catalogChoices.products); setVariants(catalogChoices.variants); setTags(safeTags); } } catch { if (!controller.signal.aborted) { setProducts([]); setVariants([]); setTags([]); setFailed(true); } } })(); return () => controller.abort(); }, [enabled]);
...
function PriceListEditor(props: Readonly<{ resourceId?: string; canManage: boolean }>) {
  ...
  const [previewState, setPreviewState] = useState<PricingPreviewSnapshot>(() => Object.freeze({ phase: "idle" }));
  const choices = useChoices(props.canManage || Boolean(props.resourceId));
  const mutations = useMemo(() => createPricingMutationController(pricingApi), []);
  const previewController = useMemo(() => createPricingPreviewController(pricingApi, setPreviewState), []);
  const lifecycle = useMemo(() => createPricingRequestLifecycle(), []);
  const readOnly = !props.canManage || record?.status === "active" || record?.status === "archived";
  const controlsDisabled = readOnly || mutationPending;
  const canAddRule = canAddPricingRule(draftRules.length);
  useEffect(() => { const cleanup = lifecycle.setup(); return () => { cleanup(); previewController.dispose(); mutations.dispose(); }; }, [lifecycle, mutations, previewController]);
  useEffect(() => { if (!props.resourceId) return; const controller = new AbortController(), ticket = lifecycle.begin(); void pricingApi.get(props.resourceId, controller.signal).then((item) => { if (!ticket.current()) return; setRecord(item); setName(item.name); setDraftItems(item.items); setDraftRules(Object.freeze(item.rules.map(pricingRuleDraft))); setPhase("loaded"); }).catch((error: unknown) => { if (ticket.current() && !(error instanceof DOMException && error.name === "AbortError")) setPhase(pricingErrorState(error)); }); return () => { controller.abort(); ticket.cancel(); }; }, [lifecycle, props.resourceId]);
  const selectedVariants = useMemo(() => new Map(choices.variants.map((variant) => [variant.variantId, variant])), [choices.variants]);
  const productNames = useMemo(() => new Map(choices.products.map((product) => [product.productId, product.title])), [choices.products]);
  const addVariant = useCallback((variantId: string) => { const variant = selectedVariants.get(variantId); if (!variant || draftItems.some((item) => item.variantId === variantId)) return; setDraftItems((items) => Object.freeze([...items, { variantId, priceCents: 0 }])); }, [draftItems, selectedVariants]);
  const previewChannel = draftRules[0]?.channel ?? "storefront";
  const previewKey = draftItems.map((item) => item.variantId).join(",");
  const previewVariantIds = useMemo(() => Object.freeze(previewKey ? previewKey.split(",") : []), [previewKey]);
  useEffect(() => {
    if (!previewVariantIds.length) { previewController.clear(); return; }
    previewController.load({ channel: previewChannel, variantIds: previewVariantIds });
  }, [previewChannel, previewController, previewVariantIds]);
  const previewEntries = useMemo(() => previewState.phase === "loaded" ? new Map(previewState.result.entries.map((entry) => [entry.variantId, entry])) : new Map(), [previewState]);
  ...
  async function run(action: "save" | "activate" | "archive") {
    ...
    if (mutations.state() === "pending") return;
    ...
    } catch (error) { if (!ticket.current()) return; const next = pricingErrorState(error); setPhase(next); setMessage(next === "conflict" ? "Kayıt sizden önce değişti veya fiyat kuralları çakıştı." : next === "verification_unavailable" ? "İşlem sonucu doğrulanamıyor. Tam sayfa yenileme gereklidir." : "Fiyat listesi işlemi tamamlanamadı."); }
  ...
  return <div className={styles.editor}>
    ...
    <fieldset disabled={controlsDisabled}><legend>Liste ayrıntıları</legend><label>Ad<input value={name} maxLength={200} onChange={(event) => setName(event.target.value)} /></label></fieldset>
    <fieldset disabled={controlsDisabled}><legend>Fiyat kuralları</legend><div className={styles.ruleRows}>{draftRules.map((rule, index) => <div className={styles.ruleRow} key={index}><label>Kanal<select value={rule.channel} onChange={(event) => updateRule(index, { channel: event.target.value as PriceChannel })}><option value="storefront">Mağaza</option><option value="quick_order">Hızlı sipariş</option><option value="in_store">Mağaza satışı</option></select></label><label>Hedefleme<select value={rule.customerTagId} onChange={(event) => updateRule(index, { customerTagId: event.target.value })}><option value="">Tüm müşteriler (global)</option>{choices.tags.map((tag) => <option key={tag.id} value={tag.id}>{tag.name}</option>)}</select></label><label>Başlangıç (UTC)<input type="datetime-local" value={rule.startsAt} onChange={(event) => updateRule(index, { startsAt: event.target.value, persistedStartsAt: "" })} /></label><label>Bitiş (UTC)<input type="datetime-local" value={rule.endsAt} onChange={(event) => updateRule(index, { endsAt: event.target.value, persistedEndsAt: "" })} /></label><label>Öncelik<input type="number" min="0" max="1000" value={rule.priority} onChange={(event) => updateRule(index, { priority: event.target.value })} /></label>{draftRules.length > 1 ? <button type="button" onClick={() => setDraftRules((rules) => Object.freeze(rules.filter((_, candidate) => candidate !== index)))}>Kuralı kaldır</button> : null}</div>)}</div><button type="button" disabled={controlsDisabled || !canAddRule} onClick={() => setDraftRules((rules) => canAddPricingRule(rules.length) ? Object.freeze([...rules, pricingRuleDraft()]) : rules)}>Kural ekle</button>{!canAddRule ? <p className={styles.notice} role="status">En fazla 100 fiyat kuralı eklenebilir.</p> : null}</fieldset>
    <fieldset disabled={controlsDisabled}><legend>Sabit varyant fiyatları</legend><label>Varyant ekle<select defaultValue="" onChange={(event) => { addVariant(event.target.value); event.target.value = ""; }}><option value="" disabled>Ürün / varyant seçin</option>{choices.variants.map((variant) => <option key={variant.variantId} value={variant.variantId}>{productNames.get(variant.productId)} — {variant.variantTitle} ({variant.sku ?? variant.variantId.slice(0, 8)})</option>)}</select></label>{choices.failed ? <p className={styles.errorNotice} role="alert">Ürün veya müşteri etiketi seçenekleri yüklenemedi.</p> : null}<div className={styles.itemRows}>{draftItems.map((item) => { const variant = selectedVariants.get(item.variantId); return <div className={styles.itemRow} key={item.variantId}><span>{variant ? `${productNames.get(variant.productId)} — ${variant.variantTitle}` : item.variantId}</span><label>Taslak fiyat (kuruş)<input type="number" min="0" max="8000000000" value={item.priceCents} onChange={(event) => { const priceCents = Number(event.target.value); setDraftItems((items) => Object.freeze(items.map((candidate) => candidate.variantId === item.variantId ? { ...candidate, priceCents } : candidate))); }} /></label><button type="button" onClick={() => setDraftItems((items) => Object.freeze(items.filter((candidate) => candidate.variantId !== item.variantId)))}>Kaldır</button></div>; })}</div></fieldset>
    <section className={styles.preview}><h2>Açıklayıcı önizleme</h2><p>Bu tablo PostgreSQL’in {CHANNEL[previewChannel]} kanalı için döndürdüğü mevcut fiyatları taslaktan ayrı gösterir; etkinleştirme anında kalıcı yetki yeniden doğrulanır.</p>{previewState.phase === "loading" ? <p role="status">Sunucu önizlemesi yükleniyor…</p> : null}{previewState.phase === "unavailable" ? <p className={styles.errorNotice} role="alert">Önizleme kullanılamıyor; efektif fiyat hesaplanmadı.</p> : null}<table><thead><tr><th>Varyant</th><th>Taban fiyat</th><th>Mevcut efektif fiyat</th><th>Taslak fiyat</th></tr></thead><tbody>{draftItems.map((item) => { const server = previewEntries.get(item.variantId); return <tr key={item.variantId}><td>{selectedVariants.get(item.variantId)?.variantTitle ?? item.variantId}</td><td>{server ? money(server.basePriceCents) : "—"}</td><td>{server ? money(server.effectivePriceCents) : "—"}</td><td>{Number.isSafeInteger(item.priceCents) ? money(item.priceCents) : "Geçersiz"}</td></tr>; })}</tbody></table></section>
    <div className={styles.actions}>{!readOnly ? <button className={styles.primary} type="button" disabled={mutationPending} onClick={() => void run("save")}>Kaydet</button> : null}{record?.status === "draft" && props.canManage ? <button type="button" disabled={mutationPending} onClick={() => void run("activate")}>Etkinleştir</button> : null}{record && record.status !== "archived" && props.canManage ? <button type="button" disabled={mutationPending} onClick={() => void run("archive")}>Arşivle</button> : null}</div>
  </div>;
}

export function PriceListConsole(props: Readonly<{ mode?: "list" | "new" | "detail"; resourceId?: string; canRead: boolean; canManage: boolean; initialItems?: readonly PriceList[]; initialTags?: readonly CustomerTag[]; initialPhase?: ViewPhase }>) {
  const mode = props.mode ?? "list"; const [phase, setPhase] = useState<ViewPhase>(props.initialPhase ?? (props.canRead ? (props.initialItems ? (props.initialItems.length ? "loaded" : "empty") : "loading") : "denied")); const [items, setItems] = useState<readonly PriceList[]>(props.initialItems ?? []), [tags, setTags] = useState<readonly CustomerTag[]>(props.initialTags ?? []); const [nonce, setNonce] = useState(0);
  const lifecycle = useMemo(() => createPricingRequestLifecycle(), []);
