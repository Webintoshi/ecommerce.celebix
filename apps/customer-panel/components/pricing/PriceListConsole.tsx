...
function PriceListEditor(props: Readonly<{ resourceId?: string; canManage: boolean }>) {
  ...
  return <div className={styles.editor}>
    ...
    <fieldset disabled={controlsDisabled}><legend>Fiyat kuralları</legend><div className={styles.ruleRows}>{draftRules.map((rule, index) => <div className={styles.ruleRow} key={index}><label>Kanal<select value={rule.channel} onChange={(event) => updateRule(index, { channel: event.target.value as PriceChannel })}><option value="storefront">Mağaza</option><option value="quick_order">Hızlı sipariş</option></select></label><label>Hedefleme<select value={rule.customerTagId} onChange={(event) => updateRule(index, { customerTagId: event.target.value })}><option value="">Tüm müşteriler (global)</option>{choices.tags.map((tag) => <option key={tag.id} value={tag.id}>{tag.name}</option>)}</select></label><label>Başlangıç (UTC)<input type="datetime-local" value={rule.startsAt} onChange={(event) => updateRule(index, { startsAt: event.target.value, persistedStartsAt: "" })} /></label><label>Bitiş (UTC)<input type="datetime-local" value={rule.endsAt} onChange={(event) => updateRule(index, { endsAt: event.target.value, persistedEndsAt: "" })} /></label><label>Öncelik<input type="number" min="0" max="1000" value={rule.priority} onChange={(event) => updateRule(index, { priority: event.target.value })} /></label>{draftRules.length > 1 ? <button type="button" onClick={() => setDraftRules((rules) => Object.freeze(rules.filter((_, candidate) => candidate !== index)))}>Kuralı kaldır</button> : null}</div>)}</div><button type="button" disabled={controlsDisabled || !canAddRule} onClick={() => setDraftRules((rules) => canAddPricingRule(rules.length) ? Object.freeze([...rules, pricingRuleDraft()]) : rules)}>Kural ekle</button>{!canAddRule ? <p className={styles.notice} role="status">En fazla 100 fiyat kuralı eklenebilir.</p> : null}</fieldset>
