...
export function StoreAdminInvitationsConsole({ canManage, storeName }: { canManage: boolean; storeName: string }) {
  ...
  const [confirmation, setConfirmation] = useState<{ submission: Pending; email: string } | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setSourcesLoaded(sources.status === "fulfilled");
  ...
  function choose(action: Pending["action"], row: ReturnType<typeof invitationRows>[number]) {
    if (pendingRef.current || active.current || confirmation) return;
    const operationId = crypto.randomUUID();
    if (action === "send" && row.canSend && row.source) void run({ action, intent: { sourceRecordId: row.source.id, expectedRecordVersion: row.source.version, operationId } });
    else if (row.invitation && (action === "resend" ? row.canResend : action === "revoke" && row.canRevoke)) {
      const submission = { action, intent: { invitationId: row.invitation.id, expectedVersion: row.invitation.version, operationId } };
      if (action === "revoke") setConfirmation({ submission, email: row.email }); else void run(submission);
    }
  }
  ...
  return <section className={styles.workspace}>
    ...
    {confirmation ? <dialog ref={dialog} aria-labelledby="invitation-revoke-title" aria-describedby="invitation-revoke-description" onCancel={() => setConfirmation(null)}>
      <h2 id="invitation-revoke-title">Davet iptalini onayla</h2>
      <p id="invitation-revoke-description">{storeName} mağazasında {confirmation.email} için davet iptal edilecek. Bu işlem davet bağlantısını geçersiz kılar; mevcut üyeliği değiştirmez.</p>
      <div className={styles.actions}><button autoFocus onClick={() => setConfirmation(null)}>Vazgeç</button><button onClick={() => { const submission = confirmation.submission; setConfirmation(null); void run(submission); }}>İptali onayla</button></div>
    </dialog> : null}
    ...
    <div className={styles.tableScroll}><table aria-label="Yönetici davetleri"><thead><tr><th>Kişi ve rol</th><th>Davet durumu</th><th>E-posta durumu</th><th>İşlemler</th></tr></thead><tbody>{rows.map(row => <tr key={row.sourceRecordId}>
      ...
      <td><div className={styles.actions}>{row.canEdit ? <a href={`/settings/administrators/${row.sourceRecordId}/edit`}>Düzenle</a> : row.invitation ? <span>Gönderilen kayıt değiştirilemez</span> : null}
    ...
    {!loading && sourcesLoaded && !rows.length && list !== null && !list.hasMore ? <p>Henüz davet kaydı yok.</p> : null}
  </section>;
}
