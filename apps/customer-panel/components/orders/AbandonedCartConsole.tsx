...
import { Archive, ArrowDownUp, ArrowRight, CheckCircle2, Clock3, Copy, Filter, Link2, Mail, Package2, Phone, RefreshCcw, Search, ShoppingCart, UserRound, Wallet } from "lucide-react";
...
export function AbandonedCartDetailPresentation(props: Readonly<{ state: State; detail?: AbandonedCartDetail; error: string; notice: string; recoveryUrl?: string; note: string; busy: boolean; canManage: boolean; onRetry(): void; onIssueRecoveryLink(): void; onCopyRecoveryLink(): void; onRecordContact(): void; onNote(value: string): void; onAddNote(): void; onRecovered(): void; onArchive(): void }>) {
  ...
  return <PanelPageShell><Link className={styles.back} href="/orders/abandoned-carts">Terk Edilen Sepetlere dön</Link><PanelPageHeader title="Sepet ayrıntısı" description={`${customer(cart)} · sürüm ${cart.version}`} actions={props.canManage && cart.status !== "archived" ? <div className={styles.actions}>{cart.status === "abandoned" ? <><button type="button" disabled={props.busy} onClick={props.onIssueRecoveryLink}><Link2 aria-hidden="true" />Kurtarma bağlantısı oluştur</button><button type="button" disabled={props.busy} onClick={props.onRecovered}><CheckCircle2 aria-hidden="true" />Kurtarıldı olarak işaretle</button></> : null}<button type="button" disabled={props.busy} onClick={props.onArchive}><Archive aria-hidden="true" />Arşivle</button></div> : null} />{props.error ? <div className={styles.inlineError} role="alert">{props.error}</div> : null}{props.notice ? <div className={styles.notice} role="status">{props.notice}</div> : null}{props.recoveryUrl ? <div className={styles.notice}><strong>Süreli kurtarma bağlantısı</strong><span>Dönüşüm tamamlanana kadar bağlantı yeniden açılabilir.</span><input aria-label="Kurtarma bağlantısı" readOnly value={props.recoveryUrl} onFocus={(event) => event.currentTarget.select()} /><button type="button" disabled={props.busy} onClick={props.onCopyRecoveryLink}><Copy aria-hidden="true" />Bağlantıyı kopyala</button></div> : null}
    ...
    <div className={styles.detailGrid}><section className={styles.panel}><header><div><h2>Sepet ürünleri</h2><p>Kalıcı katalog anlık görüntüsü</p></div></header><div className={styles.items}>{cart.items.map((item) => <article key={item.id}>{item.imageUrl ? <img src={item.imageUrl} alt="" /> : <i><Package2 /></i>}<div><strong>{item.productName}</strong><span>{item.variantName ?? "Standart"}{item.sku ? ` · ${item.sku}` : ""}</span></div><span>{item.quantity} × {money(item.unitPriceCents, cart.currency)}</span><b>{money(item.lineTotalCents, cart.currency)}</b></article>)}</div><dl className={styles.totals}><div><dt>Ara toplam</dt><dd>{money(cart.subtotalCents, cart.currency)}</dd></div><div><dt>İndirim</dt><dd>− {money(cart.discountCents, cart.currency)}</dd></div><div><dt>Toplam</dt><dd>{money(cart.totalCents, cart.currency)}</dd></div></dl></section>
      <section className={styles.panel}><header><div><h2>Müşteri ve zaman</h2><p>Sepette bırakılan gerçek iletişim bilgileri</p></div></header><dl className={styles.customer}><div><dt><UserRound />Müşteri</dt><dd>{cart.customerName ?? "Anonim sepet"}</dd></div><div><dt><Mail />E-posta</dt><dd>{cart.customerEmail ?? "Belirtilmedi"}</dd></div><div><dt><Phone />Telefon</dt><dd>{cart.customerPhone ?? "Belirtilmedi"}</dd></div><div><dt><Clock3 />Sepet başlangıcı</dt><dd>{date(cart.checkoutStartedAt)}</dd></div><div><dt><RefreshCcw />Son etkinlik</dt><dd>{date(cart.lastActivityAt)}</dd></div></dl>{props.canManage && cart.status === "abandoned" ? <div className={styles.actions}><button type="button" disabled={props.busy} onClick={props.onRecordContact}>İletişim kuruldu</button><label><span>Merchant notu</span><input value={props.note} maxLength={1000} onChange={(event) => props.onNote(event.currentTarget.value)} /></label><button type="button" disabled={props.busy || props.note.trim().length === 0} onClick={props.onAddNote}>Not ekle</button></div> : null}</section></div>
  </PanelPageShell>;
}

export function AbandonedCartDetailConsole({ cartId, canManage }: { cartId: string; canManage: boolean }) {
  const [state, setState] = useState<State>("loading"); const [detail, setDetail] = useState<AbandonedCartDetail>(); const [error, setError] = useState(""); const [notice, setNotice] = useState(""); const [recoveryUrl, setRecoveryUrl] = useState<string>(); const [note, setNote] = useState(""); const [busy, setBusy] = useState(false);
  ...
  async function issueRecoveryLink() { if (!detail || !canManage || busy) return; setBusy(true); setError(""); setNotice(""); setRecoveryUrl(undefined); try { const result = await abandonedCartApi.issueRecoveryLink(cartId); setRecoveryUrl(result.url); setNotice(`Bağlantı ${date(result.expiresAt)} tarihine kadar geçerlidir.`); } catch (failure) { setError(message(failure)); } finally { setBusy(false); } }
  async function copyRecoveryLink() { if (!recoveryUrl || busy) return; try { await navigator.clipboard.writeText(recoveryUrl); setNotice("Kurtarma bağlantısı panoya kopyalandı."); } catch { setError("Bağlantı panoya kopyalanamadı. Metni seçip elle kopyalayın."); } }
  async function recordRecoveryAttempt(kind: "contacted" | "note") { if (!detail || !canManage || busy || (kind === "note" && note.trim().length === 0)) return; setBusy(true); setError(""); setNotice(""); try { await abandonedCartApi.recordRecoveryAttempt(cartId, kind, kind === "note" ? note.trim() : undefined); setNotice(kind === "contacted" ? "İletişim kuruldu olarak kaydedildi." : "Merchant notu kaydedildi."); if (kind === "note") setNote(""); } catch (failure) { setError(message(failure)); } finally { setBusy(false); } }
  return <AbandonedCartDetailPresentation state={state} detail={detail} error={error} notice={notice} recoveryUrl={recoveryUrl} note={note} busy={busy} canManage={canManage} onRetry={() => { setState("loading"); void load(); }} onIssueRecoveryLink={() => void issueRecoveryLink()} onCopyRecoveryLink={() => void copyRecoveryLink()} onRecordContact={() => void recordRecoveryAttempt("contacted")} onNote={setNote} onAddNote={() => void recordRecoveryAttempt("note")} onRecovered={() => void mutate("recovered")} onArchive={() => void mutate("archive")} />;
}
