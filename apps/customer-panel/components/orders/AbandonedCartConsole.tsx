...
import {
  AbandonedCartApiError,
  abandonedCartApi,
} from "@/lib/abandoned-cart-ui/client";
import styles from "./abandoned-cart-console.module.css";

type State = "loading" | "loaded" | "error";
const STATUS: Readonly<Record<AbandonedCartStatus, string>> = Object.freeze({
  active: "Aktif",
...
function customerDetail(cart: AbandonedCartListItem) {
  if (cart.customerName)
    return cart.customerEmail ?? cart.customerPhone ?? "İletişim bilgisi yok";
...
function product(cart: AbandonedCartListItem) {
  return cart.firstProductName ?? "Ürün bilgisi yok";
}
function CustomerIdentity({ cart }: { cart: AbandonedCartListItem }) {
  const content = (
    <>
      ...
      {cart.customerId ? (
        <span className={styles.accountBadge}>Kayıtlı müşteri</span>
      ) : null}
      <small>{customerDetail(cart)}</small>
    </>
  );
  return cart.customerId ? (
    ...
    <div className={styles.customerIdentity}>{content}</div>
  );
}
...
function message(error: unknown) {
  return error instanceof AbandonedCartApiError
    ? error.message
    : "Sepetler yüklenemedi. Lütfen yeniden deneyin.";
}
    ...
    <article className={styles.cartCard}>
      ...
      <dl>
        ...
        <div>
          <dt>Ürün</dt>
          <dd>
            <ProductIdentity cart={cart} />
          </dd>
        </div>
        <div>
          <dt>Son etkinlik</dt>
          <dd>
            <time dateTime={cart.lastActivityAt}>
      ...
      <Link
        ...
        href={`/orders/abandoned-carts/${cart.id}`}
      >
        İncele
...
export function AbandonedCartListPresentation(
  props: Readonly<{
    state: State;
    items: readonly AbandonedCartListItem[];
    ...
    <PanelPageShell>
      ...
      {props.summary ? (
        <section className={styles.metrics} aria-label="Sepet özeti">
          <Metric
            label="Terk edilen"
            value={props.summary.abandoned.toLocaleString("tr-TR")}
            detail="Takip bekleyen sepet"
            icon={ShoppingCart}
          />
          <Metric
            label="Kurtarılan"
            value={props.summary.recovered.toLocaleString("tr-TR")}
            detail="Kalıcı kurtarma kaydı"
            icon={CheckCircle2}
            emphasis="recovered"
          />
          <Metric
            label="Kayıp değer"
            value={money(props.summary.lostValueCents, props.summary.currency)}
            detail="Terk edilmiş sepet toplamı"
            icon={Wallet}
            emphasis="lost"
          />
          <Metric
            label="Kurtarılan değer"
            value={money(
              props.summary.recoveredValueCents,
              props.summary.currency,
            )}
            detail={`Son hesaplama ${date(props.summary.asOf)}`}
            icon={RefreshCcw}
            emphasis="recovered"
          />
        </section>
      ...
      <section
        ...
        aria-label="Terk edilen sepet çalışma alanı"
      >
        <form
        ...
        >
          ...
          <label className={styles.selectControl}>
            ...
            <select
              ...
              onChange={(event) =>
                props.onStatus(
                  event.target.value as AbandonedCartStatus | "all",
                )
              }
            >
          ...
          <label className={styles.selectControl}>
            ...
            >
              ...
              <option value="highest">Tutar: yüksekten düşüğe</option>
        ...
        {props.state === "loading" ? (
          ...
          <div className={styles.listEmpty}>
            ...
            <strong>
              {hasFilters
                ? "Filtrelerle eşleşen sepet bulunamadı."
                : "Henüz terk edilmiş sepet yok"}
            </strong>
            <p>
              {hasFilters
          ...
          <>
              ...
              <table aria-label="Terk edilen sepet listesi">
                <thead>
                  <tr>
                    ...
                    <th>Son etkinlik</th>
                    <th>Ürün</th>
                    <th className={styles.numericHeading}>Toplam</th>
                    <th className={styles.actionHeading}>İşlem</th>
                  </tr>
                </thead>
                <tbody>
            ...
            {props.nextCursor ? (
              ...
              >
                {props.loadingMore ? "Yükleniyor…" : "Daha fazla sepet yükle"}
              </button>
            ) : null}
          </>
        )}
...
export function AbandonedCartConsole() {
  ...
  const [items, setItems] = useState<readonly AbandonedCartListItem[]>([]);
  const [summary, setSummary] = useState<AbandonedCartSummary>();
  ...
  const load = useCallback(
    async (cursor?: string) => {
      ...
      try {
        ...
        setItems((existing) =>
          cursor ? Object.freeze([...existing, ...list.items]) : list.items,
        );
  ...
  return (
    <AbandonedCartListPresentation
      ...
      onSubmit={() => {
        ...
        if (value.length <= 200) setSearch(value);
      }}
      onStatus={setStatus}
      onSort={setSort}
      onLoadMore={() => {
...
export function AbandonedCartDetailPresentation(
  props: Readonly<{
    state: State;
    detail?: AbandonedCartDetail;
    error: string;
    notice: string;
    recoveryUrl?: string;
    note: string;
      ...
      {props.recoveryUrl ? (
        <div className={styles.notice}>
          ...
          <input
            ...
            readOnly
            value={props.recoveryUrl}
            onFocus={(event) => event.currentTarget.select()}
          />
          <button
            type="button"
            disabled={props.busy}
            onClick={props.onCopyRecoveryLink}
          >
      ...
      <div className={styles.detailGrid}>
        <section className={styles.panel}>
          ...
          <div className={styles.items}>
            {cart.items.map((item) => (
              <article key={item.id}>
                ...
                <b>{money(item.lineTotalCents, cart.currency)}</b>
              </article>
            ))}
          </div>
          <dl className={styles.totals}>
            <div>
              <dt>Ara toplam</dt>
              <dd>{money(cart.subtotalCents, cart.currency)}</dd>
            </div>
            <div>
              <dt>İndirim</dt>
              <dd>− {money(cart.discountCents, cart.currency)}</dd>
            </div>
            <div>
              <dt>Toplam</dt>
              <dd>{money(cart.totalCents, cart.currency)}</dd>
            </div>
          </dl>
        </section>
        <section className={styles.panel}>
          ...
          {props.canManage && cart.status === "abandoned" ? (
            <div className={styles.actions}>
              ...
              <button
                type="button"
                disabled={props.busy || props.note.trim().length === 0}
...
export function AbandonedCartDetailConsole({
  cartId,
  canManage,
}: {
  cartId: string;
  ...
  const [state, setState] = useState<State>("loading");
  const [detail, setDetail] = useState<AbandonedCartDetail>();
  const [error, setError] = useState("");
  ...
  const load = useCallback(async () => {
    setError("");
    try {
      setDetail(await abandonedCartApi.get(cartId));
      setState("loaded");
    } catch (failure) {
      setError(message(failure));
      setState("error");
  ...
  async function archive() {
    ...
    setNotice("");
    try {
      await abandonedCartApi.archive(cartId, detail.version);
      await load();
      setNotice("Sepet arşivlendi.");
    } catch (failure) {
      setError(message(failure));
      if (
        failure instanceof AbandonedCartApiError &&
  ...
  async function issueRecoveryLink() {
    ...
    try {
      const result = await abandonedCartApi.issueRecoveryLink(cartId);
      setRecoveryUrl(result.url);
      setNotice(
        `Bağlantı ${date(result.expiresAt)} tarihine kadar geçerlidir.`,
      );
    } catch (failure) {
      setError(message(failure));
    } finally {
  ...
  async function copyRecoveryLink() {
    if (!recoveryUrl || busy) return;
    try {
      await navigator.clipboard.writeText(recoveryUrl);
      setNotice("Kurtarma bağlantısı panoya kopyalandı.");
    } catch {
      setError("Bağlantı panoya kopyalanamadı. Metni seçip elle kopyalayın.");
    }
  }
  async function recordRecoveryAttempt(kind: "contacted" | "note") {
    if (
      !detail ||
      !canManage ||
      busy ||
    ...
    setNotice("");
    try {
      await abandonedCartApi.recordRecoveryAttempt(
        cartId,
        kind,
  ...
  return (
    <AbandonedCartDetailPresentation
      state={state}
      detail={detail}
      error={error}
      notice={notice}
      ...
      onIssueRecoveryLink={() => void issueRecoveryLink()}
      onCopyRecoveryLink={() => void copyRecoveryLink()}
      onRecordContact={() => void recordRecoveryAttempt("contacted")}
      onNote={setNote}
      onAddNote={() => void recordRecoveryAttempt("note")}
