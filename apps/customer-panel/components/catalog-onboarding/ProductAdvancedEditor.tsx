...
type EditorApi = Pick<typeof catalogOnboardingClient, "createProduct" | "publishAfterMedia" | "updateMerchandising">;
...
export function ProductAdvancedEditor({ options, onCancel, api = catalogOnboardingClient, editor, onCreated, onUpdated, onConflictReload }: ProductAdvancedEditorProps) {
  ...
  return <form className={styles.advancedEditor} onSubmit={submit} noValidate>
    {error ? <div className={styles.error} role="alert"><span>{error}</span>{conflict ? <button type="button" className={styles.secondary} onClick={onConflictReload}>Sunucudaki sürümü yükle</button> : null}</div> : null}
