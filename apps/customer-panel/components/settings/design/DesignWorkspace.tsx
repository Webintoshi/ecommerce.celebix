...
export function DesignWorkspace({ workspace, canManage, initialSection = "brand" }: Readonly<{ workspace: StorefrontDesignWorkspace; canManage: boolean; initialSection?: WorkspaceSection }>) {
  ...
  const topbarActions = useMemo(() => section === "theme" ? null : <div className={styles.topbarActions}><div className={styles.previewSwitch} role="group" aria-label="Önizleme boyutu"><button type="button" className={previewMode === "desktop" ? styles.active : ""} onClick={() => setPreviewMode("desktop")}><Monitor size={17} />Masaüstü</button><button type="button" className={previewMode === "mobile" ? styles.active : ""} onClick={() => setPreviewMode("mobile")}><Smartphone size={17} />Mobil</button></div><button type="button" className={styles.publishButton} title={publishIssueLabel ?? undefined} disabled={!canManage || publishIssue !== null || ["saving", "publishing", "conflict"].includes(editor.status)} onClick={() => void publish()}>Yayınla</button></div>, [canManage, editor.status, previewMode, publish, publishIssue, publishIssueLabel, section]);

  return <section className={styles.workspace} data-panel-layout="open-canvas"><PanelTopbarBridge title="Tasarım" subtitle={section === "theme" ? undefined : publishIssueLabel ?? STATUS_LABEL[editor.status]} actions={topbarActions} /><nav className={styles.sectionRail} aria-label="Tasarım bölümleri">{SECTIONS.map(([key, label]) => <button type="button" key={key} className={section === key ? styles.active : ""} aria-current={section === key ? "page" : undefined} onClick={() => setSection(key)}>{label}</button>)}</nav>{section === "theme" ? <div className={styles.themeCanvas}><StarterThemeComposer canManage={canManage} /></div> : <><div className={styles.inspector}><DesignInspector section={section} design={editor.design} storeName={workspace.store.name} timezone={workspace.store.timezone} media={media} destinations={workspace.destinations} canManage={canManage} onChange={change} onUpload={upload} /></div><main className={styles.preview}><DesignPreview design={editor.design} storeName={workspace.store.name} publishedVersion={publishedVersionRef.current} publishedAt={workspace.publishedAt} media={media} destinations={workspace.destinations} mode={previewMode} now={nowRef.current} /></main></>}</section>;
}
