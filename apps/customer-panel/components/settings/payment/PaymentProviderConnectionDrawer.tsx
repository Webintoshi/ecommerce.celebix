...
export function PaymentProviderConnectionDrawer(props: Readonly<{
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const publicConfig = Object.freeze(Object.fromEntries(
    ...
    <div className={styles.drawerLayer} onMouseDown={(event) => {
      ...
      >
        ...
        <form className={styles.connectionForm} autoComplete="off" onSubmit={(event) => void save(event)}>
          ...
          {props.descriptor.publicFields.map((field) => (
            <label key={field.key}>{field.label}<input name={field.key} required maxLength={1_000} disabled={busy || !props.canManage} /></label>
          ))}
          {props.descriptor.credentialFields.map((field) => (
          ...
          <footer className={styles.drawerActions}>
            ...
            <button type="submit" className={styles.primaryButton} disabled={busy || !props.canManage}>{busy ? "Kaydediliyor…" : "Bağlantıyı kaydet"}</button>
          </footer>
        </form>
      </aside>
    </div>
