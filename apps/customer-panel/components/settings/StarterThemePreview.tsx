...
export function StarterThemePreview(props: PreviewProps) {
  ...
  if (props.composition) {
    ...
    return <section className={styles.previewSection} aria-labelledby="starter-composition-preview-title">
      ...
      <div className={`${styles.previewViewport} ${styles[mode]}`}><div className={`${styles.previewStore} ${styles[`theme-${composition.visual.colorScheme}`]} ${styles[`heading-${composition.visual.headingStyle}`]} ${styles[`cards-${composition.visual.productCardStyle}`]} ${styles[`images-${composition.visual.productImageRatio}`]}`}>
        {composition.announcement.enabled ? <div className={styles.previewMarquee}><span className={styles.previewMarqueeTrack}>{composition.announcement.items.join(" · ")}</span></div> : null}
