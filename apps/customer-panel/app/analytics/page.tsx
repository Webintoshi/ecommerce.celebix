...
export default async function AnalyticsPage({
  ...
  const [{ tenantContext }, query] = await Promise.all([
    requireServerPanelAccess(),
    searchParams,
  ]);
  if (!isMerchantActionAllowed(tenantContext.membership.role, "analytics.read"))
    redirect("/unauthorized");
  const rawTab = typeof query?.tab === "string" ? query.tab : "overview";
  const rawRange = typeof query?.range === "string" ? query.range : "30d";
  const tab = (TABS.has(rawTab) ? rawTab : "overview") as
  ...
  const initialTimezone =
    typeof query?.timezone === "string" && query.timezone.length <= 64
      ? query.timezone
      : undefined;
  const range = (
    customFrom && customTo ? "custom" : RANGES.has(rawRange) ? rawRange : "30d"
  ) as "today" | "7d" | "30d" | "90d" | "custom";
  return (
    <CommerceAnalyticsWorkspace
      tab={tab}
      range={range}
      compare={query?.compare === "1"}
