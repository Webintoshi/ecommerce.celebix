...
export function RetailFooter({ groups, presentation, storefront }: Readonly<{
  ...
  return <footer className="store-footer retail-footer" data-footer-tone={presentation.footer.tone}>
    ...
    <div className="store-container retail-footer-grid">
      ...
      {groups.map((group) => <nav aria-label={group.heading} className="retail-footer-desktop-group" key={group.heading}><span>{group.heading}</span>{group.links.map((link) => <Link href={localizeStorefrontPath(link.destination, storefront.locale)} key={`${link.destination}-${link.label}`}>{link.label}</Link>)}</nav>)}
      <div className="retail-footer-mobile">{groups.map((group) => <details key={group.heading}><summary>{group.heading}</summary><nav aria-label={`${group.heading} mobil`}>{group.links.map((link) => <Link href={localizeStorefrontPath(link.destination, storefront.locale)} key={`${link.destination}-${link.label}`}>{link.label}</Link>)}</nav></details>)}</div>
    </div>
