...
export function isActivePath(pathname: string, href: string) { return pathname === href || pathname.startsWith(`${href}/`); }
...
export function CampaignHeaderClient({ displayName, logo, navigation, desktopNavigation }: Readonly<{ displayName: string; logo?: PublicStorefrontAsset; navigation: PublicStarterNavigation; desktopNavigation: ReactNode }>) {
  ...
  useEffect(() => { const sentinel = sentinelRef.current; if (!sentinel || typeof IntersectionObserver === "undefined") return; const observer = new IntersectionObserver(([entry]) => setOpaque(!entry?.isIntersecting), { threshold: 1 }); observer.observe(sentinel); return () => observer.disconnect(); }, []);
  ...
  return <><span className={styles.sentinel} ref={sentinelRef} aria-hidden="true" /><div className={`${styles.bar} ${opaque && !nonHome ? styles.opaque : ""} ${nonHome}`}>
    <div className={styles.container}>{desktopNavigation}<Link className={styles.wordmark} href="/" aria-label={`${displayName} ana sayfa`}>{logo ? /* eslint-disable-next-line @next/next/no-img-element */<img src={logo.url} alt={logo.altText} width={logo.width} height={logo.height} /> : displayName}</Link><div className={styles.actions}><StoreUtilities /><button className={styles.menuButton} ref={triggerRef} type="button" aria-expanded={open} aria-controls="campaign-mobile-menu" aria-label="Menüyü aç" onClick={() => setOpen(true)}><Menu aria-hidden="true" /></button></div></div>
