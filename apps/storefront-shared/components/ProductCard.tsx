...
export function ProductCard({ product, cardStyle, imageRatio }: { product: PublicProduct; cardStyle: PublicStarterThemePresentation["theme"]["productCardStyle"]; imageRatio: PublicStarterThemePresentation["theme"]["productImageRatio"] }) {
  ...
  return <article className={`product-card card-${cardStyle} image-${imageRatio}`}><Link className="product-card-link" href={`/products/${product.slug}`}><div className="product-image-shell">{primary ? <>{/* eslint-disable-next-line @next/next/no-img-element */}<img className="product-image-primary" src={primary.url} alt={primary.altText || product.title} loading="lazy" width={primary.width} height={primary.height} />{secondary ? /* eslint-disable-next-line @next/next/no-img-element */<img className="product-image-secondary" src={secondary.url} alt="" loading="lazy" width={secondary.width} height={secondary.height} /> : null}</> : <span>Görsel yakında</span>}{badge ? <em className={`product-card-badge is-${badge}`}>{badge === "sale" ? "İndirim" : "Tükendi"}</em> : null}</div><div className="product-card-copy">{product.brand?.name ? <span className="product-card-brand">{product.brand.name}</span> : null}<h3>{product.title}</h3><div className="price-row">{product.compareAtCents && product.compareAtCents > product.priceCents ? <del>{formatTry(product.compareAtCents)}</del> : null}<strong>{formatTry(product.priceCents)}</strong></div></div></Link>{purchaseVariant ? <ProductCardCartButton productId={product.id} variantId={purchaseVariant.id} productTitle={product.title} available label="Sepete ekle" /> : action === "choose_options" ? <ProductQuickView product={product} /> : <button className="product-card-cart" type="button" disabled>Tükendi</button>}<FavoriteButton productId={product.id} productTitle={product.title} /></article>;
}
