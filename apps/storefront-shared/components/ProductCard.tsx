...
export function ProductCard({ product, locale, cardStyle, imageRatio }: Readonly<{
  ...
  const purchaseVariant = action === "quick_add"
    ? product.variants.find(({ available }) => available)
    : undefined;
  return (
    <article className={`product-card card-${cardStyle} image-${imageRatio}`}>
      <Link className="product-card-link" href={productPath(locale, product.slug)}>
        <div className="product-image-shell">
          {primary ? (
            <>
              ...
              {secondary ? (
                /* eslint-disable-next-line @next/next/no-img-element */
                <img className="product-image-secondary" src={secondary.url} alt="" loading="lazy" width={secondary.width} height={secondary.height} />
              ) : null}
            </>
      ...
      {purchaseVariant ? (
        <ProductCardCartButton productId={product.id} variantId={purchaseVariant.id} productTitle={product.title} available label="Sepete ekle" />
      ) : action === "choose_options" ? (
        <ProductQuickView product={product} />
      ) : (
        <button className="product-card-cart" type="button" disabled>Tükendi</button>
      )}
