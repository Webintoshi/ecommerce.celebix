...
test("homepage product rows keep merchant products visible while moving sold-out items to the end", async () => {
  ...
  assert.deepEqual(module.homepageAvailableProducts([unavailable, available] as never).map(({ id }) => id), ["available", "sold-out"]);
  assert.deepEqual(module.visibleCampaignSectionKinds({ presentation, productRows: [{ key: "category-kolyeler", items: [unavailable] }] } as never), ["product_row"]);
