...
export function CampaignCategories({ section, locale, prefetch }: Readonly<{ section: CategorySection; locale: string; prefetch?: boolean }>) {
...
export function CampaignPanels({ section, locale, prefetch }: Readonly<{ section: SplitSection; locale: string; prefetch?: boolean }>) {
  if (!section.panels.length) return null;
  return (
    <section className={styles.panels} aria-label="Kampanyalar">
      {section.panels.map((panel, index) => (
        <Link href={localizeStorefrontPath(panel.destination, locale)} prefetch={prefetch} key={`${panel.heading}-${index}`}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
...
export function CampaignStory({ section, locale, prefetch }: Readonly<{ section: StorySection; locale: string; prefetch?: boolean }>) {
  return (
    <section className={`${styles.story} ${section.image ? styles.hasStoryImage : ""}`}>
      ...
      <div>
        ...
        {section.destination ? <Link href={localizeStorefrontPath(section.destination, locale)} prefetch={prefetch}>Hikâyemizi keşfet →</Link> : null}
      </div>
    </section>
  );
}
