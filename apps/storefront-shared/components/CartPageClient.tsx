...
function CartLineControls({ line, version, disabled, onPending }: Readonly<{ line: PublicCartLine; version: number; disabled: boolean; onPending(value: boolean): void }>) {
  ...
  const remove = async () => { onPending(true); try { replaceCart(await storefrontCartClient.remove({ variantId: line.variantId, expectedVersion: version })); emitStorefrontCommerceEvent({ name: "remove_from_cart", data: { productId: line.productId, variantId: line.variantId, quantity: line.quantity, currency: "TRY", valueMinor: line.lineTotalCents } }); } catch { await refresh(); } finally { onPending(false); } };
...
export function CartPageClient({ locale, recovered = false, omittedItems = 0 }: Readonly<{ locale: string; recovered?: boolean; omittedItems?: number }>) {
  ...
  useEffect(()=>{if(visibleCart&&visibleCart.items.length>0)emitStorefrontCommerceEvent({name:"view_cart",data:{currency:"TRY",valueMinor:visibleCart.totalCents}})},[visibleCart]);
