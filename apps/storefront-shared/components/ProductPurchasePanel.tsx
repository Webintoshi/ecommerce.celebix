...
export function ProductPurchasePanel({ product }: Readonly<{ product: PublicProduct }>) {
  ...
  return <section className="purchase-panel" aria-labelledby="purchase-variants-title">
    ...
    <div className="purchase-actions"><button className="store-button" type="button" disabled={pending !== null || !allowed} onClick={() => void run("add")}>{pending === "add" ? "Ekleniyor…" : "Sepete ekle"}</button><button className="store-button store-button-secondary" type="button" disabled={pending !== null || !allowed} onClick={() => void run("buy")}>{pending === "buy" ? "Hazırlanıyor…" : "Şimdi satın al"}</button></div>
