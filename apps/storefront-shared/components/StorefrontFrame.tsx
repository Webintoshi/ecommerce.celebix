...
export function StorefrontFrame({ storefront, children }: { storefront: PublicStorefront; children: React.ReactNode }) {
  ...
  return <CartStatusProvider presentation={campaign.cart}><FavoriteStatusProvider><div className={`starter-storefront ${campaign.campaignClass} ${campaign.cornerClass} ${tokens.schemeClass} ${tokens.headingClass} ${tokens.cardClass} ${tokens.imageClass}`}><Header storefront={storefront} /><main>{children}</main><Footer storefront={storefront} /></div></FavoriteStatusProvider></CartStatusProvider>;
}
