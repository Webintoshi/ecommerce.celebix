...
function paymentFixture(status: "reserved" | "provider_ready" | "initiation_unknown" = "reserved", outcome: "created" | "replayed" = "created") {
  ...
  const storefrontRepository = { async getPublicStorefront() { throw new Error("unused"); }, async listPublicProducts() { throw new Error("unused"); }, async listPublicProductsByCategory() { throw new Error("unused"); }, async getPublicProductBySlug() { throw new Error("unused"); }, async listPublicProductMedia() { throw new Error("unused"); } };
