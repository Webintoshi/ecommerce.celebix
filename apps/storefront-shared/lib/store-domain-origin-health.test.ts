...
test("origin health returns only the exact database-bound hostname marker", async () => {
  const route = createStoreDomainOriginHealthRoute({
    ...
    resolveCacheDependency: async () => ({ required: false, status: "unavailable", metrics: null }),
    now: () => NOW,
  });

  const response = await route(request());
  assert.equal(response.status, 200);
  assert.deepEqual(await response.json(), { schemaVersion: 1, status: "ok", storeId: STORE, hostname: HOST, dependencies: { redisCache: { required: false, status: "unavailable", metrics: null } } });
