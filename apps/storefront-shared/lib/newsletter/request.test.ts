...
test("newsletter request ignores ordinary same-store cart cookies as tenant authority", async () => {
  assert.deepEqual(await parseNewsletterSubscribeRequest(request(
    { email: "ada@example.test", consent: true },
    { headers: { origin: ORIGIN, "content-type": "application/json", cookie: "__Host-celebix_cart=c1.browser-session" } },
  ), ORIGIN), { email: "ada@example.test", consent: true });
});
