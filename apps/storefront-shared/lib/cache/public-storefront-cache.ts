...
export function createCachedPublicStorefrontRepository(
  ...
  return Object.freeze({
    ...
    listPublicProductsByCategory: (input: Parameters<PublicStorefrontRepository["listPublicProductsByCategory"]>[0]) => cached({ storeId: input.storefront.id, dataClass: "catalog", scope: "category-products", input: { slug: input.slug, limit: input.limit }, ttlSeconds: ttl.catalogSeconds, parser: parseCategoryList, load: () => repository.listPublicProductsByCategory(input) }),
    ...
    getPublicStorefrontDesign: (input: Parameters<PublicStorefrontRepository["getPublicStorefrontDesign"]>[0]) => cached({ storeId: input.storefront.id, dataClass: "settings", scope: "storefront-design", input: {}, ttlSeconds: ttl.settingsSeconds, parser: parsePublicStorefrontDesign, load: () => repository.getPublicStorefrontDesign(input) }),
    ...(repository.resolveCampaignHome ? { resolveCampaignHome: (input: Parameters<NonNullable<PublicStorefrontRepository["resolveCampaignHome"]>>[0]) => cached({ storeId: input.storefront.id, dataClass: "catalog", scope: "campaign-home", input: {}, ttlSeconds: ttl.catalogSeconds, parser: parseCampaign, load: () => repository.resolveCampaignHome!(input) }) } : {}),
