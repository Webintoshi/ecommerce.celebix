...
import { createStorefrontOperationCredential, parseStorefrontCommerceCredentialKeyring, readStorefrontCredentialCookie } from "./credential.ts";
...
function runtime(repository: StorefrontCommerceRepository, selectedKeyring = keyring) {
  let uuidIndex = 0;
  ...
  return createStorefrontCommerceRuntime({ repository, keyring: selectedKeyring, now: () => new Date(NOW), randomBytes: (size) => new Uint8Array(size).fill(9), randomUuid: () => uuids[uuidIndex++] ?? randomUUID() });
}
