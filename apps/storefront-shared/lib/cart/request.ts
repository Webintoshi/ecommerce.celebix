...
function cartInvalid(): never {
  throw new TypeError("storefront_cart_request_invalid");
}
function checkoutInvalid(): never {
  throw new TypeError("storefront_checkout_request_invalid");
}
type Invalid = () => never;
...
function uuid(value: unknown, invalid: Invalid) {
  return text(value, 36, 36, invalid, UUID);
}
function integer(
  value: unknown,
  minimum: number,
...
async function jsonBody(
  ...
  for (const name of request.headers.keys())
    if (name.startsWith("x-celebix-") && name !== "x-celebix-storefront-proxy")
      invalid();
  const declared = request.headers.get("content-length");
  if (
    declared !== null &&
    (!/^(?:0|[1-9]\d*)$/.test(declared) ||
      Number(declared) > MAXIMUM_BODY_BYTES)
  )
    invalid();
  ...
  try {
    ...
    if (
      ...
      (error.message === "storefront_cart_request_invalid" ||
        error.message === "storefront_checkout_request_invalid")
    )
      throw error;
    return invalid();
  } finally {
...
export async function readCartMutationRequest(
  request: Request,
  publicOrigin: string,
): Promise<CartCommand> {
  let selected: { path: string; body: unknown };
  try {
    selected = await jsonBody(
      request,
      [
        "/api/cart/add",
        "/api/cart/quantity",
  ...
  const operation = (row: Record<string, unknown>) =>
    uuid(row.operationId, cartInvalid);
  if (selected.path === "/api/cart/add") {
    const row = exact(
      selected.body,
      ["operationId", "productId", "variantId", "quantity"],
      ["expectedVersion", "attribution"],
      cartInvalid,
    );
    return Object.freeze({
      kind: "add",
      operationId: operation(row),
      productId: uuid(row.productId, cartInvalid),
      variantId: uuid(row.variantId, cartInvalid),
      quantity: integer(row.quantity, 1, 9_999, cartInvalid),
      ...(Object.hasOwn(row, "expectedVersion")
        ? {
  ...
  if (selected.path === "/api/cart/quantity") {
    const row = exact(
      selected.body,
      ["operationId", "variantId", "quantity", "expectedVersion"],
      ["attribution"],
      cartInvalid,
    );
    return Object.freeze({
  ...
  if (selected.path === "/api/cart/remove") {
    const row = exact(
      selected.body,
      ["operationId", "variantId", "expectedVersion"],
      ["attribution"],
      cartInvalid,
    );
    return Object.freeze({
...
function contact(value: unknown): CheckoutContact {
  const row = exact(value, ["name", "email", "phone"], [], checkoutInvalid);
  return Object.freeze({
...
function address(value: unknown): CheckoutShippingAddress {
  ...
  return Object.freeze({
    addressLine1: text(row.addressLine1, 3, 300, checkoutInvalid),
    ...(Object.hasOwn(row, "addressLine2")
      ? { addressLine2: text(row.addressLine2, 1, 300, checkoutInvalid) }
      : {}),
    city: text(row.city, 2, 100, checkoutInvalid),
    district: text(row.district, 2, 100, checkoutInvalid),
    ...(Object.hasOwn(row, "postalCode")
      ? { postalCode: text(row.postalCode, 1, 16, checkoutInvalid, POSTAL) }
      : {}),
  });
}
...
function identityNumber(value: unknown): string {
  ...
  if (/^([0-9])\1{10}$/u.test(selected) || selected === "12345678901")
    checkoutInvalid();
...
export async function readCheckoutRequest(
  ...
  if (selected.path === "/api/checkout/quote") {
    const row = exact(
      selected.body,
      ["intentKind"],
      ["attribution"],
  ...
  if (selected.path === "/api/checkout/payment/start") {
    ...
    if (
      (row.intentKind !== "cart" && row.intentKind !== "buy_now") ||
      row.shippingMethod !== "standard"
    )
      checkoutInvalid();
    return Object.freeze({
      kind: "hosted_start",
      operationId: uuid(row.operationId, checkoutInvalid),
      cartVersion: integer(
        row.cartVersion,
        1,
        Number.MAX_SAFE_INTEGER,
  ...
  if (
    (row.intentKind !== "cart" && row.intentKind !== "buy_now") ||
    row.shippingMethod !== "standard" ||
    (row.paymentKind !== "bank_transfer" &&
      row.paymentKind !== "cash_on_delivery")
  )
    checkoutInvalid();
  return Object.freeze({
    kind: "complete",
