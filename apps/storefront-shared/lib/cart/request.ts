...
export async function readCartMutationRequest(request: Request, publicOrigin: string): Promise<CartCommand> {
  ...
  if (selected.path === "/api/cart/remove") { const row = exact(selected.body, ["operationId", "variantId", "expectedVersion"], ["attribution"], cartInvalid); return Object.freeze({ kind: "remove", operationId: operation(row), variantId: uuid(row.variantId, cartInvalid), expectedVersion: integer(row.expectedVersion, 0, Number.MAX_SAFE_INTEGER, cartInvalid), ...(Object.hasOwn(row, "attribution") ? { attribution: parseCommerceAttribution(row.attribution) } : {}) }); }
