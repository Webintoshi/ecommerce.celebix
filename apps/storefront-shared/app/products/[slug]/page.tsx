...
import type { PublicPolicyPage, StarterProductDetailConfigV2, StorefrontPolicyKey } from "@celebix/saas-contracts";
...
const LEGACY_PRODUCT_DETAIL: StarterProductDetailConfigV2 = Object.freeze({ galleryStyle: "grid", showSku: true, showBrand: true, showBreadcrumbs: true, showRelatedProducts: true, showApprovedReviews: true, mobileStickyPurchase: true, showSizeGuide: true, informationSections: Object.freeze(["description", "materials_and_care", "certifications", "shipping_and_returns"] as const) });
const PRODUCT_POLICY_KEYS = Object.freeze(["payment_delivery", "returns_exchanges"] as const satisfies readonly StorefrontPolicyKey[]);
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const options: StarterProductDetailConfigV2 = presentation.schemaVersion === 3 ? presentation.productDetail : presentation.schemaVersion === 2 ? Object.freeze({ ...LEGACY_PRODUCT_DETAIL, ...presentation.productDetail }) : LEGACY_PRODUCT_DETAIL;
  const [relatedProducts, publishedPolicies] = await Promise.all([
    options.showRelatedProducts && selected.runtime.repository.listRelatedPublicProducts ? selected.runtime.repository.listRelatedPublicProducts({ storefront, now: new Date(), productSlug: item.slug, limit: 4 }).then(({ items }) => items).catch(() => []) : Promise.resolve([]),
    options.informationSections.includes("shipping_and_returns") ? Promise.all(PRODUCT_POLICY_KEYS.map(async (key) => { try { const policy = buildPublicPolicyPage(await selected.runtime.content.getPolicy({ hostname: storefront.hostname, now: new Date(), key })); return policy.published ? policy : null; } catch { return null; } })).then((policies) => policies.filter((policy): policy is PublicPolicyPage => policy !== null)) : Promise.resolve([]),
  ]);
  return <StorefrontFrame storefront={storefront}>
    ...
    <ProductDetailExperience product={item} relatedProducts={relatedProducts} publishedPolicies={publishedPolicies} options={options} cardStyle={presentation.theme.productCardStyle} imageRatio={presentation.theme.productImageRatio} />
  </StorefrontFrame>;
}
