...
export default async function HomePage() {
  ...
  const products = await runtime.repository.listPublicProducts({
    storefront,
    now: new Date(),
    limit: presentation.theme.homeProductLimit,
  });
  const heroMedia =
    presentation.hero.image ??
    products.items.find((product) => product.media.length)?.media[0];
  const marqueeTokens = presentation.marquee
    ? starterMarqueeTokens(presentation.marquee)
    : null;
  if (design.publicationVersion > 1)
    return (
  ...
  return (
    <StorefrontFrame storefront={storefront} design={design}>
      {presentation.marquee && marqueeTokens ? (
        <aside
          className={`store-marquee ${marqueeTokens.iconClass} ${marqueeTokens.speedClass} ${marqueeTokens.directionClass} ${marqueeTokens.animationClass}`}
          aria-label="Mağaza duyuruları"
        >
          <span className="marquee-icon" aria-hidden="true">
            {marqueeTokens.iconSymbol}
          </span>
          <span className="marquee-track">
            {presentation.marquee.items.join(" · ")}
          </span>
        </aside>
      ) : null}
      {presentation.promotion ? (
        ...
        >
          ...
          {presentation.promotion.body ? (
            <small>{presentation.promotion.body}</small>
          ) : null}
        </Link>
      ) : null}
      {presentation.hero.enabled ? (
        <section className={`home-hero ${heroMedia ? "has-hero-media" : ""}`}>
          {heroMedia ? (
            <img
          ...
          <div className="store-container hero-copy">
      ...
      {presentation.theme.showBrandStory ? (
        <section className="brand-story">
          <div className="store-container">
            <span>MAĞAZA DENEYİMİ</span>
            <h2>Az, öz ve özenle seçilmiş.</h2>
            <p>
              {presentation.displayName}, ürünlerini güvenli Celebix altyapısı
              üzerinden aynı sade vitrin diliyle sunar.
            </p>
          </div>
        </section>
      ) : null}
