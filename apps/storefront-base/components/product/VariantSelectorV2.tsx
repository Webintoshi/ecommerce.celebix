  ...
  return (
    <div className="space-y-6">
        ...
        return (
          <div key={group.id} className="space-y-3">
            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold uppercase tracking-wide text-[#7B1113]">
                {group.name}
              </span>
              {selectedValue ? (
                <>
                  <span className="text-gray-400">-</span>
                  <span className="text-sm font-medium text-[#6b4b4c]">{selectedValue}</span>
                </>
              ) : null}
            </div>
                ...
                {group.values.map((value) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "relative h-16 w-16 overflow-hidden rounded-full border-2 transition-all duration-200",
                        isSelected
                    ...
                    >
                      ...
                      {isSelected ? <div className="absolute inset-0 rounded-full border-2 border-[#7B1113]" /> : null}
                    </button>
                  );
                })}
              </div>
                      ...
                      className={cn(
                        ...
                        isSelected
                          ...
                          : isOutOfStock
                            ...
                            : "border-gray-300 bg-white text-[#7B1113] hover:border-[#7B1113]",
                      )}
                    >
      ...
      <p className="text-sm text-[#6b4b4c]">
        Secilen: <span className="font-medium text-[#7B1113]">{currentVariant?.name}</span>
      </p>
    </div>
  );
}
