...
export default function CheckoutPage() {
  ...
  const handleNextStep = () => {
    ...
    if (!selectedShippingMethod || shippingRates.length === 0) {
      toast.error("Bu teslimat bolgesi icin kargo secenegi bulunamadi.");
      return;
    }
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-3 rounded-2xl border border-gray-200 bg-gray-50/60 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <p className="text-sm font-semibold text-gray-900">Teslimat Yontemi</p>
                          <p className="text-xs text-gray-500">Checkout ekraninda musteriye gosterilir.</p>
                        </div>
                        {selectedShippingRate ? (
                          <span className={cn("text-sm font-semibold", resolvedShippingCost === 0 ? "text-emerald-600" : "text-gray-900")}>
                            {resolvedShippingCost === 0 ? "Ucretsiz" : formatPrice(resolvedShippingCost)}
                          </span>
                        ) : null}
                      </div>
                      ...
                      {shippingRates.length > 0 ? (
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="flex flex-wrap items-center justify-between gap-2">
                                    ...
                                    <span className={cn("text-sm font-semibold", ratePrice === 0 ? "text-emerald-600" : "text-gray-900")}>
                                      {ratePrice === 0 ? "Ucretsiz" : formatPrice(ratePrice)}
                                    </span>
                                  </div>
                                  {(rate.estimatedDays || rate.minOrder) ? (
                                    <p className="mt-1 text-xs text-gray-500">
                                      {[rate.estimatedDays, rate.minOrder ? `${formatPrice(rate.minOrder)} uzeri ucretsiz` : null]
                                        .filter(Boolean)
                                        .join(" - ")}
                                    </p>
                                  ) : null}
                        ...
                        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                          Bu adres icin tanimli teslimat secenegi bulunamadi.
                        </div>
                      )}
                    </div>
