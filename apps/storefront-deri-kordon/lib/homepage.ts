...
function normalizeHeroSlides(payload: unknown): HomepageHeroBanner[] {
  ...
  return rawSlides
    .map((slide, index) => {
      ...
      return {
        id: typeof rawId === "number" || typeof rawId === "string" ? rawId : index + 1,
        desktop,
...
function normalizePromoBanners(payload: unknown): HomepagePromoBanner[] {
  ...
  return rawBanners
    ...
    .filter((banner): banner is HomepagePromoBanner => Boolean(banner));
}

async function fetchHomepageCategories(supabase: ServerClient) {
...
async function fetchHomepageProducts(supabase: ServerClient) {
...
async function loadHomepageData(): Promise<HomepageData> {
  ...
  const [heroBannersData, categoriesData, productsData, promoBannersData] = await Promise.all([
    supabase.from("settings").select("value").eq("key", "hero_banners").maybeSingle(),
    fetchHomepageCategories(supabase),
    fetchHomepageProducts(supabase),
    supabase.from("settings").select("value").eq("key", "promo_banners").maybeSingle(),
  ]);

  return {
    heroBanners: normalizeHeroSlides(heroBannersData.data?.value),
...
export const getHomepageData = unstable_cache(loadHomepageData, ["storefront-homepage-data"], {
  revalidate: 60,
});
