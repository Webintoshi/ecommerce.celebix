...
import { normalizeManagedAdminHostname } from "./hostname.ts";
...
export function createAdminDomainService(input:Readonly<{repository:AdminDomainPersistence;provider:CustomHostnameProvider;hostnamePolicy:StorefrontHostnamePolicy;generateId:()=>string}>):AdminDomainService{
  ...
  async function requestRecheck(selected:StoreDomainVersionedServiceInput):Promise<AdminDomainView>{
    const requested=await versioned("requestRecheck",selected);
    if(requested.kind!=="custom_alias"||requested.status!=="pending_verification"||requested.dnsInstructions.length>0)return requested;
    let snapshot;
    try{snapshot=await provider.find(requested.hostname);if(snapshot===null)snapshot=await provider.create(requested.hostname);}catch{throw failure("provider_unavailable");}
    if(snapshot.hostname!==requested.hostname)throw failure("provider_unavailable");
    try{return await repository.bindProvider({tenantContext:selected.tenantContext,now:selected.now,domainId:requested.id,expectedVersion:requested.version,providerHostnameId:snapshot.providerHostnameId,ownershipValidation:instructions(snapshot.ownershipValidation),certificateValidation:instructions(snapshot.certificateValidation.find((item)=>item.type!=="http")??null)});}catch(caught){return mapped(caught);}
  }
  return Object.freeze({
    async list(selected){try{return await repository.list(selected);}catch(caught){return mapped(caught);}},
    async create(selected){if(!selected||!UUID.test(selected.operationId))throw failure("invalid_input");let hostname;try{hostname=normalizeManagedAdminHostname(selected.hostname,hostnamePolicy);}catch{throw failure("invalid_input");}const domainId=generateId();if(!UUID.test(domainId))throw failure("provider_unavailable");let prepared;try{prepared=await repository.prepareCreate({tenantContext:selected.tenantContext,now:selected.now,operationId:selected.operationId,fingerprint:digest(hostname),domainId,hostname,provider:"cloudflare_for_saas",cnameTarget:hostnamePolicy.cnameTarget});}catch(caught){return mapped(caught);}if(prepared.replayed){const current=await repository.list({tenantContext:selected.tenantContext,now:selected.now});const durable=current.find((domain)=>domain.id===prepared.domain.id);if(durable&&durable.version>prepared.domain.version)return durable;}let snapshot;try{snapshot=await provider.create(hostname);}catch(caught){if(!(caught instanceof CloudflareCustomHostnameError)||!["duplicate","unavailable","rate_limited"].includes(caught.code))throw failure(caught instanceof CloudflareCustomHostnameError&&caught.code==="invalid_input"?"invalid_input":"provider_unavailable");try{snapshot=await provider.find(hostname);}catch{throw failure("provider_unavailable");}if(snapshot===null)throw failure("provider_unavailable");}if(snapshot.hostname!==hostname)throw failure("provider_unavailable");try{return await repository.bindProvider({tenantContext:selected.tenantContext,now:selected.now,domainId:prepared.domain.id,expectedVersion:prepared.domain.version,providerHostnameId:snapshot.providerHostnameId,ownershipValidation:instructions(snapshot.ownershipValidation),certificateValidation:instructions(snapshot.certificateValidation.find((item)=>item.type!=="http")??null)});}catch(caught){return mapped(caught);}},
    requestRecheck,makePrimary:(selected)=>versioned("makePrimary",selected),disable:(selected)=>versioned("disable",selected),
  });
}
