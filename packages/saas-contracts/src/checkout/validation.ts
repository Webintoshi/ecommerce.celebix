...
export function parseCheckoutQuote(value: unknown): CheckoutQuote {
  return guarded(() => {
    ...
    if ((shippingOptions.length === 0) !== (selectedShippingId === null) || (selectedShippingId !== null && !shippingOptions.some((option) => option.id === selectedShippingId)) || (selectedShippingId !== null && shippingCents !== shippingOptions.find((option) => option.id === selectedShippingId)!.priceCents)) invalid();
