...
export function extractGoogleTagManagerId(value: unknown) {
...
export function extractGoogleSearchConsoleVerification(value: unknown) {
  const raw = normalizeString(value);
  if (!raw) {
    return "";
  }
...
export function extractMetaPixelId(value: unknown) {
  const raw = normalizeString(value);
  if (!raw) {
    return "";
  }
