...
test("a numeric internal barcode claimed by another variant reports a clear conflict", async () => {
  const client = new FakeClient((text) => {
    if (text.includes("saas.catalog_create_variant")) {
      throw { code: "23505", constraint: "product_variants_store_numeric_internal_barcode_key" };
    }
    return [];
  });
  await assert.rejects(repository(new FakePool(client)).createVariant({
    tenantContext: tenantContext(), now: NOW, operationId: OPERATION_ID, productId: PRODUCT_ID,
    variant: { title: "Beyaz", barcode: "970000123", priceCents: 100, stockTracking: true, stockQuantity: 1, attributes: { renk: "Beyaz" } },
  }), (error: unknown) => error instanceof CatalogRepositoryError && error.code === "barcode_conflict");
});
