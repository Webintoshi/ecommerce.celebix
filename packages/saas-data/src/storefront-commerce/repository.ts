...
export type StorefrontCommerceErrorCode =
  (typeof STOREFRONT_COMMERCE_ERROR_CODES)[number];
...
function failure(
  code: StorefrontCommerceErrorCode = "unavailable",
): StorefrontCommerceRepositoryError {
  return new StorefrontCommerceRepositoryError(code);
}
...
function mapped(
  outcome: string,
): StorefrontCommerceRepositoryError | undefined {
  return ERROR_CODES.has(outcome)
    ? failure(outcome as StorefrontCommerceErrorCode)
    : undefined;
}
...
function exactResult(
  ...
  try {
    return exactCommerceInput(value, required);
  } catch {
    throw failure();
  }
}
...
function parseCheckoutQuoteProjection(value: unknown) {
  ...
  const selected = exactCommerceInput(
    value,
    ["cart", "paymentMethods"],
    ["estimatedDays"],
  );
  const cart = exactCommerceInput(selected.cart, [
    "version",
    "currency",
    "itemCount",
    "subtotalCents",
  ...
  return parsePublicCheckoutQuote({
    ...selected,
    cart: { ...cart, checkoutBlocker: null },
  });
}
...
export class PostgresStorefrontCommerceRepository implements StorefrontCommerceRepository {
  ...
  constructor(options: PostgresStorefrontCommerceRepositoryOptions) {
    const parsed = exactCommerceInput(options, [
      "pool",
      "role",
      "timeouts",
      "audit",
    ...
    if (
      parsed.role !== "celebix_saas_host_resolver" ||
      !parsed.pool ||
      typeof (parsed.pool as { connect?: unknown }).connect !== "function" ||
      typeof parsed.audit !== "function"
    )
      throw failure();
    const timeouts = exactCommerceInput(parsed.timeouts, [
      "poolCheckoutMs",
      "statementMs",
      "lockMs",
      "idleTransactionMs",
  ...
  private emitUnknown(): void {
    try {
      const pending = this.options.audit(
        Object.freeze({ type: "storefront_checkout_commit_unknown" }),
      );
  ...
  async recordCartAttribution(
    input: Parameters<StorefrontCommerceRepository["recordCartAttribution"]>[0],
  ): Promise<void> {
    try {
      ...
      await this.write(
        ...
        [
          ...
          JSON.stringify(commerceAttribution(parsed.attribution)),
        ],
        ["recorded"],
        (value) => {
          exactResult(value, []);
  ...
  async restoreCart(
    input: Parameters<StorefrontCommerceRepository["restoreCart"]>[0],
  ) {
    try {
      const parsed = exactCommerceInput(input, [
        "hostname",
      ...
      if (
        ...
        !/^[0-9a-f]{64}$/.test(parsed.tokenDigest)
      )
        throw failure("invalid_input");
      return await this.write(
        ...
        ["restored"],
        (value) => {
          const selected = exactResult(value, [
            "cart",
            "restoredItems",
            "omittedItems",
            "adjustedItems",
          ...
          if (
            !Number.isSafeInteger(selected.restoredItems) ||
            (selected.restoredItems as number) < 1 ||
            !Number.isSafeInteger(selected.omittedItems) ||
            (selected.omittedItems as number) < 0 ||
            !Number.isSafeInteger(selected.adjustedItems) ||
            (selected.adjustedItems as number) < 0
          )
            throw failure();
          return Object.freeze({
            cart: parsePublicCart(selected.cart),
            restoredItems: selected.restoredItems as number,
            omittedItems: selected.omittedItems as number,
            adjustedItems: selected.adjustedItems as number,
          });
        },
      );
    } catch (error) {
  ...
  async resolveCart(
    input: Parameters<StorefrontCommerceRepository["resolveCart"]>[0],
  ): Promise<PublicCart> {
    try {
      const parsed = exactCommerceInput(input, [
        "hostname",
  ...
  async mutateCart(
    input: Parameters<StorefrontCommerceRepository["mutateCart"]>[0],
  ) {
    try {
      ...
      const cart = Object.hasOwn(parsed, "cart")
        ? commerceGeneratedCredential(parsed.cart, now, 31)
        : undefined;
      if (
        (candidates.length === 0) !== Boolean(cart) ||
        (candidates.length === 0 &&
          (parsed.action !== "add" || parsed.expectedVersion !== 0))
      )
        throw failure("invalid_input");
      if (
        parsed.action !== "add" &&
      ...
      const quantity =
        parsed.action === "remove"
          ? undefined
          : commerceQuantity(parsed.quantity);
      if (parsed.action === "remove" && Object.hasOwn(parsed, "quantity"))
        throw failure("invalid_input");
      ...
      return await this.write(
        ...
        [
          ...
          cart?.id ?? commerceUuid("00000000-0000-4000-8000-000000000000"),
          cart?.keyId ?? null,
          cart?.digest ?? null,
          cart?.expiresAt ?? null,
          operationId,
        ...
        (value) => {
          ...
          if (
            typeof selected.credentialCreated !== "boolean" ||
            selected.credentialCreated !== (candidates.length === 0)
          )
            throw failure();
          return Object.freeze({
            credentialCreated: selected.credentialCreated,
            cart: parsePublicCart(selected.cart),
          });
        },
      );
  ...
  async createBuyNow(
    input: Parameters<StorefrontCommerceRepository["createBuyNow"]>[0],
  ): Promise<void> {
    try {
      const parsed = exactCommerceInput(
        input,
        ["hostname", "now", "intent", "productId", "variantId", "quantity"],
        ["attribution"],
      );
      const now = commerceDate(parsed.now);
      const intent = commerceGeneratedCredential(
        parsed.intent,
        now,
        15 / (24 * 60),
      );
      await this.write(
        Object.hasOwn(parsed, "attribution")
          ? "SELECT outcome,result_payload FROM saas.public_buy_now_create($1::text,$2::timestamptz,$3::uuid,$4::text,$5::text,$6::timestamptz,$7::uuid,$8::uuid,$9::integer,$10::jsonb)"
        ...
        [
          ...
          intent.digest,
          intent.expiresAt,
          commerceUuid(parsed.productId),
          commerceUuid(parsed.variantId),
          commerceQuantity(parsed.quantity),
          ...(Object.hasOwn(parsed, "attribution")
            ? [JSON.stringify(commerceAttribution(parsed.attribution))]
            : []),
        ...
        ["committed"],
        (value) => {
          const selected = exactResult(value, ["intentKind"]);
          if (selected.intentKind !== "buy_now") throw failure();
        },
      );
    } catch (error) {
      if (error instanceof StorefrontCommerceRepositoryError) throw error;
  ...
  async quote(input: Parameters<StorefrontCommerceRepository["quote"]>[0]) {
    try {
      const parsed = exactCommerceInput(
        input,
        ["hostname", "now", "intentKind", "candidates"],
        ["attribution"],
      );
      ...
      return await this.write(
        hasAttribution
          ? "SELECT outcome,result_payload FROM saas.public_checkout_quote($1::text,$2::timestamptz,$3::text,$4::jsonb,$5::jsonb)"
  ...
  async complete(
    input: Parameters<StorefrontCommerceRepository["complete"]>[0],
  ) {
  ...
  async getReceipt(
    input: Parameters<StorefrontCommerceRepository["getReceipt"]>[0],
  ) {
    try {
      const parsed = exactCommerceInput(input, [
        "hostname",
      ...
      return await this.read(
        ...
        [
          commerceHostname(parsed.hostname),
          commerceDate(parsed.now),
          JSON.stringify(commerceCandidates(parsed.receiptCandidates)),
  ...
  async listAccountOrders(
    input: Parameters<StorefrontCommerceRepository["listAccountOrders"]>[0],
  ) {
    try {
      const parsed = exactCommerceInput(input, [
        "hostname",
