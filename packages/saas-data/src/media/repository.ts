...
type Authority = Readonly<{ storeId: string; principalId: string; membershipId: string; role: StoreMembershipRole; planId: string; planCode: string; planVersion: number; storageBytes: number; now: Date }>;
...
function authority(context: TenantContext, now: Date): Authority {
  ...
  return Object.freeze({ storeId: uuid(context.store.id), principalId: uuid(context.principal.id), membershipId: uuid(context.membership.id), role: context.membership.role, planId: uuid(context.entitlements.planId), planCode: string(context.entitlements.planCode, 1, 64), planVersion: integer(context.entitlements.version, 1), storageBytes, now: new Date(now) });
}
function authorize(value: Authority, operation: CatalogProductOperation): void { if (!isCatalogProductOperationAllowed(value.role, operation)) throw failure("membership_denied"); }
