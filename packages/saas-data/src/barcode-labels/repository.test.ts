...
test("internal reservation uses the 13-digit function and parses its exact result", async () => {
  const queries: Array<{ text: string; values?: unknown[] }> = [];
  const pool: PostgresPoolLike = { connect: async () => ({
    async query(text: string, values?: unknown[]) {
      queries.push({ text, values });
      return text.startsWith("SELECT outcome")
        ? { rows: [{ outcome: "reserved", result_payload: { barcode: "9800000000007", replayed: false } }], rowCount: 1 } as never
        : { rows: [], rowCount: null } as never;
    },
    release() {},
  }) };
  ...
  assert.deepEqual(result, { barcode: "9800000000007", replayed: false });
  const domain = queries.find(({ text }) => text.includes("saas.barcode_label_reserve_ean13_internal("));
