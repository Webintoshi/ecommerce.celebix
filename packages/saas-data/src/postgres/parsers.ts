...
const FEATURES = new Set<PlanFeatureKey>(["catalog", "orders", "customers", "content", "media", "analytics", "checkout", "custom_domains", "staff_management", "promotions", "integrations", "accounting", "marketplaces"]);
...
export function parseCreateStarterTenantResult(value: unknown): CreateStarterTenantResult {
  ...
  if (root.schemaVersion !== 1 || typeof root.replayed !== "boolean" || root.provisioningStatus !== "ready") corrupt();
  ...
  uuid(plan.planId); timestamp(plan.validFrom); if (plan.validUntil !== undefined) timestamp(plan.validUntil);
  if (!Array.isArray(plan.features) || plan.features.length === 0) corrupt();
  const seen = new Set<string>();
  for (const feature of plan.features) { if (typeof feature !== "string" || !FEATURES.has(feature as PlanFeatureKey) || seen.has(feature)) corrupt(); seen.add(feature); }
  const limits = exact(plan.limits, LIMIT_KEYS);
  for (const key of LIMIT_KEYS) integer(limits[key]);
  url(root.panelUrl); const storefrontUrl = url(root.storefrontUrl);
  if (storefrontUrl !== `https://${hostname}`) corrupt();
  return structuredClone(root) as unknown as CreateStarterTenantResult;
}

export function parseTenantOperationRow(value: unknown): TenantOperationRecord {
  ...
  const result = row.result_payload === null ? undefined : parseCreateStarterTenantResult(row.result_payload);
