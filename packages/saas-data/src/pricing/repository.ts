...
import { canonicalPricingItems, canonicalPricingRules, equalPricingProjection, pricingFingerprint } from "./canonical.ts";
...
function row(result: Readonly<{ rows: unknown[]; rowCount?: number | null }>) {
  try { if (result.rowCount !== 1 || !Array.isArray(result.rows) || result.rows.length !== 1) throw unavailable(); const raw = result.rows[0]; if (typeof raw !== "object" || raw === null || Array.isArray(raw) || Object.keys(raw).sort().join(",") !== "outcome,result_payload") throw unavailable(); const outcome = (raw as Record<string, unknown>).outcome; if (typeof outcome !== "string" || outcome.length > 64) throw unavailable(); return { outcome, result: (raw as Record<string, unknown>).result_payload }; } catch { throw unavailable(); }
}
