import type { AnalyticsConnectionMutationResult, AnalyticsConnectionStatus, AnalyticsConnectionView, AnalyticsDashboard, AnalyticsPeriod, CommerceAnalyticsSnapshot, TenantContext } from "@celebix/saas-contracts";
...
export type AnalyticsLifecycleEventName = "payment_failed"|"refund"|"order_cancelled"|"cart_abandoned"|"cart_resumed"|"cart_recovered"|"recovery_message_queued"|"recovery_message_sent"|"recovery_message_failed";
export type AnalyticsOutboxPayload = Readonly<{name:"purchase";valueCents:number;currency:string;source:AnalyticsPurchaseSource}>
  | Readonly<{name:AnalyticsLifecycleEventName;schemaVersion:1;currency:string;valueMinor:number}>;
export type AnalyticsOutboxClaim = Readonly<{ eventId:string; leaseToken:string; websiteId:string; hostname:string; attemptCount:number; payload:AnalyticsOutboxPayload }>;
...
export interface AnalyticsRepository {
  ...
  commerceSnapshot(input:Readonly<{tenantContext:TenantContext;now:Date;rangeStart:Date;rangeEnd:Date}>):Promise<Readonly<CommerceAnalyticsSnapshot>>;
  updateCommerceSettings(input:Readonly<{tenantContext:TenantContext;now:Date;expectedVersion:number;candidateInactivityMinutes:number;abandonedInactivityHours:number;recoveryLinkHours:number;automaticRecoveryEnabled:boolean;maximumMessageAttempts:number;minimumMessageIntervalHours:number;trackingPolicy:"disabled"|"anonymous_commerce"}>):Promise<Readonly<CommerceAnalyticsSnapshot["settings"]>>;
