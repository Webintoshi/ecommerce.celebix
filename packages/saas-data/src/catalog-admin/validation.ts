...
export function catalogAdminImportRows(value: unknown): readonly CatalogAdminImportRow[] { if (!Array.isArray(value) || value.length < 1 || value.length > 100) fail(); return Object.freeze(value.map((entry) => { const row = exactCatalogAdminInput(entry, ["title", "slug", "priceCents", "stockQuantity"], ["sku"]); if (!Number.isSafeInteger(row.priceCents) || (row.priceCents as number) < 0 || !Number.isSafeInteger(row.stockQuantity) || (row.stockQuantity as number) < 0) fail(); const sku = row.sku === undefined ? undefined : catalogAdminText(row.sku, 1, 64, SKU); return Object.freeze({ title: catalogAdminText(row.title, 1, 200), slug: catalogAdminSlug(row.slug), priceCents: row.priceCents as number, ...(sku === undefined ? {} : { sku }), stockQuantity: row.stockQuantity as number }); })); }
