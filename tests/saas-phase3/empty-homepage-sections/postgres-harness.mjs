...
const UP = "202608090098_empty_homepage_sections.up.sql";
const DOWN = "202608090098_empty_homepage_sections.down.sql";
const ASSERTIONS = "202608090098_empty_homepage_sections_assertions.sql";
const DB = `empty_homepage_${randomBytes(5).toString("hex")}`;
...
function start() {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "cx-empty-home-"));
...
function main() {
  ...
  try {
    ...
    scenario("PostgreSQL 16 applies migration 098", () => assert.match(psql(box, "SHOW server_version;").stdout, /^16[.]/));
    scenario("v1 and v2 compositions accept an intentionally empty section list", () => {
      assert.equal(psql(box, "SELECT saas.campaign_starter_composition_valid(pg_catalog.jsonb_set(saas.storefront_theme_default_composition(),ARRAY['sections'],'[]'::jsonb,false));").stdout.trim(), "t");
      assert.equal(psql(box, `SELECT saas.campaign_starter_composition_valid('{"schemaVersion":1,"visual":{"colorScheme":"neutral","headingStyle":"serif","cornerStyle":"soft","headerStyle":"overlay","productCardStyle":"editorial","productImageRatio":"portrait"},"announcement":{"enabled":false,"items":[]},"navigation":{"rootCategoryIds":[]},"sections":[],"productDetail":{"galleryStyle":"grid","showSku":true,"showBrand":true,"showRelatedProducts":true,"mobileStickyPurchase":true},"cart":{"showCheckoutReadiness":true,"showShippingProgress":true}}'::jsonb);`).stdout.trim(), "t");
    });
    scenario("malformed, unknown and oversized section authority remains denied", () => {
      assert.equal(psql(box, "SELECT saas.campaign_starter_composition_valid(pg_catalog.jsonb_set(saas.storefront_theme_default_composition(),ARRAY['sections'],'{}'::jsonb,false));").stdout.trim(), "f");
      assert.equal(psql(box, "SELECT saas.campaign_starter_composition_valid(pg_catalog.jsonb_set(saas.storefront_theme_default_composition(),ARRAY['sections'],(SELECT pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object('kind','product_row','enabled',true,'heading','Ürünler','source','latest','limit',4)) FROM pg_catalog.generate_series(1,13)),false));").stdout.trim(), "f");
      assert.equal(psql(box, "SELECT saas.campaign_starter_composition_valid(pg_catalog.jsonb_set(saas.storefront_theme_default_composition(),ARRAY['sections'],'[]'::jsonb,false)||'{\"unexpected\":true}'::jsonb);").stdout.trim(), "f");
    });
    ...
    const documentSql = `$empty_homepage$${emptyDocument}$empty_homepage$::jsonb`;
    const saved = rpc(box, `saas.storefront_design_save_draft(${authority()},'70000000-0000-4000-8000-000000000098','${fingerprint("empty-draft")}',1,${documentSql})`);
    scenario("authenticated owner saves the empty homepage draft", () => { assert.equal(saved.outcome, "saved"); assert.equal(saved.result.draftVersion, 2); });
    const published = rpc(box, `saas.storefront_design_publish(${authority()},'80000000-0000-4000-8000-000000000098','${fingerprint("empty-publish")}',2,1)`);
    scenario("authenticated owner publishes the empty homepage", () => {
      assert.equal(published.outcome, "published");
      assert.equal(psql(box, `SELECT pg_catalog.jsonb_array_length(published_config->'composition'->'sections') FROM saas.storefront_designs WHERE store_id='${STORE}';`).stdout.trim(), "0");
    });
    scenario("public presentation and hostname resolver preserve exact empty sections", () => {
    ...
    scenario("all durable constraints accept empty but reject malformed sections", () => {
      psql(box, `BEGIN;SET LOCAL ROLE celebix_saas_owner;
        UPDATE saas.merchant_admin_records SET config=pg_catalog.jsonb_set(config,ARRAY['sections'],'[]'::jsonb,false) WHERE id='${RECORD}';
        UPDATE saas.campaign_starter_publications SET config=pg_catalog.jsonb_set(config,ARRAY['sections'],'[]'::jsonb,false) WHERE store_id='${STORE}';COMMIT;`);
      assert.notEqual(psql(box, `BEGIN;SET LOCAL ROLE celebix_saas_owner;UPDATE saas.campaign_starter_publications SET config=pg_catalog.jsonb_set(config,ARRAY['sections'],'{}'::jsonb,false) WHERE store_id='${STORE}';COMMIT;`, DB, true).status, 0);
    });
    scenario("runtime roles receive no new helper authority", () => assert.equal(psql(box, "SELECT has_function_privilege('celebix_saas_app','saas.campaign_starter_composition_valid_without_empty_homepage(jsonb)','EXECUTE') OR has_table_privilege('celebix_saas_app','saas.storefront_designs','UPDATE');").stdout.trim(), "f"));
    scenario("backup and restore retain the empty published homepage", () => {
      const archive = path.join(box.root, "empty-homepage.dump");
      ...
      assert.equal(psql(box, `SELECT pg_catalog.jsonb_array_length(published_config->'composition'->'sections') FROM saas.storefront_designs WHERE store_id='${STORE}';`, RESTORE_DB).stdout.trim(), "0");
    });
    scenario("unguarded and data-loss rollback attempts are rejected", () => {
      assert.notEqual(psql(box, readFileSync(path.join(SQL, DOWN), "utf8"), DB, true).status, 0);
      assert.notEqual(psql(box, `SET celebix.allow_empty_homepage_sections_down='on';\n${readFileSync(path.join(SQL, DOWN), "utf8")}`, DB, true).status, 0);
    });
    scenario("non-empty rollback and reapply restore the exact authority", () => {
      psql(box, `BEGIN;SET LOCAL ROLE celebix_saas_owner;
        UPDATE saas.merchant_admin_records SET config=saas.storefront_theme_default_composition() WHERE id='${RECORD}';
        UPDATE saas.campaign_starter_publications SET config=saas.storefront_theme_default_composition() WHERE store_id='${STORE}';
        UPDATE saas.storefront_designs SET draft_config=pg_catalog.jsonb_set(draft_config,ARRAY['composition'],saas.storefront_theme_default_composition(),false),published_config=pg_catalog.jsonb_set(published_config,ARRAY['composition'],saas.storefront_theme_default_composition(),false) WHERE store_id='${STORE}';
        COMMIT;`);
      apply(box, DOWN, DB, "SET celebix.allow_empty_homepage_sections_down='on';\n");
      assert.equal(psql(box, "SELECT saas.campaign_starter_composition_valid(pg_catalog.jsonb_set(saas.storefront_theme_default_composition(),ARRAY['sections'],'[]'::jsonb,false));").stdout.trim(), "f");
      apply(box, UP);
      apply(box, ASSERTIONS);
      assert.equal(psql(box, "SELECT saas.campaign_starter_composition_valid(pg_catalog.jsonb_set(saas.storefront_theme_default_composition(),ARRAY['sections'],'[]'::jsonb,false));").stdout.trim(), "t");
