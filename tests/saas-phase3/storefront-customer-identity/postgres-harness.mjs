...
async function main() {
  let box;
  try {
    for (const file of [UP, DOWN, ASSERTIONS, MAGIC_UP, MAGIC_DOWN, MAGIC_ASSERTIONS]) assert.equal(existsSync(path.join(SQL, file)), true, file);
    box = start();
    ...
    await scenario("the same email creates an independent account through a magic ticket in another store", () => { const challenge = "40000000-0000-4000-8000-000000000084", codeDigest = "1".repeat(64), ticketDigest = "2".repeat(64); assert.equal(authStart(box, HOST_B, challenge, "2".repeat(64), codeDigest, "084", ticketDigest).outcome, "accepted"); const result = publicCall(box, verifySql({ hostname: HOST_B, challenge, emailDigest: "2".repeat(64), verifierKind: "ticket", verifierDigest: ticketDigest, account: "50000000-0000-4000-8000-000000000086", session: "60000000-0000-4000-8000-000000000086", sessionDigest: "3".repeat(64), correlation: "verify_00086" })); assert.equal(result.outcome, "profile_required"); assert.equal(publicCall(box, verifySql({ hostname: HOST_B, challenge, emailDigest: "2".repeat(64), verifierKind: "code", verifierDigest: codeDigest, account: "50000000-0000-4000-8000-000000000087", session: "60000000-0000-4000-8000-000000000087", sessionDigest: "7".repeat(64), correlation: "verify_00087" })).outcome, "challenge_invalid"); assert.equal(psql(box, "SELECT count(DISTINCT store_id) FROM saas.storefront_accounts WHERE email_normalized='ada@example.test';").stdout.trim(), "2"); });
    ...
    await scenario("rollback refuses active magic-link authority", () => { assert.equal(authStart(box, HOST_B, "40000000-0000-4000-8000-000000000088", "3".repeat(64), "4".repeat(64), "088", "5".repeat(64)).outcome, "accepted"); psql(box, "BEGIN;SET LOCAL ROLE celebix_saas_owner;UPDATE saas.storefront_login_challenges SET created_at=pg_catalog.clock_timestamp(),last_sent_at=pg_catalog.clock_timestamp(),expires_at=pg_catalog.clock_timestamp()+INTERVAL '10 minutes' WHERE id='40000000-0000-4000-8000-000000000088';COMMIT;"); psql(box, `ALTER DATABASE ${DB} SET celebix.allow_storefront_magic_link_auth_down='on';`, "postgres"); const result = psql(box, readFileSync(path.join(SQL, MAGIC_DOWN), "utf8"), DB, true); assert.notEqual(result.status, 0); assert.match(result.stderr, /STOREFRONT_MAGIC_LINK_AUTH_DOWN_BLOCKED/u); });
    await scenario("guarded rollback and reapply preserve guest commerce in an empty database", () => { psql(box, `ALTER DATABASE ${EMPTY} SET celebix.allow_storefront_magic_link_auth_down='on';ALTER DATABASE ${EMPTY} SET celebix.allow_storefront_customer_identity_down='on';`, "postgres"); apply(box, MAGIC_DOWN, EMPTY); apply(box, DOWN, EMPTY); assert.equal(psql(box, "SELECT to_regprocedure('saas.public_cart_resolve(text,timestamp with time zone,jsonb)') IS NOT NULL;", EMPTY).stdout.trim(), "t"); apply(box, UP, EMPTY); apply(box, ASSERTIONS, EMPTY); apply(box, MAGIC_UP, EMPTY); apply(box, MAGIC_ASSERTIONS, EMPTY); });
