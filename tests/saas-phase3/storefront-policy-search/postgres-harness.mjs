...
INSERT INTO saas.products(id,store_id,slug,title,description,status,currency,version,archived_at,created_at,updated_at) VALUES('${PRODUCT_A}','${STORE_A}','altin-yuzuk','Altın Yüzük','Özel tasarım','active','TRY',1,NULL,'2026-01-02','2026-01-02'),('${PRODUCT_A2}','${STORE_A}','gumus-kolye','Gümüş Kolye',NULL,'active','TRY',1,NULL,'2026-01-01','2026-01-01');
...
INSERT INTO saas.product_variants(id,product_id,store_id,title,sku,price_cents,stock_tracking,stock_quantity,status,attributes,version,created_at,updated_at) VALUES('${VARIANT_A}','${PRODUCT_A}','${STORE_A}','14 Ayar','YZK-1090',1127100,true,3,'active','{}',1,'2026-01-02','2026-01-02'),('${VARIANT_A2}','${PRODUCT_A2}','${STORE_A}','Standart','KLK-100',200000,false,0,'active','{}',1,'2026-01-01','2026-01-01');
...
async function main() {
  ...
  try {
    ...
    await scenario("search returns only matching same-store active products", () => { const result = publicCall(box, `saas.public_search_products('${HOST_A}','${NOW}','altın',48,NULL)`); assert.deepEqual(result.result_payload.items.map(({ slug }) => slug), ["altin-yuzuk"]); assert.doesNotMatch(JSON.stringify(result), /costCents|storeId|objectKey|OTHER-1/); });
    ...
    await scenario("search cursor is deterministic and bounded", () => { const first = publicCall(box, `saas.public_search_products('${HOST_A}','${NOW}','',1,NULL)`).result_payload; assert.equal(first.items.length, 1); assert.equal(typeof first.nextCursor, "string"); const second = publicCall(box, `saas.public_search_products('${HOST_A}','${NOW}','',1,'${first.nextCursor}')`).result_payload; assert.equal(second.items.length, 1); assert.notEqual(first.items[0].id, second.items[0].id); assert.equal(publicCall(box, `saas.public_search_products('${HOST_A}','${NOW}','',49,NULL)`).outcome, "invalid_input"); });
