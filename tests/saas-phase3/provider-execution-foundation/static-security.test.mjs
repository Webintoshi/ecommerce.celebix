...
const PROFILE_UP_FILE = "202607250049_merchant_provider_profiles.up.sql";
const PROFILE_DOWN_FILE = "202607250049_merchant_provider_profiles.down.sql";
const PROFILE_ASSERTIONS_FILE = "202607250049_merchant_provider_profiles_assertions.sql";
const EXECUTION_UP_FILE = "202607250050_merchant_provider_execution.up.sql";
const EXECUTION_DOWN_FILE = "202607250050_merchant_provider_execution.down.sql";
const EXECUTION_ASSERTIONS_FILE = "202607250050_merchant_provider_execution_assertions.sql";
const MANIFEST_FILE = "phase3i-provider-execution-foundation-manifest.json";
const up = readFileSync(path.join(SQL, PROFILE_UP_FILE), "utf8");
const down = readFileSync(path.join(SQL, PROFILE_DOWN_FILE), "utf8");
const assertions = readFileSync(path.join(SQL, PROFILE_ASSERTIONS_FILE), "utf8");
const executionUp = readFileSync(path.join(SQL, EXECUTION_UP_FILE), "utf8");
const executionDown = readFileSync(path.join(SQL, EXECUTION_DOWN_FILE), "utf8");
const executionAssertions = readFileSync(path.join(SQL, EXECUTION_ASSERTIONS_FILE), "utf8");
const manifest = JSON.parse(readFileSync(path.join(SQL, MANIFEST_FILE), "utf8"));

test("provider foundation manifest pins the exact 001 through 050 chain", () => {
  ...
  assert.deepEqual(manifest.migrationChain.slice(-4).map(({ file }) => file), [
    PROFILE_UP_FILE, PROFILE_ASSERTIONS_FILE, EXECUTION_UP_FILE, EXECUTION_ASSERTIONS_FILE,
  ]);
  assert.deepEqual(manifest.rollbackArtifacts.map(({ file }) => file), [PROFILE_DOWN_FILE, EXECUTION_DOWN_FILE]);
...
test("validation claiming is atomic bounded and never performs provider network work", () => {
  ...
  assert.doesNotMatch(`${up}\n${down}\n${assertions}\n${executionUp}\n${executionDown}\n${executionAssertions}`, /\b(fetch|axios|curl|http_request|dblink|postgres_fdw)\b/i);
});

test("execution lifecycle uses exact leases, disjoint roles, and no direct table grants", () => {
  assert.match(executionUp, /FOR UPDATE OF candidate SKIP LOCKED LIMIT 1/);
  assert.match(executionUp, /p_lease_expires_at>p_now\+interval '15 minutes'/);
  assert.match(executionUp, /job\.lease_id<>p_lease_id/);
  assert.match(executionUp, /merchant_provider_reconcile/);
  assert.match(executionUp, /candidate\.attempt_count<5/);
  assert.match(executionUp, /status NOT IN\('provider_outcome_unknown','reconciliation_required'\)/);
  assert.doesNotMatch(executionUp, /GRANT\s+(?:SELECT|INSERT|UPDATE|DELETE)[\s\S]*merchant_provider_(?:jobs|workflow_operations)/i);
  assert.match(executionAssertions, /has_table_privilege\('celebix_saas_app','saas\.merchant_provider_jobs','SELECT,INSERT,UPDATE,DELETE'\)/);
  const grants = [...executionUp.matchAll(/GRANT EXECUTE ON FUNCTION[\s\S]*?TO (celebix_saas_app|celebix_saas_workflow);/g)];
  const appGrant = grants.find((match) => match[1] === "celebix_saas_app")?.[0] ?? "";
  const workflowGrant = grants.find((match) => match[1] === "celebix_saas_workflow")?.[0] ?? "";
  assert.doesNotMatch(appGrant, /merchant_provider_(?:claim|heartbeat|finalize|reconcile)/);
  assert.doesNotMatch(workflowGrant, /merchant_provider_queue/);
});

test("rollbacks are guarded and remove only their versioned authority surfaces", () => {
  ...
  assert.match(executionDown, /MERCHANT_PROVIDER_EXECUTION_ROLLBACK_REQUIRES_DRAIN/);
  assert.match(executionDown, /DROP TABLE saas\.merchant_provider_workflow_operations;/);
  assert.doesNotMatch(executionDown, /DROP TABLE saas\.(?:stores|memberships|subscriptions|merchant_provider_jobs|merchant_provider_profiles)/);
});
