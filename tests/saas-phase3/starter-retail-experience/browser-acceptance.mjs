...
export const hiddenQuantityPresentation={...presentation,cart:{...presentation.cart,showQuantitySelector:false}};
export const hiddenQuantityStorefront={...storefront,presentation:hiddenQuantityPresentation};
export const design={schemaVersion:2,publicationVersion:1,publishedAt:"2026-08-01T00:00:00.000Z",brand:{logo:null,favicon:null,primaryColor:"#171717",accentColor:"#171717",backgroundColor:"#f8f8f6",textColor:"#171717",fontFamily:"inter"},typography:{headingFont:{family:"Inter",category:"sans-serif",availableWeights:["400","500","600","700","800"],source:"google"},bodyFont:{family:"Inter",category:"sans-serif",availableWeights:["400","500","600","700","800"],source:"google"},headingWeight:"700",bodyWeight:"400",headingSizePx:40,bodySizePx:16},hero:{enabled:false,slides:[]},promotion:{headline:"",body:"",destination:null,startsAt:null,endsAt:null,enabled:false},announcement:{items:[],icon:"none",speed:"normal",direction:"left",animation:"continuous",enabled:false}};
export function FixturePage({path}:{path:string}){if(path==="/")return <CampaignHome storefront={storefront as any} design={design as any} projection={{presentation,productRows:[{key:"featured",items:products}]} as any}/>;if(path==="/quantity-hidden")return <StorefrontFrame storefront={hiddenQuantityStorefront as any} design={design as any}><ProductDetailExperience product={products[0] as any} relatedProducts={products.slice(1) as any} publishedPolicies={[]} options={hiddenQuantityPresentation.productDetail as any} cardStyle="editorial" imageRatio="portrait" showQuantitySelector={false}/></StorefrontFrame>;if(path==="/products")return <StorefrontFrame storefront={storefront as any} design={design as any}><section className="store-section store-container"><h1 className="sr-only">Ürünler</h1><ProductExplorer products={products as any} cardStyle="editorial" imageRatio="portrait"/></section></StorefrontFrame>;if(path.startsWith("/products/"))return <StorefrontFrame storefront={storefront as any} design={design as any}><ProductDetailExperience product={products[0] as any} relatedProducts={products.slice(1) as any} publishedPolicies={[]} options={presentation.productDetail as any} cardStyle="editorial" imageRatio="portrait" showQuantitySelector={true}/></StorefrontFrame>;if(path==="/search")return <StorefrontFrame storefront={storefront as any} design={design as any}><section className="store-container store-page"><header className="page-heading"><span>ARAMA</span><h1>Arama sonucu</h1></header><ProductGrid products={[]} cardStyle="editorial" imageRatio="portrait" emptyMessage="Aramanızla eşleşen ürün bulunamadı."/></section></StorefrontFrame>;return <StorefrontFrame storefront={storefront as any} design={design as any}><section className="store-container store-page"><h1>Sayfa</h1></section></StorefrontFrame>}
`);
  ...
  writeFileSync(path.join(api, "route.ts"), `let version=1,quantity=1;\nconst line=${JSON.stringify(line)};\nconst project=()=>({version,currency:"TRY",itemCount:quantity,subtotalCents:1289000*quantity,shippingCents:0,totalCents:1289000*quantity,checkoutReady:quantity>0,checkoutBlocker:quantity>0?null:"empty_cart",items:quantity>0?[{...line,quantity,lineTotalCents:1289000*quantity}]:[]});\nexport async function POST(request:Request,{params}:{params:Promise<{action:string}>}){const {action}=await params;const body=await request.json();if(action==="add"||action==="quantity")quantity=body.quantity;if(action==="remove")quantity=0;version+=1;return Response.json({cart:project()},{headers:{"set-cookie":"fixture_cart=present; Path=/; SameSite=Lax"}})}\n`);
...
async function assertListingToolbar(cdp, label) {
  const result = await cdp.evaluate(`(() => {
    ...
    const active=document.querySelector('.explorer-filters button[aria-pressed="true"]');
    const select=document.querySelector('.explorer-order select');
    if(!(toolbar instanceof HTMLElement)||!(header instanceof HTMLElement)||!(input instanceof HTMLInputElement)||!(active instanceof HTMLButtonElement)||!(select instanceof HTMLSelectElement))return null;
    const toolbarRect=toolbar.getBoundingClientRect(),headerRect=header.getBoundingClientRect();
    const inputStyle=getComputedStyle(input),activeStyle=getComputedStyle(active),selectStyle=getComputedStyle(select);
    return {
      headerGap:Math.round(toolbarRect.top-headerRect.bottom),
      ...
      activeBorders:[activeStyle.borderTopWidth,activeStyle.borderRightWidth,activeStyle.borderBottomWidth,activeStyle.borderLeftWidth],
      activeBackground:activeStyle.backgroundColor,
      selectBorders:[selectStyle.borderTopWidth,selectStyle.borderRightWidth,selectStyle.borderBottomWidth,selectStyle.borderLeftWidth],
    };
  })()`);
  assert.ok(result, `${label}:toolbar-required`);
  assert.ok(result.headerGap >= 20 && result.headerGap <= 72, `${label}:headerGap:${result.headerGap}`);
  assert.deepEqual(result.inputBorders, ["0px", "0px", "1px", "0px"], `${label}:flat-search`);
  assert.deepEqual(result.activeBorders, ["0px", "0px", "1px", "0px"], `${label}:flat-active-filter`);
  assert.ok(["rgba(0, 0, 0, 0)", "transparent"].includes(result.activeBackground), `${label}:active-filter-background:${result.activeBackground}`);
  assert.deepEqual(result.selectBorders, ["0px", "0px", "1px", "0px"], `${label}:flat-sort`);
}
...
async function main() {
  ...
  let quantityMutationRequests = 0;
  try {
    ...
    cdp.on("Network.requestWillBeSent", ({ request }) => { if (/__Host-celebix_|(?:c1|i1|u1|r1)[.]current/u.test(request.url)) sensitiveSurface.networkUrl = true; if (new URL(request.url).pathname === "/api/cart/quantity") quantityMutationRequests += 1; });
    ...
    await viewport(cdp, 390, 844); screenshots.push(await screenshot(cdp, "side-cart-390x844.png")); measurements.push(await measure(cdp, "side-cart-390x844")); await key(cdp, "Escape"); await waitFor(cdp, "!document.querySelector('.side-cart-dialog')&&getComputedStyle(document.body).overflow!=='hidden'&&document.activeElement?.textContent?.includes('Sepete ekle')", "side-cart-escape-focus-restored");
    await click(cdp, "[...document.querySelectorAll('button')].find((node)=>node.getAttribute('aria-label')==='Sepet')", "side-cart-open-for-close-button"); await waitFor(cdp, "document.querySelector('.side-cart-dialog')", "side-cart-reopened"); await click(cdp, "document.querySelector('button[aria-label=\"Sepeti kapat\"]')", "side-cart-close-button"); await waitFor(cdp, "!document.querySelector('.side-cart-dialog')&&document.activeElement?.getAttribute('aria-label')==='Sepet'", "side-cart-close-button-focus-restored");
    await click(cdp, "[...document.querySelectorAll('button')].find((node)=>node.getAttribute('aria-label')==='Sepet')", "side-cart-open-for-backdrop"); await waitFor(cdp, "document.querySelector('.side-cart-dialog')", "side-cart-backdrop-open"); assert.equal(await cdp.evaluate("(() => { const node=document.querySelector('.side-cart-backdrop'); if(!(node instanceof HTMLElement))return false; node.dispatchEvent(new MouseEvent('mousedown',{bubbles:true})); return true; })()"), true, "side-cart-backdrop-required"); await waitFor(cdp, "!document.querySelector('.side-cart-dialog')&&document.activeElement?.getAttribute('aria-label')==='Sepet'", "side-cart-backdrop-focus-restored");
    await navigate(cdp, "/quantity-hidden"); await waitFor(cdp, "document.querySelector('.purchase-panel')&&!document.querySelector('.loading-page')", "quantity-hidden-product"); assert.equal(await cdp.evaluate("document.querySelectorAll('.purchase-quantity').length"), 0, "hidden_purchase_quantity_selector_must_not_render"); if (await cdp.evaluate(`Boolean(${variant})`)) await click(cdp, variant, "quantity-hidden-variant"); await click(cdp, "[...document.querySelectorAll('.purchase-actions button')].find((node)=>node.textContent.includes('Sepete ekle'))", "quantity-hidden-add"); await waitFor(cdp, "document.querySelector('.side-cart-quantity-copy')", "quantity-hidden-copy"); assert.equal(await cdp.evaluate("document.querySelectorAll('.side-cart-quantity').length"), 0, "hidden_quantity_selector_must_not_render"); assert.equal(await cdp.evaluate("document.querySelector('.side-cart-quantity-copy')?.textContent"), "1 adet", "hidden_quantity_copy_required"); screenshots.push(await screenshot(cdp, "side-cart-quantity-hidden-390x844.png")); measurements.push(await measure(cdp, "side-cart-quantity-hidden-390x844")); await key(cdp, "Escape"); await waitFor(cdp, "!document.querySelector('.side-cart-dialog')", "quantity-hidden-closed");
    ...
    const result = Object.freeze({ applicationMode: BUILT_MODE ? "built" : "fixture", baseOrigin: "loopback", viewportMatrix: VIEWPORTS.map(([width, height]) => ({ width, height })), measurements, quantityMutationRequests, quantitySelectorEnabled: true, quantitySelectorDisabled: true, closePaths: ["Escape", "close-button", "backdrop"], newsletterAfterCart: true, reducedMotionDuration, screenshotCount: screenshots.length, screenshots: screenshots.map((file) => path.relative(ROOT, file)), consoleErrors: 0, networkFailures: 0, sensitiveSurface });
