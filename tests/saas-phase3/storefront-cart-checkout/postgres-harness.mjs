...
function completeSql({ operation = "71000000-0000-4000-8000-000000000081", fingerprint = "d".repeat(64), kind = "cart", credentials = candidates(), version = 3, payment = "bank_transfer", order = "72000000-0000-4000-8000-000000000081", customer = "73000000-0000-4000-8000-000000000081", address = "74000000-0000-4000-8000-000000000081", event = "75000000-0000-4000-8000-000000000081", receipt = "76000000-0000-4000-8000-000000000081", receiptDigest = "e".repeat(64), customerCredential = "77000000-0000-4000-8000-000000000081", customerDigest = "f".repeat(64), hostname = HOST } = {}) {
  return `saas.public_checkout_complete('${hostname}','${NOW}','${kind}','${credentials}'::jsonb,'${operation}','${fingerprint}',${version},'${DELIVERY}'::jsonb,'${payment}','${order}','${customer}','${address}','${event}','${receipt}','receipt-key-01','${receiptDigest}','2026-08-01T12:00:00Z','${customerCredential}','customer-key-01','${customerDigest}','2026-08-30T12:00:00Z')`;
}
...
async function main() {
  ...
  try {
    ...
    await scenario("new cart add creates a bound credential and trusted projection", () => { const result = mutate(box); assert.equal(result.outcome, "committed"); assert.equal(result.result_payload.credentialCreated, true); assert.equal(result.result_payload.cart.version, 1); assert.equal(result.result_payload.cart.itemCount, 2); assert.equal(result.result_payload.cart.totalCents, 2264100); assert.doesNotMatch(JSON.stringify(result), /storeId|credentialDigest|operationId/); });
    ...
    await scenario("missing payment and shipping configuration fail closed", () => { psql(box, `SET ROLE celebix_saas_owner;UPDATE saas.payment_methods SET state='disabled' WHERE store_id='${STORE}';`); assert.equal(quote(box).outcome, "payment_unavailable"); psql(box, `SET ROLE celebix_saas_owner;UPDATE saas.payment_methods SET state='active' WHERE store_id='${STORE}';UPDATE saas.merchant_admin_records SET status='draft' WHERE store_id='${STORE}' AND record_kind='shipping_setting';`); assert.equal(quote(box).outcome, "shipping_unavailable"); psql(box, `SET ROLE celebix_saas_owner;UPDATE saas.merchant_admin_records SET status='active' WHERE store_id='${STORE}' AND record_kind='shipping_setting';`); });
    await scenario("price drift is detected before checkout writes", () => { psql(box, `SET ROLE celebix_saas_owner;UPDATE saas.product_variants SET price_cents=1200000,version=version+1,updated_at='${LATER}' WHERE id='${VARIANT}';`); assert.equal(complete(box).outcome, "price_changed"); assert.equal(psql(box, `SELECT count(*) FROM saas.orders WHERE store_id='${STORE}';`).stdout.trim(), "0"); psql(box, `SET ROLE celebix_saas_owner;UPDATE saas.product_variants SET price_cents=1127100,version=version+1,updated_at='${NOW}' WHERE id='${VARIANT}';`); });
