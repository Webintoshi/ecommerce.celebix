...
const READINESS_UP = "202608010073_storefront_checkout_readiness.up.sql";
const READINESS_DOWN = "202608010073_storefront_checkout_readiness.down.sql";
const READINESS_ASSERTIONS = "202608010073_storefront_checkout_readiness_assertions.sql";
const READINESS_MANIFEST = "phase4c-storefront-checkout-readiness-manifest.json";
...
function pgCatalogFunctionBody(box, signature) { return psql(box, `SELECT pg_catalog.pg_get_functiondef('${signature}'::regprocedure);`).stdout; }
function migrations() {
...
async function main() {
  let box;
  try {
    for (const file of [UP, DOWN, ASSERTIONS, MANIFEST, READINESS_UP, READINESS_DOWN, READINESS_ASSERTIONS, READINESS_MANIFEST]) assert.equal(existsSync(path.join(SQL, file)), true, file);
    box = start();
    psql(box, `CREATE DATABASE ${DB};`, "postgres");
    for (const file of migrations()) apply(box, file);
    installLegacyCheckoutAuthority(box);
    seed(box);
    apply(box, UP); apply(box, ASSERTIONS); apply(box, READINESS_UP); apply(box, READINESS_ASSERTIONS);
    ...
    await scenario("manifest pins migration 073 artifacts", () => { const manifest = JSON.parse(readFileSync(path.join(SQL, READINESS_MANIFEST), "utf8")); assert.equal(manifest.postgresqlMajor, 16); for (const artifact of [...manifest.artifacts, ...manifest.rollbackArtifacts]) assert.equal(createHash("sha256").update(readFileSync(path.join(SQL, artifact.file))).digest("hex"), artifact.sha256, artifact.file); });
    ...
    await scenario("new cart add creates a bound credential and trusted projection", () => { const result = mutate(box); assert.equal(result.outcome, "committed"); assert.equal(result.result_payload.credentialCreated, true); assert.equal(result.result_payload.cart.version, 1); assert.equal(result.result_payload.cart.itemCount, 2); assert.equal(result.result_payload.cart.totalCents, 2264100); assert.equal(result.result_payload.cart.checkoutBlocker,null); assert.equal(result.result_payload.cart.items[0].media.url.startsWith("https://media.saas-staging.celebix.site/"),true); assert.doesNotMatch(JSON.stringify(result), /storeId|credentialDigest|operationId/); });
    ...
    await scenario("cart quote recomputes totals and projects active built-ins", () => { const result = quote(box); assert.equal(result.outcome, "quoted"); assert.deepEqual(result.result_payload.paymentMethods.map(({ kind }) => kind), ["bank_transfer", "cash_on_delivery"]); assert.equal(result.result_payload.cart.shippingCents, 9900); assert.equal(result.result_payload.cart.checkoutBlocker,null); assert.equal(result.result_payload.estimatedDays, 3); });
    ...
    await scenario("missing payment and shipping configuration fail closed", () => { psql(box, `SET ROLE celebix_saas_owner;UPDATE saas.payment_methods SET state='disabled' WHERE store_id='${STORE}';`); assert.equal(quote(box).outcome, "payment_unavailable"); assert.equal(resolveCart(box).result_payload.checkoutReady,false); assert.equal(resolveCart(box).result_payload.checkoutBlocker,"payment_unavailable"); psql(box, `SET ROLE celebix_saas_owner;UPDATE saas.payment_methods SET state='active' WHERE store_id='${STORE}';UPDATE saas.merchant_admin_records SET status='draft' WHERE store_id='${STORE}' AND record_kind='shipping_setting';`); assert.equal(quote(box).outcome, "shipping_unavailable"); assert.equal(resolveCart(box).result_payload.checkoutReady,false); assert.equal(resolveCart(box).result_payload.checkoutBlocker,"shipping_unavailable"); psql(box, `SET ROLE celebix_saas_owner;UPDATE saas.merchant_admin_records SET status='active' WHERE store_id='${STORE}' AND record_kind='shipping_setting';`); });
    await scenario("price drift is retained as one unavailable cart line before checkout writes", () => { psql(box, `SET ROLE celebix_saas_owner;UPDATE saas.product_variants SET price_cents=1200000,version=version+1,updated_at='${LATER}' WHERE id='${VARIANT}';UPDATE saas.payment_methods SET state='disabled' WHERE store_id='${STORE}';`); const projection=resolveCart(box).result_payload; assert.equal(projection.items.length,1); assert.equal(projection.items[0].unitPriceCents,1127100); assert.equal(projection.items[0].available,false); assert.equal(projection.checkoutReady,false); assert.equal(projection.checkoutBlocker,"stock_unavailable"); psql(box, `SET ROLE celebix_saas_owner;UPDATE saas.payment_methods SET state='active' WHERE store_id='${STORE}';`); assert.equal(complete(box).outcome, "price_changed"); assert.equal(psql(box, `SELECT count(*) FROM saas.orders WHERE store_id='${STORE}';`).stdout.trim(), "0"); psql(box, `SET ROLE celebix_saas_owner;UPDATE saas.product_variants SET price_cents=1127100,version=version+1,updated_at='${NOW}' WHERE id='${VARIANT}';`); });
    ...
    await scenario("rollback 073 restores the migration 072 projection without touching commerce data", () => { apply(box, READINESS_DOWN); assert.equal(pgCatalogFunctionBody(box, "saas.storefront_cart_projection(uuid,uuid,timestamp with time zone)").includes("checkoutBlocker"),false); assert.equal(psql(box, `SELECT count(*) FROM saas.orders WHERE store_id='${STORE}';`).stdout.trim(),"3"); apply(box, READINESS_UP); apply(box, READINESS_ASSERTIONS); assert.equal(pgCatalogFunctionBody(box, "saas.storefront_cart_projection(uuid,uuid,timestamp with time zone)").includes("checkoutBlocker"),true); });
    await scenario("rollback removes only migration 072 authority", () => { apply(box, READINESS_DOWN); apply(box, DOWN); assert.equal(psql(box, "SELECT to_regclass('saas.storefront_carts') IS NULL;").stdout.trim(), "t"); assert.equal(psql(box, "SELECT to_regprocedure('saas.public_policy_index(text,timestamp with time zone)') IS NOT NULL;").stdout.trim(), "t"); assert.equal(psql(box, "SELECT 'saas.storefront_checkout_operations'::regclass::oid;").stdout.trim(), legacyCheckoutOperationOid); assert.equal(psql(box, "SELECT to_regclass('saas.storefront_checkout_operations_legacy_064') IS NULL;").stdout.trim(), "t"); });
    await scenario("reapply restores commerce schema and readiness functions", () => { apply(box, UP); apply(box, ASSERTIONS); apply(box, READINESS_UP); apply(box, READINESS_ASSERTIONS); assert.equal(psql(box, "SELECT to_regclass('saas.storefront_carts') IS NOT NULL;").stdout.trim(), "t"); assert.equal(psql(box, "SELECT to_regprocedure('saas.public_checkout_complete(text,timestamp with time zone,text,jsonb,jsonb,uuid,text,bigint,jsonb,text,uuid,uuid,uuid,uuid,uuid,text,text,timestamp with time zone,uuid,text,text,timestamp with time zone)') IS NOT NULL;").stdout.trim(), "t"); assert.equal(pgCatalogFunctionBody(box, "saas.storefront_cart_projection(uuid,uuid,timestamp with time zone)").includes("checkoutBlocker"),true); });
