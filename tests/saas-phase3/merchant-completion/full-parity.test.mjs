...
const POSTGRES_HARNESSES = Object.freeze([
  ...
  ["tests/saas-phase3/quick-order-hosted-payment-authority/postgres-harness.mjs", 16],
...
test("current Phase 3 PostgreSQL inventory is exactly 31 executable harnesses and 893 scenarios", async () => {
  const expectedPaths = POSTGRES_HARNESSES.map(([harness]) => harness);
  assert.equal(POSTGRES_HARNESSES.length, 31);
  assert.equal(POSTGRES_HARNESSES.reduce((total, [, scenarios]) => total + scenarios, 0), 893);
  assert.deepEqual(
