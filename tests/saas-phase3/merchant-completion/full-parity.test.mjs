...
test("current Phase 3 PostgreSQL inventory is exactly 26 executable harnesses and 836 scenarios", async () => {
  const expectedPaths = POSTGRES_HARNESSES.map(([harness]) => harness);
  assert.equal(POSTGRES_HARNESSES.length, 26);
  assert.equal(POSTGRES_HARNESSES.reduce((total, [, scenarios]) => total + scenarios, 0), 836);
  assert.deepEqual(
