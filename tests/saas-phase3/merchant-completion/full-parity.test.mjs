...
test("current Phase 3 PostgreSQL inventory is exactly 31 executable harnesses and 895 scenarios", async () => {
  const expectedPaths = POSTGRES_HARNESSES.map(([harness]) => harness);
  assert.equal(POSTGRES_HARNESSES.length, 31);
  assert.equal(POSTGRES_HARNESSES.reduce((total, [, scenarios]) => total + scenarios, 0), 895);
  assert.deepEqual(
