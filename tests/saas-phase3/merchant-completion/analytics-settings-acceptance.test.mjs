...
test("analytics and typed storefront settings retain durable authority boundaries", async () => {
  const [analyticsSql, analyticsRepository, analyticsRuntime, analyticsHandler, analyticsClient, analyticsPage, settingsSql, settingsValidation, notificationPage, heroPage, promotionPage, marqueePage, merchantConsole] = await Promise.all([
    ...
    source("apps/customer-panel/lib/analytics-http/dashboard-handler.ts"),
  ...
  assert.match(analyticsRuntime, /access\.readiness\.mode\s*!==\s*"approved_staging"/);
  assert.match(analyticsRuntime, /METHODS=\["dashboard","getConnection","getConnectionAuthority","beginConnection","activateConnection","disableConnection"\]/);
  assert.match(analyticsRuntime, /Object\.fromEntries\(METHODS\.map\(\(method\)=>\[method,repository\[method\]\.bind\(repository\)\]\)\)/);
  ...
  assert.match(analyticsPage, /<PanelAnalyticsView/);
  ...
  assert.doesNotMatch(merchantConsole, /sendProvider|completeProvider|executeProvider|providerResponse/);
});
