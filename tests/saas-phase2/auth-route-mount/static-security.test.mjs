...
test("Phase 2B2B2C2 diff is confined to the exact Atlas allowlist", () => {
  const allowed = [
    ...
    /^apps\/customer-panel\/lib\/panel-auth-authority\/(?:config|bootstrap-request-authority|callback-request-authority(?:\.test)?)\.ts$/,
    ...
    /^apps\/customer-panel\/lib\/self-serve-callback-edge\/(?:callback-request(?:\.test)?|edge\.test)\.ts$/,
